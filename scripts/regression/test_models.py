...
class TrainTester:
    ...
    def update_baseline(
        ...
        try:
            ...
            self._update_config_values(
                config, model_key, train_type, test_type, new_loss, new_resume_loss, new_result
            )
    ...
    def assert_loss_consistent(
        ...
        if not resume_loss_matches:
            return (last_training_loss, None), \
        ...
        if abs(last_training_loss - first_resume_loss) > LOSS_TOLERANCE:
            return (last_training_loss, first_resume_loss), \
...
class BaseTrainingTest:
    ...
    def execute_training_workflow(
        ...
        result = self._run_generation_test(
            model_key, generate_dir, expected_result, should_update
        )
