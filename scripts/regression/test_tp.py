...
class PTTrainTest(unittest.TestCase):
    ...
    def test_pt_full(self):
        ...
        update_args = {
            ...
            "train_dataset_path": "./tests/fixtures/dummy/ernie/pt-train.jsonl",
            "eval_dataset_path": "./tests/fixtures/dummy/ernie/pt-train.jsonl",
    ...
    def test_pt_full_function_call(self):
        output_dir = os.path.join(OUTPUT_DIR, "pt_full_function_call")
        update_args = {
            "model_name_or_path": MODEL_NAME_OR_PATH,
        ...
        print(f"cmd {cmd}")
        training_p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        print(f"pt_full_function_call cmd is : {cmd}")
        print(training_p.stdout)
        pt_full_function_call_output = training_p.stdout
        pt_full_function_call_log_file = os.path.join(LOG_PATH, str(os.path.basename(MODEL_NAME_OR_PATH)) + "pt_full_function_call.log")
        if pt_full_function_call_output and pt_full_function_call_output.strip():
            with open(pt_full_function_call_log_file, "w", encoding="utf-8") as pt_full_function_call_f:
                pt_full_function_call_f.write(pt_full_function_call_output)
        # test training result
        self.pttrain_tester.assert_result(training_p.returncode, training_p.stdout)
        ...
        # test model resume
        reusme_p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        print(f"pt_full_function_call reusme cmd is : {cmd}")
        print(reusme_p.stdout)
        pt_full_function_call_reusme_output = reusme_p.stdout
        pt_full_function_call_reusme_log_file = os.path.join(LOG_PATH, str(os.path.basename(MODEL_NAME_OR_PATH)) + "pt_full_function_call_reusme.log")
        if pt_full_function_call_reusme_output and pt_full_function_call_reusme_output.strip():
            with open(
                pt_full_function_call_reusme_log_file, "w", encoding="utf-8"
            ) as pt_full_function_call_reusme_f:
                pt_full_function_call_reusme_f.write(pt_full_function_call_reusme_output)
        self.pttrain_tester.assert_result(reusme_p.returncode, reusme_p.stdout)
