...
template <typename T>
__global__ void
PopulateValidInfoKernel(const T *topk_router_indices,
                        const int *num_activated_offset, T *valid_expert_ids,
                        int *original_token_indices, int num_tokens, int K) {
  int token_idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (token_idx < num_tokens) {
...
__global__ void ComputeXGatherIdxKernel(const int *s_scatter_idx_all,
                                        int *x_gather_idx_all, int K, int n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
cudaError_t launch_simple_arange(int *output, int64_t N,
...
RouterMetadataCuda(const paddle::Tensor &topk_router_indices,
                   const paddle::Tensor &expert_frequency_offset, int K) {
  int64_t num_tokens = topk_router_indices.shape()[0];
  int64_t num_experts = expert_frequency_offset.shape()[0];
  PADDLE_ENFORCE_LE(
      num_tokens * K, static_cast<int64_t>(std::numeric_limits<int>::max()),
  ...
  if (topk_router_indices.numel() == 0) {
    ...
    return {padded_expert_frequency_offset, empty_indices, empty_indices,
            empty_indices, num_activated_per_token_offset};
  }
  ...
  if (num_tokens > 0) {
    CountValidExpertsKernel<T><<<blocks, threads, 0, stream>>>(
        ...
        num_activated_per_token.data<int>(), num_tokens, K);
  ...
  if (num_tokens > 0) {
    cub::DeviceScan::InclusiveSum(
        nullptr, scan_temp_storage_bytes, num_activated_per_token.data<int>(),
    ...
    PD_CHECK(cudaMemsetAsync(num_activated_per_token_offset.data<int>(), 0,
                             sizeof(int), stream) == cudaSuccess,
             "cudaMemsetAsync failed.");

    cub::DeviceScan::InclusiveSum(
        scan_temp_storage, scan_temp_storage_bytes,
  ...
  PopulateValidInfoKernel<T><<<blocks, threads, 0, stream>>>(
      ...
      valid_expert_ids.template data<T>(), original_token_indices.data<int>(),
      num_tokens, K);
  PD_CHECK(cudaGetLastError() == cudaSuccess,
  ...
  PD_CHECK(launch_simple_arange(original_flat_indices_valid.data<int>(),
                                total_valid_experts, stream) == cudaSuccess,
  ...
  cub::DeviceRadixSort::SortPairs<T, int>(
      nullptr, sort_temp_bytes, valid_expert_ids.template data<T>(),
      sorted_valid_expert_ids.template data<T>(),
      original_flat_indices_valid.data<int>(), s_scatter_idx_valid.data<int>(),
      total_valid_experts, 0, sizeof(T) * 8, stream);
  ...
  cub::DeviceRadixSort::SortPairs<T, int>(
      sort_temp_storage, sort_temp_bytes, valid_expert_ids.template data<T>(),
      sorted_valid_expert_ids.template data<T>(),
      original_flat_indices_valid.data<int>(), s_scatter_idx_valid.data<int>(),
      total_valid_experts, 0, sizeof(T) * 8, stream);
  PD_CHECK(cudaGetLastError() == cudaSuccess,
  ...
  GenerateReverseScatterKernel<<<blocks_valid_out, threads, 0, stream>>>(
      s_scatter_idx_valid.data<int>(), s_reverse_scatter_idx_valid.data<int>(),
      total_valid_experts);
  PD_CHECK(cudaGetLastError() == cudaSuccess,
  ...
  PD_CHECK(launch_simple_arange(original_full_indices.data<int>(),
                                total_elements, stream) == cudaSuccess,
  ...
  cub::DeviceRadixSort::SortPairs<T, int>(
      nullptr, sort_all_bytes, topk_router_indices_flat.template data<T>(),
      sorted_topk_indices_temp.template data<T>(),
      original_full_indices.data<int>(), s_scatter_idx_all.data<int>(),
      total_elements, 0, sizeof(T) * 8, stream);
  ...
  cub::DeviceRadixSort::SortPairs<T, int>(
      sort_all_temp_storage, sort_all_bytes,
  ...
  ComputeXGatherIdxKernel<<<blocks_all_out, threads, 0, stream>>>(
      s_scatter_idx_all.data<int>(), x_gather_idx_all.data<int>(), K,
      total_elements);
  PD_CHECK(cudaGetLastError() == cudaSuccess,
  ...
  return {padded_expert_frequency_offset, x_gather_idx, s_scatter_idx_valid,
          s_reverse_scatter_idx_valid, num_activated_per_token_offset};
}

std::vector<paddle::Tensor>
RouterMetadataDispatch(const paddle::Tensor &topk_router_indices,
                       const paddle::Tensor &expert_frequency_offset, int K) {
  if (topk_router_indices.dtype() == paddle::DataType::INT32) {
    return RouterMetadataCuda<int>(topk_router_indices, expert_frequency_offset,
                                   K);
  } else if (topk_router_indices.dtype() == paddle::DataType::INT64) {
    return RouterMetadataCuda<int64_t>(topk_router_indices,
                                       expert_frequency_offset, K);
  } else {
    PD_THROW(
...
PD_BUILD_OP(router_metadata)
    ...
    .Outputs({"PaddedExpertFrequencyOffset", "XGatherIdx", "SScatterIdxValid",
              "SReverseScatterIdxValid", "NumActivatedExpertPerTokenOffset"})
    .Attrs({"K: int"})
