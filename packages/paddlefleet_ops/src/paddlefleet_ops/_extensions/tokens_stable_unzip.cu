...
#
#     http://www.apache.org/licenses/LICENSE-2.0  // NOLINT
#
...
*/
#include "paddle/common/array.h"                     // NOLINT
...
template <int MAX_NUM_EXPERTS> struct __align__(16) expert_base_offset {
  int data[MAX_NUM_EXPERTS];
};

template <typename X_T, typename routemap_T, typename probs_T, bool has_scale,
          bool fill_x, int MAX_NUM_EXPERTS_C>
__global__ void tokens_unzip_stable_kernel(
    const X_T *__restrict__ X, const routemap_T *__restrict__ routemap_topk,
    const probs_T *__restrict__ probs_topk, const float *__restrict__ XScale,
    const expert_base_offset<MAX_NUM_EXPERTS_C> expert_base_offset,
    X_T *__restrict__ X_unzipped, int *__restrict__ zipped_expertwise_rowmap,
    probs_T *__restrict__ probs_unzipped, float *__restrict__ XScale_unzipped,
    int *global_expertwise_block_cumsum, const int total_zipped_tokens_num,
    const int token_length, const int scale_length, const int num_experts,
    const int topk) {
...
#pragma unroll
    ...
    for (int row = block_row_base; row < block_row_base + CUMSUM_BLOCK_SIZE;
         row++) {
      if (row >= total_zipped_tokens_num)
        break;
...
void dispatch_tokens_unzip_stable(
    const paddle::Tensor &X, const paddle::Tensor &expert_routemap_topk,
    ...
    const int total_zipped_tokens_num, const int token_length,
    const int topk, // deprecated  // NOLINT
    const int num_experts, const int scale_length, const bool fill_x) {
  dim3 grid, block;
  grid.x =
...
#define DISPATCH_CASE_IMPL(TOKEN_T, PROB_T, INT_T, HAS_SCALE, FILL_X)          \
  do {                                                                         \
    ...
    kernel<<<grid, block, 0, X.stream()>>>(                                    \
        ...
        XScale ? XScale->data<float>() : nullptr, expert_offsets,              \
        GET_DATA(X_unzipped, TOKEN_T),                                         \
        GET_DATA(zipped_expertwise_rowmap, INT_T),                             \
        GET_DATA(token_prob_unzipped, PROB_T), XScale_unzipped.data<float>(),  \
        global_expertwise_block_cumsum.data<int>(), total_zipped_tokens_num,   \
        token_length, scale_length, num_experts, topk);                        \
  } while (0)
...
std::vector<paddle::Tensor> tokens_unzip_stable(
    const paddle::Tensor &X, const paddle::optional<paddle::Tensor> &XScale,
    const paddle::Tensor &expert_routemap_topk,
    const paddle::Tensor &expert_prob_topk, const int &topk,
    const int &num_experts, const std::vector<int> &tokens_per_expert,
    const int padding_multiplex, const bool fill_x) {
  ...
  */
  ...
  PD_SWITCH_NUM_EXPERTS(
      num_experts, ([&] {
        ...
        if (XScale && fill_x) {
          XScale_unzipped = paddle::empty({output_rows, quanted_cols},
                                          XScale->dtype(), XScale->place());
        } else {
          XScale_unzipped =
        ...
        if (fill_x) {
          if (X.dtype() == paddle::DataType::BFLOAT16) {
            ...
            cudaMemsetAsync(X_unzipped_ptr, 0,
          ...
          if (XScale) {
            ...
            cudaMemsetAsync(XScale_unzipped_ptr, 0,
        ...
        if (expert_prob_topk.dtype() == paddle::DataType::BFLOAT16) {
          ...
          cudaMemsetAsync(token_prob_unzipped_ptr, 0,
        ...
        if (rows != 0) {
          dispatch_tokens_unzip_stable<MAX_NUM_EXPERTS_C>(
              X, expert_routemap_topk, expert_prob_topk, XScale, expert_offset,
              X_unzipped, zipped_expertwise_rowmap, token_prob_unzipped,
              XScale_unzipped, global_expertwise_block_cumsum, rows, cols,
              topk_calculated, num_experts, quanted_cols, fill_x);
        }
      }));
  return {X_unzipped, zipped_expertwise_rowmap, token_prob_unzipped,
          XScale_unzipped};
}

PD_BUILD_OP(tokens_unzip_stable)
    .Inputs({"X", paddle::Optional("Xscale"), "expert_routemap_topk",
             "expert_prob_topk"})
    .Outputs({"X_unzipped", "zipped_expertwise_rowmap", "token_prob_unzipped",
              paddle::Optional("XScale_unzipped")})
    .Attrs({"topk: int", "num_experts: int",
            "tokens_per_expert: std::vector<int>", "padding_multiplex: int",
            "fill_output: bool"})
