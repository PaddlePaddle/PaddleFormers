...
template <typename T> struct UnzippedProbInfo {
...
__global__ void tokens_zip_prob_kernel(
    ...
    const int *__restrict__ dispatched_indices, T *zipped_probs,
    int64_t zipped_rows, int topk, int num_expert) {
...
tokens_zip_prob_impl(const std::vector<paddle::Tensor> &unzipped_probs,
        ...
        if (grid > 0) {
          tokens_zip_prob_kernel<T, MAX_NUM_EXPERTS_C>
              <<<grid, thread, 0, zipped_probs.stream()>>>(
                  unzipped_probs_info, zipped_expertwise_rowmap.data<int>(),
                  dispatched_indices.data<int>(), zipped_probs.data<T>(),
                  zipped_rows, topk_int, num_expert_int);
        }
      }));
  return {zipped_probs};
}
...
std::vector<paddle::Tensor>
tokens_zip_prob(const std::vector<paddle::Tensor> &unzipped_probs,
                const paddle::Tensor &zipped_expertwise_rowmap,
                const paddle::Tensor &dispatched_indices) {
  ...
  if (dtype == paddle::DataType::FLOAT32) {
    return tokens_zip_prob_impl<float>(unzipped_probs, zipped_expertwise_rowmap,
...
__global__ void tokens_zip_prob_seq_subbatch_kernel(
    ...
    const int *__restrict__ zipped_expertwise_rowmap,
    const int *__restrict__ dispatched_indices, T *zipped_probs,
    int64_t zipped_rows, int topk, int num_expert, int64_t subbatch_rows) {
  int64_t idx = threadIdx.x + static_cast<int64_t>(blockDim.x) * blockIdx.x;
  int64_t stride = static_cast<int64_t>(blockDim.x) * gridDim.x;
  int64_t limit = zipped_rows * topk;
  while (idx < limit) {
...
std::vector<paddle::Tensor> tokens_zip_prob_seq_subbatch_impl(
    ...
    const paddle::Tensor &zipped_expertwise_rowmap,
    const paddle::Tensor &dispatched_indices, int64_t subbatch_rows,
    paddle::DataType dtype) {
  auto zipped_expertwise_rowmap_shape = zipped_expertwise_rowmap.shape();
  auto dispatched_indices_shape = dispatched_indices.shape();
  PD_CHECK(zipped_expertwise_rowmap_shape.size() == 2);
...
#define LAUNCH_TOKENS_ZIP_PROB_SEQ_SUBBATCH_DYNAMIC_CASE(__T)                  \
  ...
  auto unzipped_probs_info = GetTensorDevicePtrs<const __T>(                   \
      unzipped_probs, &ptr_tensor, zipped_probs.stream(),                      \
      zipped_probs.place());                                                   \
  LAUNCH_TOKENS_ZIP_PROB_SEQ_SUBBATCH_CASE_IMPL(__T, unzipped_probs_info);     \
  break
  ...
  do {                                                                         \
    if (grid > 0) {                                                            \
      tokens_zip_prob_seq_subbatch_kernel<                                     \
          __T, typename std::remove_reference<                                 \
                   decltype(__unzipped_probs_info)>::type>                     \
          <<<grid, thread, 0, zipped_probs.stream()>>>(                        \
              __unzipped_probs_info, zipped_expertwise_rowmap.data<int>(),     \
              dispatched_indices.data<int>(), zipped_probs.data<__T>(),        \
              zipped_rows, topk_int, num_expert_int, subbatch_rows);           \
    }                                                                          \
  } while (0)
...
std::vector<paddle::Tensor>
tokens_zip_prob_seq_subbatch(const std::vector<paddle::Tensor> &unzipped_probs,
                             const paddle::Tensor &zipped_expertwise_rowmap,
                             const paddle::Tensor &dispatched_indices,
  ...
  auto dtype = unzipped_probs[0].dtype();
  if (dtype == paddle::DataType::FLOAT32) {
    return tokens_zip_prob_seq_subbatch_impl<float>(
        unzipped_probs, zipped_expertwise_rowmap, dispatched_indices,
        subbatch_rows, dtype);
  } else if (dtype == paddle::DataType::BFLOAT16) {
...
PD_BUILD_OP(tokens_zip_prob)
    .Inputs({paddle::Vec("unzipped_prob"), "zipped_expertwise_rowmap",
             "dispatched_indices"})
    .Outputs({"zipped_prob"})
