...
def router_metadata(topkrouterindices,expertfrequencyoffset,k):
    ...
    if in_dynamic_or_pir_mode():
        ...
        outs = {}
        outs_list = ['PaddedExpertFrequencyOffset','XGatherIdx','SScatterIdxValid','SReverseScatterIdxValid','NumActivatedExpertPerTokenOffset']
        for key, value in ins_map.items():
            # handle optional inputs
            if value is not None:
                ins[key] = value
...
@unified
def tokens_unzip_stable(x,xscale,expert_routemap_topk,expert_prob_topk,topk,num_experts,tokens_per_expert,padding_multiplex,fill_output):
    # The output variable's dtype use default value 'float32',
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("tokens_unzip_stable", x,xscale,expert_routemap_topk,expert_prob_topk,topk,num_experts,tokens_per_expert,padding_multiplex,fill_output)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        ins = {}
        ins_map = {'X' : x,'Xscale@OPTIONAL' : xscale,'expert_routemap_topk' : expert_routemap_topk,'expert_prob_topk' : expert_prob_topk}
        outs = {}
        ...
        helper.append_op(type="tokens_unzip_stable", inputs=ins, outputs=outs, attrs={'topk' : topk,'num_experts' : num_experts,'tokens_per_expert' : tokens_per_expert,'padding_multiplex' : padding_multiplex,'fill_output' : fill_output})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def tokens_unzip_gather(x,x_scale,zipped_expertwise_rowmap,expert_id,tokens_per_expert,padding_multiplex):
    ...
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("tokens_unzip_gather", x,x_scale,zipped_expertwise_rowmap,expert_id,tokens_per_expert,padding_multiplex)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        helper.append_op(type="tokens_unzip_gather", inputs=ins, outputs=outs, attrs={'expert_id' : expert_id,'tokens_per_expert' : tokens_per_expert,'padding_multiplex' : padding_multiplex})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def tokens_zip_prob_seq_subbatch(unzipped_prob,zipped_expertwise_rowmap,dispatched_indices,subbatch_rows):
    ...
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("tokens_zip_prob_seq_subbatch", unzipped_prob,zipped_expertwise_rowmap,dispatched_indices,subbatch_rows)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        ins = {}
        ins_map = {'unzipped_prob@VECTOR' : unzipped_prob,'zipped_expertwise_rowmap' : zipped_expertwise_rowmap,'dispatched_indices' : dispatched_indices}
        outs = {}
        ...
        helper.append_op(type="tokens_zip_prob_seq_subbatch", inputs=ins, outputs=outs, attrs={'subbatch_rows' : subbatch_rows})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def fuse_weighted_swiglu_fp8_quant(expert_out_list,prob,using_pow2_scaling,use_ue8m0):
    ...
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("fuse_weighted_swiglu_fp8_quant", expert_out_list,prob,using_pow2_scaling,use_ue8m0)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        helper.append_op(type="fuse_weighted_swiglu_fp8_quant", inputs=ins, outputs=outs, attrs={'using_pow2_scaling' : using_pow2_scaling,'use_ue8m0' : use_ue8m0})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def tokens_unzip_slice(x,zipped_expertwise_rowmap,num_experts,total_unzipped_rows,start_idx,end_idx):
    ...
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("tokens_unzip_slice", x,zipped_expertwise_rowmap,num_experts,total_unzipped_rows,start_idx,end_idx)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        outs['idx_unzipped'] = helper.create_variable(dtype='float32')
        helper.append_op(type="tokens_unzip_slice", inputs=ins, outputs=outs, attrs={'num_experts' : num_experts,'total_unzipped_rows' : total_unzipped_rows,'start_idx' : start_idx,'end_idx' : end_idx})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def tokens_zip_unique_add_subbatch(x_zipped,x_unzipped,idx_unzipped,zipped_rows,subbatch_rows):
    ...
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("tokens_zip_unique_add_subbatch", x_zipped,x_unzipped,idx_unzipped,zipped_rows,subbatch_rows)
        res = []
        start_idx = 0
        ...
        helper.append_op(type="tokens_zip_unique_add_subbatch", inputs=ins, outputs=outs, attrs={'zipped_rows' : zipped_rows,'subbatch_rows' : subbatch_rows})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def fuse_transpose_split_fp8_quant(x,input_scales,outs,scales,tokens_per_expert,pow_2_scales,use_ue8m0):
    ...
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("fuse_transpose_split_fp8_quant", x,input_scales,outs,scales,tokens_per_expert,pow_2_scales,use_ue8m0)
        res = []
        start_idx = 0
        return res[0] if len(res)==1 else res
    else:
        ...
        helper.append_op(type="fuse_transpose_split_fp8_quant", inputs=ins, outputs=outs, attrs={'tokens_per_expert' : tokens_per_expert,'pow_2_scales' : pow_2_scales,'use_ue8m0' : use_ue8m0})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def fuse_stack_fp8_quant(x,using_pow2_scaling,using_ue8m0_scale,output_scale_transpose):
    ...
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("fuse_stack_fp8_quant", x,using_pow2_scaling,using_ue8m0_scale,output_scale_transpose)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        outs['scale'] = helper.create_variable(dtype='float32')
        helper.append_op(type="fuse_stack_fp8_quant", inputs=ins, outputs=outs, attrs={'using_pow2_scaling' : using_pow2_scaling,'using_ue8m0_scale' : using_ue8m0_scale,'output_scale_transpose' : output_scale_transpose})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
...
def fuse_stack_transpose_fp8_quant(x,using_pow2_scaling,using_ue8m0_scale,output_scale_transpose):
    ...
    # and the actual dtype of output variable will be inferred in runtime.
    if in_dynamic_or_pir_mode():
        outs = _C_ops._run_custom_op("fuse_stack_transpose_fp8_quant", x,using_pow2_scaling,using_ue8m0_scale,output_scale_transpose)
        res = []
        start_idx = 0
        res.append(outs[start_idx])
        start_idx += 1
        ...
        outs['output'] = helper.create_variable(dtype='float32')
        outs['scale'] = helper.create_variable(dtype='float32')
        helper.append_op(type="fuse_stack_transpose_fp8_quant", inputs=ins, outputs=outs, attrs={'using_pow2_scaling' : using_pow2_scaling,'using_ue8m0_scale' : using_ue8m0_scale,'output_scale_transpose' : output_scale_transpose})
        res = [outs[out_name] if out_name in outs.keys() else None for out_name in outs_list]
        return res[0] if len(res)==1 else res
