...
__global__ void tokens_unzip_slice_kernel(
    const T *__restrict__ x, const int *__restrict__ zipped_expertwise_rowmap,
    int64_t *__restrict__ index_out, int64_t total_zipped_rows, int num_experts,
  ...
  for (int64_t elem =
       ...
       elem < total; elem += static_cast<int64_t>(blockDim.x) * gridDim.x) {
...
tokens_unzip_slice(const paddle::Tensor &x,
                   const paddle::Tensor &zipped_expertwise_rowmap,
                   const int num_experts, const int total_unzipped_rows,
...
#define LAUNCH_TOKENS_UNZIP_SLICE_KERNEL_IMPL(__cpp_dtype)                     \
  do {                                                                         \
    tokens_unzip_slice_kernel<__cpp_dtype><<<grid, block, 0, stream>>>(        \
        x.data<__cpp_dtype>(), zipped_expertwise_rowmap.data<int>(),           \
        index_unzipped.data<int64_t>(), total_zipped_rows, num_experts,        \
        start_idx, end_idx);                                                   \
  } while (0)
...
PD_BUILD_OP(tokens_unzip_slice)
    ...
    .Attrs({"num_experts: int", "total_unzipped_rows: int", "start_idx: int",
            "end_idx: int"})
