...
#define LAUNCH_FUSED_SPAQ(__using_pow2_scaling, __with_prob, __k_has_clamp)    \
  do {                                                                         \
    auto kernel = FusedSPAQKernel<__using_pow2_scaling, __with_prob, ScaleT,   \
                                  use_ue8m0, __k_has_clamp>;                   \
    kernel<<<grid, block, 0, stream>>>(x_data, prob_data, out_data,            \
                                       scale_data, rows, cols, clamp_value);   \
  } while (0)

#define LAUNCH_FUSED_SPAQ_VEC4(__using_pow2_scaling, __with_prob,              \
                               __k_has_clamp)                                  \
  do {                                                                         \
    auto kernel = FusedSPAQKernelVec4<__using_pow2_scaling, __with_prob,       \
                                      thread_per_block, ScaleT, use_ue8m0,     \
                                      __k_has_clamp>;                          \
    kernel<<<grid, block, 0, stream>>>(x_data, prob_data, out_data,            \
                                       scale_data, rows, cols, scale_cols,     \
                                       clamp_value);                           \
  } while (0)
...
#define LAUNCH_FUSED_SPAQ_VEC8(__using_pow2_scaling, __with_prob,              \
                               __k_has_clamp)                                  \
  do {                                                                         \
    auto kernel = FusedSPAQKernelVec8<__using_pow2_scaling, __with_prob,       \
                                      thread_per_block, ScaleT, use_ue8m0,     \
                                      __k_has_clamp>;                          \
    kernel<<<grid, block, 0, stream>>>(x_data, prob_data, out_data,            \
                                       scale_data, rows, cols, scale_cols,     \
...
#define LAUNCH_FUSED_SPAQ_SCALAR(__using_pow2_scaling, __with_prob,            \
                                 __k_has_clamp)                                \
  do {                                                                         \
    auto kernel = FusedSPAQKernel<__using_pow2_scaling, __with_prob, ScaleT,   \
                                  use_ue8m0, __k_has_clamp>;                   \
    ...
    for (int64_t row_offset = 0; row_offset < rows;                            \
      ...
      kernel<<<grid, block, 0, stream>>>(x_chunk, prob_chunk, out_chunk,       \
                                         scale_chunk, chunk_rows, cols,        \
                                         clamp_value);                         \
    }                                                                          \
  } while (0)
...
fast_swiglu_vec4(const bfloat16x4_t &lhs, const bfloat16x4_t &rhs,
                 const double clamp_value = 0.0) {
  float cv = static_cast<float>(clamp_value);
...
template <bool using_pow2_scaling, bool with_prob, int thread_per_block,
          typename ScaleT, bool ue8m0, bool kHasClamp = false>
__global__ void FusedSPAQKernelVec4(const phi::bfloat16 *__restrict__ Xin,
  ...
  const unsigned int mask = 0xffffffff; // whole warp mask  // NOLINT
...
__global__ void FusedSPAQKernel(const phi::bfloat16 *__restrict__ Xin,
                                const float *__restrict__ prob,
                                phi::float8_e4m3fn *__restrict__ out,
                                ScaleT *__restrict__ scales, const int64_t rows,
                                const int64_t cols,
                                const double clamp_value = 0.0) {
  // Configure shared memory
  __shared__ float
      smem_tile[256]; // Shared memory for activation values  // NOLINT
  __shared__ float warp_max[2][4]; // NOLINT
  __shared__ __nv_bfloat16
      quant_block_amax[2]; // Shared memory for quant block maxima  // NOLINT

  const __nv_bfloat16 *X = reinterpret_cast<const __nv_bfloat16 *>(Xin);
  const int x_offset = threadIdx.x;
  const int quant_block_idx =
      threadIdx.x / 128; // 0 or 1, two quant blocks per block  // NOLINT
  ...
  if (in_x_idx < cols / 2) [[likely]] { // NOLINT
    __nv_bfloat16 x1 = X[src_idx];      // First half of the input  // NOLINT
    __nv_bfloat16 x2 =
        X[src_idx + cols / 2]; // Second half of the input  // NOLINT
  ...
  __syncthreads(); // Ensure all threads have loaded their data  // NOLINT
  ...
  int warp_id = (threadIdx.x % 128) /
                32; // Warp ID within the quant block (0-3)  // NOLINT
  ...
  if (warp_id == 0 && lane < 4) {
    if (threadIdx.x < 256) { // Ensure only valid threads participate  // NOLINT
...
void dispatch_fused_spaq(const phi::bfloat16 *x_data, const float *prob_data,
                         phi::float8_e4m3fn *out_data, void *void_scale_data,
                         cudaStream_t stream, const int64_t rows,
                         const int64_t cols, const bool &using_pow2_scaling,
  ...
  if (cols % 16 == 0) {
    ...
    DISPATCH_BOOL(
        ...
        DISPATCH_BOOL(with_prob, k_with_prob,
                      grid.y = rows > 65535 ? 65535
    ...
    DISPATCH_BOOL(
        using_pow2_scaling, k_using_pow2_scaling,
        DISPATCH_BOOL(with_prob, k_with_prob,
                      LAUNCH_FUSED_SPAQ_SCALAR(k_using_pow2_scaling,
                                               k_with_prob, kHasClamp);))
  }
}
...
static std::vector<paddle::Tensor> FusedWeightedSwigluActQuantImpl(
    const paddle::Tensor &x, const paddle::optional<paddle::Tensor> &prob,
    const bool using_pow2_scaling, const bool use_ue8m0,
  ...
  if (prob) {
    PADDLE_ENFORCE_EQ(prob.get().dtype(), phi::DataType::FLOAT32,
  ...
  if (use_ue8m0) {
    ...
    PADDLE_ENFORCE_LE(
        rows, kMaxRowsForUe8m0ScaleIndex,
        common::errors::InvalidArgument(
    ...
    out = GetEmptyTensor({token_num, hidden_size / 2},
                         phi::DataType::FLOAT8_E4M3FN, x.place());
    ...
    scale = GetEmptyTensor(
        {padded_token_num, ceil_div_i64(hidden_size_scale, 4)},
        {1, padded_token_num}, paddle::DataType::INT32, x.place());
    if (token_num == 0 || hidden_size == 0) {
      return {out, scale};
    }
  } else {
    scale = GetEmptyTensor({rows, (cols / 2 + 127) / 128},
                           phi::DataType::FLOAT32, place);
  ...
  if (use_ue8m0 && cols % 8 == 0) {
    dispatch_fused_spaq<phi::float8_e4m3fn, true, kHasClamp>(
        x_data, prob_data, out_data, scale_data, stream, rows, cols,
        using_pow2_scaling, !!prob, clamp_value);
  } else {
    dispatch_fused_spaq<float, false, kHasClamp>(
        x_data, prob_data, out_data, scale_data, stream, rows, cols,
        using_pow2_scaling, !!prob, clamp_value);
  }
