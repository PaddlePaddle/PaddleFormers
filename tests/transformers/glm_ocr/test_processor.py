...
class Glm46VProcessorTest(ProcessorTesterMixin, unittest.TestCase):
    ...
    def test_save_load_pretrained_default(self):
        ...
        self.assertEqual(
            processor.image_processor.__class__.__name__, "Glm46VImageProcessor"
        )
    ...
    def test_image_token_expansion(self):
        ...
        expected_num_image_tokens = (
            inputs["image_grid_thw"][0].prod().item() // merge_length
        )
        actual_num_image_tokens = (
            (inputs["input_ids"] == processor.image_token_id).sum().item()
        )
    ...
    def test_multiple_images(self):
        ...
        inputs = processor(
            text=input_str, images=two_images, return_tensors="pd"
        )

        merge_length = processor.image_processor.merge_size**2
        expected_total = sum(
            thw.prod().item() // merge_length
