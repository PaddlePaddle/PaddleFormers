...
from paddleformers.transformers import (  # Qwen2_5_VLConfig,; Qwen2_5_VLModel,
    AutoProcessor,
...
class Qwen2_5_VLIntegrationTest(unittest.TestCase):
    def setUp(self):
        ...
        logger.info(f"Qwen2_5_VLIntegrationTest [PID:{os.getpid()}] Pre device: {paddle.get_device()}")
        if paddle.device.cuda.device_count() > 0:
            paddle.set_device("gpu")
            logger.info(f"Set device to GPU: {paddle.get_device()}")
        else:
            paddle.set_device("cpu")
            logger.info("No GPU available, using CPU")
        logger.info(f"Qwen2_5_VLIntegrationTest [PID:{os.getpid()}] Post device: {paddle.get_device()}")
