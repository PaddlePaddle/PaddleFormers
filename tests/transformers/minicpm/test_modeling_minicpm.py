...
class MiniCPMCompatibilityTest(unittest.TestCase):
    test_model_id = "hf-internal-testing/tiny-random-MiniCPMModel"
    ...
    @require_package("transformers", "torch")
    def test_MiniCPM_converter(self):
        # 1. create common input
        input_ids = np.random.randint(100, 200, [1, 20])

        # 2. forward the paddle model
        from paddleformers.transformers import MiniCPMModel

        paddle_model = MiniCPMModel.from_pretrained(self.torch_model_path, convert_from_hf=True, dtype="float32")
        paddle_model.eval()
    ...
    @require_package("transformers", "torch")
    def test_MiniCPM_converter_from_local_dir(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            paddle_model = MiniCPMModel.from_pretrained(tempdir, convert_from_hf=True, dtype="float32")
            paddle_model.eval()
            paddle_logit = paddle_model(paddle.to_tensor(input_ids))[0]
            ...
            uc_load_model = MiniCPMForCausalLM.from_pretrained(
                self.torch_model_path,
                convert_from_hf=True,
                dtype="float32",
                load_checkpoint_format="unified_checkpoint",
            )
            fc_load_model = MiniCPMForCausalLM.from_pretrained(
            ...
            model_config = MiniCPMConfig.from_pretrained(self.torch_model_path)
            model_config.fuse_attention_qkv = True
            model_config.fuse_attention_ffn = True
            fc_fused_load_model = MiniCPMForCausalLM.from_pretrained(
                self.torch_model_path,
                config=model_config,
                convert_from_hf=True,
                dtype="float32",
    ...
    @parameterized.expand([("MiniCPMModel",), ("MiniCPMForCausalLM",)])
    @require_package("transformers", "torch")
    def test_MiniCPM_classes_from_local_dir(self, class_name, pytorch_class_name: str | None = None):
