...
def prepare_video(
    num_frames, num_channels, width=10, height=10, return_tensors="pil"
):
    ...
    for i in range(num_frames):
        video.append(
            np.random.randint(
                255, size=(width, height, num_channels), dtype=np.uint8
            )
        )
...
class VideoProcessingTestMixin:
    ...
    def test_video_processor_to_json_file(self):
        for video_processing_class in self.video_processor_list:
            video_processor_first = video_processing_class(
                **self.video_processor_dict
            )
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                video_processor_second = video_processing_class.from_json_file(
                    json_file_path
                )

            self.assertEqual(
                video_processor_second.to_dict(),
                video_processor_first.to_dict(),
            )
    ...
    def test_video_processor_from_dict_with_kwargs(self):
        ...
        video_processor = self.fast_video_processing_class.from_dict(
            self.video_processor_dict, size=42, crop_size=84
        )
    ...
    def test_video_processor_from_and_save_pretrained(self):
        for video_processing_class in self.video_processor_list:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                video_processor_second = video_processing_class.from_pretrained(
                    tmpdir
                )
    ...
    def test_video_processor_save_load_with_autovideoprocessor(self):
        for video_processing_class in self.video_processor_list:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                video_processor_second = AutoVideoProcessor.from_pretrained(
                    tmpdir
                )
    ...
    def test_cast_dtype_device(self):
        for video_processing_class in self.video_processor_list:
            if self.test_cast_dtype is not None:
                ...
                self.assertNotEqual(
                    encoding[self.input_name].place, paddle.CPUPlace()
                )
                self.assertEqual(
                    encoding[self.input_name].dtype, paddle.float32
                )

                encoding = video_processor(
                ...
                self.assertEqual(
                    encoding[self.input_name].dtype, paddle.float16
                )
                ...
                self.assertEqual(
                    encoding[self.input_name].dtype, paddle.bfloat16
                )

                # Test text + video feature case
                encoding = video_processor(video_inputs, return_tensors="pd")
                encoding.update(
                    {"input_ids": paddle.Tensor([[1, 2, 3], [4, 5, 6]])}
                )
    ...
    def test_call_numpy(self):
        for video_processing_class in self.video_processor_list:
            ...
            self.assertEqual(
                tuple(encoded_videos.shape), (1, *expected_output_video_shape)
            )
            ...
            self.assertEqual(
                ...
                (
                    self.video_processor_tester.batch_size,
                    *expected_output_video_shape,
                ),
            )
    ...
    def test_call_sample_frames(self):
        for video_processing_class in self.video_processor_list:
            ...
            encoded_videos = video_processing(
                video_inputs[0], return_tensors="pd", num_frames=3
            )[self.input_name]
            ...
            metadata = [
                [VideoMetadata(duration=2.0, total_num_frames=8, fps=4)]
            ]
            # batched_metadata = metadata * len(video_inputs)
            encoded_videos = video_processing(
                video_inputs[0],
            ...
            with self.assertRaises(ValueError):
                encoded_videos = video_processing(
                    video_inputs[0], return_tensors="pd", num_frames=10
                )[self.input_name]
    ...
    def test_video_processor_preprocess_arguments(self):
        ...
        for video_processing_class in self.video_processor_list:
            ...
            if hasattr(video_processor, "_valid_processor_keys") and hasattr(
            ...
            if hasattr(
                video_processor.preprocess, "_filter_out_non_signature_kwargs"
            ):
                if hasattr(self.video_processor_tester, "prepare_video_inputs"):
                    ...
                    self.skipTest(
                        reason="No valid input preparation method found"
                    )
    ...
    def test_override_instance_attributes_does_not_affect_other_instances(self):
        ...
        if not (
            hasattr(video_processor_1, "size")
            and isinstance(video_processor_1.size, dict)
        ) or not (
            hasattr(video_processor_1, "image_mean")
            and isinstance(video_processor_1.image_mean, list)
        ):
            self.skipTest(
