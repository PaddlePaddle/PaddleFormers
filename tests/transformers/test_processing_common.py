...
class ProcessorTesterMixin:
    ...
    def get_component(self, attribute, **kwargs):
        if (
            attribute not in MODALITY_TO_AUTOPROCESSOR_MAPPING
            and "tokenizer" in attribute
        ):
            auto_processor_class = MODALITY_TO_AUTOPROCESSOR_MAPPING[
                "tokenizer"
            ]
            component = auto_processor_class.from_pretrained(
                self.tmpdir, subfolder=attribute, **kwargs
            )
        else:
    ...
    def get_processor(self):
        ...
        processor = self.processor_class(
            **components, **self.prepare_processor_dict()
        )
        return processor
    ...
    def prepare_text_inputs(
        ...
        return [
            ...
            f" {special_token_to_add} upper older longer string",
        ] + [f"lower newer {special_token_to_add}"] * (batch_size - 2)
    ...
    def prepare_audio_inputs(self, batch_size: int | None = None):
        ...
        audio_array = np.random.randn(int(sampling_rate * duration)).astype(
            np.float32
        )
        if batch_size is None:
            return audio_array
        ...
        for key, value in self.prepare_processor_dict().items():
            ...
            if key not in "chat_template":
                ...
                if (
                    ...
                    and isinstance(next(iter(obj[key].keys())), str)
                    and isinstance(next(iter(value.keys())), int)
                ):
    ...
    def test_processor_from_and_save_pretrained_as_nested_dict(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            saved_files = processor_first.save_pretrained(
                tmpdir, legacy_serialization=False
            )
            check_json_file_has_correct_format(saved_files[0])
            ...
            for attribute in processor_first.get_attributes():
                if (
                    ...
                    attribute_reloaded = auto_processor_class.from_pretrained(
                        tmpdir, subfolder=attribute
                    )
                else:
                    auto_processor_class = MODALITY_TO_AUTOPROCESSOR_MAPPING[
                        attribute
                ...
                if "tokenizer" not in attribute:
                    self.assertEqual(
                        repr(attribute_first), repr(attribute_reloaded)
                    )
    ...
    def test_unstructured_kwargs_batched(self):
        ...
        self.assertLessEqual(inputs[self.images_input_name][0][0].mean(), 0)
        self.assertTrue(
            len(inputs[self.text_input_name][0])
            == len(inputs[self.text_input_name][1])
    ...
    def test_args_overlap_kwargs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            processor_second = self.processor_class.from_pretrained(
                tmpdir, image_processor=image_processor
            )
    ...
    def test_tokenizer_defaults_preserved_by_kwargs_video(self):
        if "video_processor" not in self.processor_class.attributes:
            self.skipTest(
                f"video_processor attribute not present in {self.processor_class}"
            )
        processor_components = self.prepare_components()
        processor_components["tokenizer"] = self.get_component(
            "tokenizer", max_length=167, padding="max_length"
        )
        processor_kwargs = self.prepare_processor_dict()
    ...
    def test_prepare_and_validate_optional_call_args(self):
        ...
        self.assertEqual(
            optional_call_args,
            {
                arg_name: f"optional_{i}"
                for i, arg_name in enumerate(optional_call_args_name)
            },
        )
        ...
        self.assertEqual(
            optional_call_args, {optional_call_args_name[0]: "optional_1"}
        )
    ...
    def test_chat_template_save_loading(self):
        ...
        existing_tokenizer_template = getattr(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                processor.chat_template, reloaded_processor.chat_template
            )
            # When we don't use single-file chat template saving, processor and tokenizer chat templates
            # should remain separate
            self.assertEqual(
                getattr(reloaded_processor.tokenizer, "chat_template", None),
                existing_tokenizer_template,
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                reloaded_processor.chat_template,
                reloaded_processor.tokenizer.chat_template,
            )
    ...
    def _test_apply_chat_template(
        ...
        if getattr(processor, processor_name).__class__.__name__.endswith(
            "Fast"
        ):
            return_tensors = "pd"
        ...
        for idx, url in enumerate(input_data[:batch_size]):
            batch_messages[idx][1]["content"] = [
                batch_messages[idx][1]["content"][0],
        ...
        continue_prompt = processor.apply_chat_template(
            batch_messages, continue_final_message=True, tokenize=False
        )
        for prompt in continue_prompt:
            self.assertTrue(
                prompt.endswith("It is the sound of")
            )  # no `eos` token at the end
    ...
    def test_apply_chat_template_video_frame_sampling(self):
        ...
        self.assertEqual(len(out_dict_with_video[self.videos_input_name]), 1)
        self.assertEqual(
            len(out_dict_with_video[self.videos_input_name][0]), 11
        )
        ...
        messages = [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "Which of these animals is making the sound?",
                    },
                ],
            },
            {
    ...
    def test_apply_chat_template_assistant_mask(self):
        ...
        text_is_same = assistant_text == processor.decode(
            assistant_ids, clean_up_tokenization_spaces=True
        )
        ids_is_same = (
            processor.tokenizer.encode(
                assistant_text, add_special_tokens=False
            ),
            assistant_ids.tolist(),
        )
