...
class TestAttentionInterface(unittest.TestCase):
    ...
    def assert_tensor_close(self, a, b, atol=1e-2, rtol=1e-2):
        ...
        a = a.to('float32')
        b = b.to('float32')
        self.assertTrue(paddle.allclose(a, b, rtol=rtol, atol=atol, equal_nan=True),
    ...
    def test_correctness(self):
        ...
        causal_mask = causal_mask.unsqueeze(0).unsqueeze(0).expand(
            shape=[self.batch_size, self.num_heads, -1, -1]
        )
        
        # Get the output from the naive reference implementation
        eager_output_causal = self.naive_attn_sink(self.query, self.key, self.value, self.sink, causal_mask, self.scaling)
        ...
        # Get the output from the naive reference implementation
        eager_output_flashmask = self.naive_attn_sink(self.query, self.key, self.value, self.sink, dense_mask, self.scaling)
