...
class TestQuantedModel(unittest.TestCase):
    ...
    def test_quant_model(self):
        ...
        self.assertTrue(
            paddle.allclose(out.mean(-1), EXPECTED_MEAN, atol=1e-5, rtol=1e-5)
        )
        ...
        self.assertTrue(
            paddle.allclose(
                out[0, 0, :30], EXPECTED_SLICE, atol=1e-5, rtol=1e-5
            )
        )

        self.assertTrue(
            type(self.model.model.layers[0].self_attn.qkv_proj).__name__
            == "QuantizationLinear"
        )
        ...
        self.assertTrue(
            type(lora_model.model.model.layers[0].self_attn.qkv_proj).__name__
