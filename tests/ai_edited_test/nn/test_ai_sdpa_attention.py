...
class TestSDPAAttentionForward(unittest.TestCase):
    ...
    @patch(
        "paddleformers.nn.attention.sdpa_attention.nn.functional.scaled_dot_product_attention"
    )
    def test_basic_forward_shape(self, mock_sdpa):
        """sdpa_attention_forward should produce correct output shape."""
        from paddleformers.nn.attention.sdpa_attention import (
    ...
    @patch(
        "paddleformers.nn.attention.sdpa_attention._gen_from_sparse_attn_mask_indices"
    )
    @patch(
        "paddleformers.nn.attention.sdpa_attention.nn.functional.scaled_dot_product_attention"
    )
