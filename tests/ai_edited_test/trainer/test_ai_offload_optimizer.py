...
class TestOffload(unittest.TestCase):
    ...
    def test_offload_cuda(self):
        ...
        self.assertTrue(
            x.place.is_cuda_pinned_place() or x.place.is_cpu_place()
        )
    ...
    def test_offload_cpu(self):
        ...
        with patch(
            "paddleformers.trainer.utils.offload_optimizer.paddle.is_compiled_with_cuda",
            return_value=False,
        ):
            with patch(
                "paddleformers.trainer.utils.offload_optimizer.paddle.is_compiled_with_xpu",
                return_value=False,
            ):
                offload(x)
...
class TestHackOffloadOptimizer(unittest.TestCase):
    ...
    def test_hack_offload_optimizer_patches_optimizer(self):
        ...
        original_add_accumulator = paddle.optimizer.Optimizer._add_accumulator
        try:
            hack_offload_optimizer()
            # After hacking, _add_accumulator should be replaced
            new_add_accumulator = paddle.optimizer.Optimizer._add_accumulator
            self.assertNotEqual(original_add_accumulator, new_add_accumulator)
        finally:
            # Restore original method
            paddle.optimizer.Optimizer._add_accumulator = (
                original_add_accumulator
            )
