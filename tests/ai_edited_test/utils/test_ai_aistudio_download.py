...
@unittest.skip(
    "CI has real AISTUDIO_ACCESS_TOKEN env var that mocks cannot override"
)
...
class TestGetTokenFromEnvironment(unittest.TestCase):
    ...
    def test_with_aistudio_token(self):
        with patch.dict(
            os.environ, {"AISTUDIO_TOKEN": "aistudio_token"}, clear=False
        ):
...
class TestGetTokenFromFile(unittest.TestCase):
    def test_file_not_found(self):
        with patch(
            "paddleformers.utils.download.aistudio_hub_download.AISTUDIO_TOKEN_PATH",
            "/nonexistent/path",
        ):
...
class TestGetTokenToSend(unittest.TestCase):
    ...
    def test_true_no_cached_token_raises(self):
        with (
            patch(
                "paddleformers.utils.download.aistudio_hub_download.get_token",
                return_value=None,
            ),
            self.assertRaises(LocalTokenNotFoundError),
        ):
    ...
    def test_implicit_disabled(self):
        with patch.dict(
            os.environ, {"AISTUDIO_HUB_DISABLE_IMPLICIT_TOKEN": "1"}
        ):
...
class TestBuildAistudioHeaders(unittest.TestCase):
    def test_no_token(self):
        with patch(
            "paddleformers.utils.download.aistudio_hub_download.get_token_to_send",
            return_value=None,
        ):
...
class TestAistudioHubUrl(unittest.TestCase):
    ...
    def test_custom_endpoint(self):
        url = aistudio_hub_url(
            "user/repo", "config.json", endpoint="http://custom.api.com"
        )
...
class TestAistudioHubTryToLoadFromCache(unittest.TestCase):
    def test_no_cache_dir(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            result = aistudio_hub_try_to_load_from_cache(
                "user/repo", "config.json", cache_dir=tmpdir
            )
            self.assertIsNone(result)
    ...
    def test_invalid_repo_type_raises(self):
        with self.assertRaises(ValueError):
            aistudio_hub_try_to_load_from_cache(
                "user/repo", "config.json", repo_type="dataset"
            )
