...
def test_non_packing_iterator_keeps_leftover_yield():
    sequences = [
    ...
    # packing=false leftover yield after StopIteration is the GLM-4 SFT CI GT path.
    assert [[sequence.token_ids for sequence in batch] for batch in batches] == [
        [[1, 2]],
        [[3, 4]],
        [[3, 4]],
    ]
