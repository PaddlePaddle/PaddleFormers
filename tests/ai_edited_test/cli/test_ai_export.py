...
class TestCheckDownloadRepo(unittest.TestCase):
    ...
    def test_non_local_model_with_explicit_hub(self):
        ...
        with patch(
            "paddleformers.cli.export.export.check_repo",
            return_value="/cached/model",
        ) as mock_check:
            result = check_download_repo(
                "some/repo", download_hub="huggingface"
            )
    ...
    def test_non_local_model_with_env_download_source(self):
        ...
        with (
            ...
            patch.dict(os.environ, {"DOWNLOAD_SOURCE": "modelscope"}),
        ):
...
class TestRunExport(unittest.TestCase):
    ...
    def test_run_export_raises_when_no_checkpoint(self):
        ...
        with (
            patch(
                "paddleformers.cli.export.export.read_args", return_value=args
            ) as _,
            patch(
                "paddleformers.cli.export.export.get_export_args"
            ) as mock_get_export,
            patch("paddleformers.cli.export.export.paddle.set_device"),
            patch(
                "paddleformers.cli.export.export.os.path.isdir",
                return_value=False,
            ),
            patch(
                "paddleformers.cli.export.export.os.path.isfile",
                return_value=False,
            ),
        ):
            mock_model_args = MagicMock(lora=True)
            mock_finetuning_args = MagicMock(
                output_dir=non_model_dir, device="gpu"
            )
    ...
    def test_run_export_raises_when_lora_false(self):
        ...
        with (
            ...
            patch(
                "paddleformers.cli.export.export.is_valid_model_dir",
                return_value=True,
            ),
        ):
            mock_model_args = MagicMock(lora=False)
            mock_finetuning_args = MagicMock(
                output_dir="/tmp/model", device="gpu"
            )
            mock_get_export.return_value = (
                mock_model_args,
                MagicMock(),
            ...
            with self.assertRaises(ValueError) as ctx:
                run_export(args)
            self.assertIn(
                "Only support merge lora checkpoint", str(ctx.exception)
            )
    ...
    def test_run_export_lora_merge_success(self):
        ...
        with (
            ...
            patch(
                "paddleformers.cli.export.export.check_download_repo",
                return_value="/tmp/base",
            ),
            patch(
                "paddleformers.cli.export.export.resolve_file_path",
                return_value="/tmp/base/model.safetensors",
            ),
            patch(
                "paddleformers.cli.export.export.MergeConfig",
                return_value=mock_merge_config,
            ) as mock_merge_config_cls,
            patch(
                "paddleformers.cli.export.export.MergeModel",
                return_value=mock_mergekit,
            ) as mock_merge_model_cls,
            patch("paddleformers.cli.export.export.logger"),
        ):
