...
def _build_import_path(subdomains: list, imp):
    import_path = ["paddleformers.fleet"]
...
def _get_class_from_path(subdomains, imp):
    ...
    try:
        ...
        if inspect.isclass(class_):
            ...
            if issubclass(class_, (FleetLayer, paddle.nn.Layer)):
                result = class_
        else:
            class_ = None
        error = None
...
def _test_domain_module_imports(module, subdomains: list):
    ...
    for module in failed_list:
        print(
            "Module did not match a valid signature of paddleformers.fleet Model (hence ignored):",
            module,
        )
