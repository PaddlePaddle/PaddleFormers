...
def apply_mla_rope_for_q_ref(q, cos, sin, qk_head_dim, emb_dim, cu_seqlens_q=None):
    ...
    if cu_seqlens_q is None:
        ...
        cos_row = paddle.index_select(cos_f32, positions, axis=0)  # [T, emb_dim]
...
def apply_mla_rope_for_kv_ref(kv, k_pos_emb, cos, sin, emb_dim, k_dim, v_dim, cu_seqlens_kv=None):
    ...
    if cu_seqlens_kv is None:
        ...
        cos_row = cos_f32[:seq_len].reshape([1, seq_len, 1, emb_dim])
        sin_row = sin_f32[:seq_len].reshape([1, seq_len, 1, emb_dim])
        rope_emb = _apply_rope_emb_ref(emb_f32, cos_row, sin_row)  # [bs, seq, 1, emb_dim]
        ...
        cos_row = paddle.index_select(cos_f32, positions, axis=0).reshape([-1, 1, emb_dim])
        sin_row = paddle.index_select(sin_f32, positions, axis=0).reshape([-1, 1, emb_dim])
        rope_emb = _apply_rope_emb_ref(emb_f32, cos_row, sin_row)  # [T, 1, emb_dim]
        heads = kv.shape[1]
...
def _rand(shape, dtype, seed):
    ...
    return paddle.to_tensor(rng.randn(*shape).astype("float32"), place="gpu").astype(dtype)
...
class TestApplyMLARopeForQ(_BaseMLARopeTest):
    def _run_bshd_fwd(self, bs, seq, heads, qk_head_dim, emb_dim, dtype):
        ...
        out = fused_apply_mla_rope_for_q(q_in, cos, sin, qk_head_dim, emb_dim, None, 0, 1, False)
    ...
    def _run_thd_fwd(self, seq_lens, heads, qk_head_dim, emb_dim, dtype):
        ...
        cu = paddle.to_tensor(np.cumsum([0, *list(seq_lens)]).astype("int32"), place="gpu")
        ...
        out = fused_apply_mla_rope_for_q(q_in, cos, sin, qk_head_dim, emb_dim, cu, 0, 1, False)
    ...
    def _run_bshd_bwd(self, bs, seq, heads, qk_head_dim, emb_dim, dtype):
        ...
        out_ref = apply_mla_rope_for_q_ref(q_ref, cos, sin, qk_head_dim, emb_dim, None)
        ...
        out_cu = fused_apply_mla_rope_for_q(q_cu, cos, sin, qk_head_dim, emb_dim, None, 0, 1, False)
...
class TestApplyMLARopeForKV(_BaseMLARopeTest):
    def _run_bshd_fwd(self, bs, seq, heads, k_dim, v_dim, emb_dim, dtype):
        ...
        # Reference: apply directly in bshd layout (no transpose to avoid
        # layout-induced numeric differences).
        key_ref, val_ref = apply_mla_rope_for_kv_ref(kv, k_pos_emb, cos, sin, emb_dim, k_dim, v_dim, None)
    ...
    def _run_thd_fwd(self, seq_lens, heads, k_dim, v_dim, emb_dim, dtype):
        ...
        key_out, val_out = fused_apply_mla_rope_for_kv(kv, k_pos_emb, cos, sin, emb_dim, k_dim, v_dim, cu, 0, 1, False)
        key_ref, val_ref = apply_mla_rope_for_kv_ref(kv, k_pos_emb, cos, sin, emb_dim, k_dim, v_dim, cu)
    ...
    def _run_bshd_bwd(self, bs, seq, heads, k_dim, v_dim, emb_dim, dtype):
        ...
        dkey_np = np.random.randn(bs, seq, heads, k_dim + emb_dim).astype("float32")
        ...
        key_ref, val_ref = apply_mla_rope_for_kv_ref(kv_ref, emb_ref, cos, sin, emb_dim, k_dim, v_dim, None)
        ...
        gkv_ref, gemb_ref = paddle.grad([key_ref, val_ref], [kv_ref, emb_ref], [dkey_ref, dval_ref])
        ...
        key_cu, val_cu = fused_apply_mla_rope_for_kv(kv_cu, emb_cu, cos, sin, emb_dim, k_dim, v_dim, None, 0, 1, False)
        ...
        gkv_cu, gemb_cu = paddle.grad([key_cu, val_cu], [kv_cu, emb_cu], [dkey_cu, dval_cu])
