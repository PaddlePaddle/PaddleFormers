...
def dequant_ref(fp8_tensor: paddle.Tensor, scale: paddle.Tensor, block_size: int = 128) -> paddle.Tensor:
...
def simulate_fused_transpose_split_quant(tokens_per_expert, seq_len, pow_2_scales, using_fp8=False):
    x = paddle.randn([sum(tokens_per_expert), seq_len], dtype="bfloat16")
    if using_fp8:
        x = x.cast("float8_e4m3fn")
    xscale = paddle.randn([sum(tokens_per_expert), (seq_len + 127) // 128], dtype="float32") if using_fp8 else None
    ...
    fuse_transpose_split_fp8_quant(x, xscale, out, scale, tokens_per_expert, pow_2_scales, use_ue8m0=False)

    out_ref, scale_ref = fused_transpose_split_quant_ref(x, xscale, tokens_per_expert, pow_2_scales)
...
class TestFusedTransposeSplitQuant(unittest.TestCase):
    def test_all(self):
        ...
        for using_fp8 in fp8_choice:
            simulate_fused_transpose_split_quant([0, 0], 1024, False, using_fp8=using_fp8)
            simulate_fused_transpose_split_quant([128, 2 * 128], 0, True, using_fp8=using_fp8)
            simulate_fused_transpose_split_quant([128], 1, False, using_fp8=using_fp8)
            simulate_fused_transpose_split_quant([0, 128, 0, 2 * 128], 127, True, using_fp8=using_fp8)
            simulate_fused_transpose_split_quant([3 * 128, 4 * 128, 5 * 128], 233, False, using_fp8=using_fp8)
