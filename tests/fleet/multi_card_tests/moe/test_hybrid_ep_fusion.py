...
class TestHybridEPFusion(unittest.TestCase):
    ...
    def _build_moe_layer(self, config):
        transformer_layer_spec = get_gpt_layer_local_spec(config, num_experts=config.n_routed_experts)
        return MoELayer(
            config,
    ...
    def test_hybrid_ep_moe_fusion_and_overlap_segments(self):
        ...
        input_data = paddle.randn(4, 64, config.hidden_size, dtype=paddle.bfloat16)
        ...
        dispatch_args = moe_layer.dispatch_preprocess((input_data, topk_weights, topk_indices))
        dispatched_args = moe_layer.compute_dispatch(dispatch_args, async_finish=True)
        expert_output = moe_layer.compute_experts(dispatched_args, is_first_fwd=True)
        overlapped_output = moe_layer.compute_combine(expert_output, async_finish=True)
        self.assertEqual(
    ...
    def test_hybrid_ep_dispatcher_is_allowed_for_overlap_layer(self):
        ...
        transformer_layer = build_spec_layer(transformer_layer_spec, pg_collection=self.pg_collection)

        self.assertEqual(transformer_layer.mlp.moe_token_dispatcher_type, "hybridep")
