...
@unittest.skipUnless(
    _check_nccl(), "NCCL not available (CUDA driver version mismatch)"
)
class TestMoEPipelineParallel(unittest.TestCase):
    """Test MoE with pipeline parallel configuration."""
    ...
    def test_moe_pp_basic(self):
        """Test MoE model with pipeline parallel."""
        micro_batch_size = 1
        num_acc = BATCH_SIZE // micro_batch_size
        data = paddle.randint(
