...
from paddleformers.transformers.linear_utils import ColumnParallelLinear, RowParallelLinear
...
class ParallelTransformerLayer(nn.Layer):
    ...
    def __init__(self, hidden_size):
        ...
        self.ffn_proj1 = ColumnParallelLinear(
            hidden_size, hidden_size * 4,
            gather_output=False,
            has_bias=True
        )
        self.ffn_proj2 = RowParallelLinear(
...
class ParallelModel(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.LayerList([
            ParallelTransformerLayer(config.hidden_size) 
        ...
        self.output_layer = ColumnParallelLinear(
            ...
            gather_output=True,  # Gather output for final result
            has_bias=True
        )
...
class TestHFFormatSaverTP4Sharding2(TestMultipleGpus):
    ...
    def test_save_checkpoint_tp4_sharding2(self):
        ...
        if rank < saver.num_saver_ranks:  # Only save ranks should create files
            ...
            self.assertTrue(len(safetensor_files) > 0, 
                          f"Rank {rank}: No safetensors files created")
            ...
            for file in safetensor_files:
                ...
                self.assertTrue(os.path.isfile(file_path), 
                              f"Rank {rank}: File {file} should be a regular file")
                file_size = os.path.getsize(file_path)
                self.assertTrue(file_size > 0, 
                              f"Rank {rank}: File {file} should have positive size")
        ...
        if rank == 0:
            ...
            for ext in expected_extensions:
                ...
                self.assertTrue(len(matching_files) > 0, 
                              f"Rank {rank}: No {ext} files found")
