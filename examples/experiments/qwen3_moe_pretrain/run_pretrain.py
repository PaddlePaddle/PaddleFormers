...
from paddleformers.transformers.qwen3_moe import (
    Qwen3MoeConfig,
    Qwen3MoeForCausalLM,
...
# from paddleformers.trainer.utils.load_hf_ckpt import load_huggingface_ckpt
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_args.tokenizer_name_or_path, download_hub="modelscope")
    config = Qwen3MoeConfig.from_pretrained(model_args.model_name_or_path)
    ...
    if training_args.pipeline_parallel_degree > 1:
        model_class = Qwen3MoeForCausalLMPipe
    ...
    architectures_to_check = {"Qwen3Moe", "DeepseekV2", "DeepseekV3"}
    if (
    ...
    def resume_from_custom_func(model):
        if training_args.resume_from_huggingface_ckpt:
            ...
            raise NotImplementedError("Not support resume from huggingface ckpt yet")
        else:
