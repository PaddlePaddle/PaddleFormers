...
from token_dispatcher import (
    MoEFlexTokenDispatcherFast as MoEFlexTokenDispatcher,
    PreDispatchNode,
)
...
class MoELayer(nn.Layer):
    def __init__(
        ...
        self.token_drop_steps = (
            config.token_drop_steps
            if hasattr(config, "token_drop_steps")
            else None
        )
    ...
    def _parse_moe_expert_parallel(
        self, n_routed_experts, expert_model_parallel_size
    ):
        assert n_routed_experts >= expert_model_parallel_size, (
            f"expert n_routed_experts={n_routed_experts} >= moe_world_size={expert_model_parallel_size}"
        )
        assert n_routed_experts % expert_model_parallel_size == 0, (
    ...
    def forward(
        ...
        if self.using_flex_token:
            return self.forward_flex_token(
                hidden_states, probs, routing_map, l_aux, l_zloss
            )
        else:
    ...
    def forward_drop_token(
        ...
        if self.expert_model_parallel_size > 1:
            ...
            tokens_per_expert_post_gather = tokens_per_expert_group.reshape(
                [
                    self.expert_model_parallel_size,
                    self.n_routed_experts_per_device,
                ]
            ).sum(axis=0)
        ...
        for i, num_tokens in enumerate(tokens_per_expert):
            ...
            if num_tokens == 0:
                continue
            expert = self.experts[
                i + self.moe_rank * self.n_routed_experts_per_device
            ]
    ...
    def expert_forward(self, dispatched_input):
        ...
        chunks = paddle.split(
            dispatched_input,
            num_or_sections=self.get_tokens_per_expert(),
            axis=0,
        )
    ...
    def forward_flex_token(
        ...
        if (
            hasattr(self.config, "dsv3_use_fp8_gemm")
            and self.config.dsv3_use_fp8_gemm
        ):
            if (
                hasattr(self.config, "dsv3_use_fp8_dispatch")
                ...
                hidden_states, token_indices, token_probs = (
                    self.token_dispatcher.pre_dispatch(
    ...
    def post_dispatch_compute(
        self, hidden_states, dispatched_indices, dispatched_probs
    ):
        (global_input_tokens, token_permuted_indices, prob_permuted_indices) = (
            self.token_dispatcher.post_dispatch(
...
class MoEFlexTokenLayer(nn.Layer):
    def __init__(
        ...
        self.experts = nn.LayerList(
            [expert_class(**expert_kwargs)] * self.num_local_experts
        )
        self.router = gate
...
class Fp8DispatchQuantNode:
    def __init__(
        self,
        token_dispatcher,
        dsv3_use_fp8_dispatch,
        name="fp8_dispatch_quant_node",
    ):
    ...
    def forward(self, hidden_states, probs, routing_map):
        ...
        if self.dsv3_use_fp8_dispatch:
            ...
            # pre_dispatch
            token_indices, token_probs = self.pre_dispatch_node.forward(
                routing_map, probs
            )
...
class Fp8DispatchNode:
    ...
    def forward(
        ...
        # dispatch
        hs_2d_dispatched, dispatched_probs, states = (
            self.dispatch_act_node.forward(
                hs_2d,
                token_indices,
                token_probs,
        ...
        self.token_dispatcher._comm_manager.tokens_per_expert = states[
            "tokens_per_expert"
        ]
    ...
    def backward(self, output_grad, event_to_wait=None):
        ...
        if self.dsv3_use_fp8_dispatch:
            if event_to_wait is not None:
                ...
                buffer = get_buffer(
                    self.token_dispatcher._comm_manager.group,
                    get_hidden_bytes(output_grad),
                )
                custom_stream = paddle.device.Stream(
                    stream_base=buffer.runtime.get_comm_stream()
                )
            else:
                custom_stream = paddle.device.current_stream()
            with paddle.device.stream_guard(custom_stream):
                output_combine_grad = paddle.reshape(
                    output_grad, [-1, output_grad.shape[-1]]
                )
                ...
                quant_event = None
                if event_to_wait is not None:
                    quant_event = deep_ep.get_event_from_custom_stream(
                        custom_stream.stream_base
                    )
            return (
...
class FusionMlpNode:
    ...
    def __init__(
        ...
        self.adaptive_remained_O1_recompute_ratio = (
            custom_map.config.adaptive_remained_O1_recompute_ratio
        )
    ...
    def prepare_env_subbatch(
        self, unzipped_tokens=None, unzipped_tokens_scale=None, is_fwd=True
    ):
        if is_fwd:
            assert (
                unzipped_tokens is not None
                and unzipped_tokens_scale is not None
            )
            self.experts_group_gemm_node.input_fp8 = unzipped_tokens
            self.experts_group_gemm_node.input_scale = unzipped_tokens_scale
            self.m_indices = self.experts_group_gemm_node.gen_m_indices(
                self.padding_token_per_experts
            )
            self.experts_group_gemm_node.fwd_subbatch = True
        else:
            self.m_indices = (
                self.experts_group_gemm_node.gen_m_indices(
                    self.padding_token_per_experts
                )
    ...
    def gemm_forward_subbatch(
        ...
        expert_out = self.experts_group_gemm_node.forward(
            (
                ...
                unzipped_tokens_scale[start_idx:end_idx],
            ),
    ...
    def gemm_backward_subbatch(
        ...
        unzipped_inp_grad = (
            (
                unzipped_grad[0][start_idx:end_idx].contiguous(),
                unzipped_grad[1][start_idx:end_idx].contiguous(),
            )
        ...
        unzipped_grad, unzipped_probs_grad = (
            self.experts_group_gemm_node.backward(
                ...
                input_fp8_slice=self.experts_group_gemm_node.input_fp8[
                    start_idx:end_idx
                ].contiguous(),
                input_scale_slice=self.experts_group_gemm_node.input_scale[
                    start_idx:end_idx
                ].contiguous(),
                tokens_per_expert=split_list_prefix(
                    padding_token_per_experts, start_idx, end_idx
                ),
    ...
    def forward(self, hs_2d_dispatched, dispatched_indices, dispatched_probs):
        ...
        if self.dsv3_use_fp8_dispatch:
            total_unzipped_tokens = extract_first_if_tuple(
                unzipped_tokens
            ).shape[0]
            ...
            if (
                self.mlp_fwd_subbatch_rows != 0
                and total_unzipped_tokens > self.mlp_fwd_subbatch_rows * 2
            ):
                assert self.experts_group_gemm_node.recompute_fwd_gate_up, (
                    "recompute_fwd_gate_up must be true when use_mlp_subbatch = True"
                )
                map_unzipped_indices_to_zipped = (
                    paddlefleet.extensions.ops.tokens_unzip_slice(
                ...
                subbatch_rows = min(
                    (total_unzipped_tokens // num_experts) // 128 * 128,
                    self.mlp_fwd_subbatch_rows,
                )
                nparts = (
                    total_unzipped_tokens + subbatch_rows - 1
                ) // subbatch_rows
                output = paddle.empty(
                    [0, extract_first_if_tuple(hs_2d_dispatched).shape[-1]],
                    dtype=paddle.float32,
                )
                self.prepare_env_subbatch(
                    unzipped_tokens, unzipped_tokens_scale, True
                )
                logger.info(
                ...
                for i in range(nparts):
                    ...
                    end_idx = min(
                        start_idx + subbatch_rows, total_unzipped_tokens
                    )
    ...
    def backward(self, hidden_states_out_grad):
        ...
        total_zipped_tokens = extract_first_if_tuple(
            hidden_states_out_grad
        ).shape[0]
        total_unzipped_tokens = extract_first_if_tuple(unzipped_grad).shape[0]
        hidden_states_size = extract_first_if_tuple(
            hidden_states_out_grad
        ).shape[-1]
        num_experts = len(self.tokens_per_expert)
        padding_token_per_experts = [
        ...
        if (
            self.mlp_bwd_subbatch_rows != 0
            and total_unzipped_tokens > self.mlp_bwd_subbatch_rows * 2
        ):
            map_unzipped_indices_to_zipped = (
                paddlefleet.extensions.ops.tokens_unzip_slice(
            ...
            for i in range(nparts):
                reset_status = (
                    True if i == nparts - 1 else False
                )  # release saved status in the last part.
                start_idx = i * subbatch_rows
            ...
            dispatched_probs_grad = (
                paddlefleet.extensions.ops.tokens_zip_prob_seq_subbatch(
                    probs_grad_list,
                    self.unzip_node.zipped_expertwise_rowmap,
                    self.dispatched_indices,
                    subbatch_rows,
...
class FusionMoeNode:
    def __init__(
        ...
        self.dsv3_use_fp8_dispatch = dsv3_use_fp8_dispatch
        self.dispatch_quant_node = Fp8DispatchQuantNode(
            self.token_dispatcher, dsv3_use_fp8_dispatch
        )
    ...
    def forward(self, hidden_states, probs, routing_map):
        if self.dsv3_use_fp8_dispatch:
            (hs_fp8, hs_scale), token_indices, token_probs = (
                self.dispatch_quant_node.forward(
                    hidden_states, probs, routing_map
                )
            )
            (
            ...
            hs_2d_dispatched, dispatched_indices, dispatched_probs = (
                self.dispatch_node.forward(hidden_states, probs, routing_map)
            )
            hidden_states_out = self.mlp_node.forward(
                hs_2d_dispatched, dispatched_indices, dispatched_probs
            )
    ...
    def backward(self, output_grad):
        ...
        hs_dispatched_grad, dispatched_probs_grad = self.mlp_node.backward(
            hidden_states_out_grad
        )

        if self.dsv3_use_fp8_dispatch:
            hs_fp8_grad, token_probs_grad = self.dispatch_node.backward(
                hs_dispatched_grad, dispatched_probs_grad
            )
            hs_grad, probs_grad, routing_map_grad = (
                self.dispatch_quant_node.backward(hs_fp8_grad, token_probs_grad)
            )
            return hs_grad, probs_grad, routing_map_grad
        else:
            hs_bf16_grad, token_probs_grad = self.dispatch_node.backward(
                hs_dispatched_grad, dispatched_probs_grad
            )
