...
from paddlefleet.utils import WrappedTensor, deprecate_inference_params, make_viewless_tensor
...
class Qwen25VLVisionTransformerBlock(TransformerBlock):
    ...
    def __init__(
        ...
        spec: Union[TransformerBlockSublayersSpec, LayerSpec],
        ...
        assert not self.config.fp8,"qwen_25-vl does not support fp8"
...
class Qwen2VisionModel(VisionLayer):
    ...
    def __init__(
        ...
        if self.add_class_token:
            self.class_token = paddle.nn.Parameter(paddle.randn(1, self.class_token_len, self.visual_hidden_size))
    ...
    def rot_pos_emb(self, grid_thw):
        ...
        for t, h, w in grid_thw:
            hpos_ids = paddle.arange(h).unsqueeze(1).expand([-1, w]).contiguous()
            ...
            wpos_ids = paddle.arange(w).unsqueeze(0).expand([h, -1]).contiguous()
            ...
            pos_ids.append(paddle.stack([hpos_ids, wpos_ids], dim=-1).repeat(t, 1).contiguous())
        pos_ids = paddle.concat(pos_ids, dim=0)
    ...
    def forward(
        self, x: paddle.Tensor, grid_thw: paddle.Tensor, attention_mask: Optional[paddle.Tensor] = None
    ) -> paddle.Tensor:
        ...
        Args:
            ...
            grid_thw (paddle.Tensor): The temporal, height and width of feature shape of each image/frame.
            attention_mask (paddle.Tensor with dtype=bool): Attention mask to use.

        Returns:
            x (paddle.Tensor): output after final transformer block.
        """
...
class Qwen25VisionModel(VisionLayer):
    ...
    def rot_pos_emb(self, grid_thw):
        ...
        for t, h, w in grid_thw:
            ...
            wpos_ids = wpos_ids.permute(0, 2, 1, 3)
            wpos_ids = wpos_ids.flatten()
            pos_ids.append(paddle.stack([hpos_ids, wpos_ids], dim=-1).repeat([t, 1]))
        pos_ids = paddle.concat(pos_ids, dim=0)
        max_grid_size = grid_thw[:, 1:].max()
        rotary_pos_emb_full = self.rotary_pos_emb(max_grid_size)
        rotary_pos_emb = rotary_pos_emb_full[pos_ids].flatten(1)
    ...
    def get_packed_seq_params(
        self,
        grid_thw: Optional[paddle.Tensor],
    ...
    def get_window_index(self, grid_thw):
        ...
        for grid_t, grid_h, grid_w in grid_thw:
            ...
            index = paddle.arange(grid_t * llm_grid_h * llm_grid_w).reshape([grid_t, llm_grid_h, llm_grid_w])
        ...
        window_index = paddle.concat(window_index, dim=0)
    ...
    def forward(
        ...
        cu_window_seqlens = paddle.tensor(
            cu_window_seqlens,
            dtype=paddle.int32,
        )
