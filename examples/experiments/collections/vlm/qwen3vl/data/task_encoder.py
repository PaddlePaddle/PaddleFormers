...
from megatron.energon import Batch
...
from nemo.collections.vlm.qwen2vl.data.multimodal_tokens import HF_IMAGE_TOKEN_INDEX, HF_VIDEO_TOKEN_INDEX, IGNORE_INDEX, IMAGE_TOKEN_INDEX, PAD_TOKEN_INDEX, VIDEO_TOKEN_INDEX
...
@dataclass
class Qwen3VLTaskBatch(Batch):
    """Encoded Batch Format For Qwen3VL"""
...
def convert_to_qwen3vl_content(user_input: str, image_pattern: str = '<image>', video_pattern: str = '<video>'):
...
class Qwen2VLTaskEncoder(DefaultTaskEncoder[ChatMLSample, Qwen3VLTaskSample, Qwen3VLTaskBatch, dict]):
    ...
    def encode_sample(self, sample: ChatMLSample):
        ...
        if _from_system_:  # ['conversations':[{'from':'human', 'value':[]}, {'from':'gpt', 'value':[]}]
            ...
            for turn_idx, turn in enumerate(conversation):
                ...
                if role == 'human':
                    role = 'user'
                    content = convert_to_qwen3vl_content(content)
                elif role == 'gpt':
                    role = 'assistant'
    ...
    def batch(self, samples: list[Qwen3VLTaskSample]) -> Qwen3VLTaskBatch:
        """
    ...
    def encode_batch(self, batch: Qwen3VLTaskBatch) -> dict:
