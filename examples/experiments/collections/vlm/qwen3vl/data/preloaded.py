...
def process_vision(processor, images, videos, fps=None, model_version="qwen3_vl"):
    assert isinstance(processor, Qwen2VLImageProcessor), "proceesor must bi a Qwen2VLImageProcessor"
...
class Qwen3VLDataset(PreloadedSupervisedDataset):
    """Dataset for supervised fine-tuning."""
...
class Qwen3VLPreloadedDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        data_config: Qwen2VLDataConfig = Qwen2VLDataConfig,
        seq_length: int = 2048,
    ...
    def setup(self, stage: str = "") -> None:
        ...
        if self.use_packed_sequence:
            ...
            self._validation_ds = Qwen3VLDataset(
                self.paths[0],
                self.data_config,
                self.tokenizer,
                self.image_processor,
