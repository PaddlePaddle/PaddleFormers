...
class TrainingArguments:
    ...
    def __post_init__(self):
        ...
        if self.use_hybrid_parallel:
            ...
            if not paddle.distributed.parallel.parallel_helper._is_parallel_ctx_initialized():
                ...
                if is_context_parallel_supported():
                    hybrid_configs = {
                        ...
                        "cp_degree": self.expert_model_parallel_size,
                        "order": order,
                    }
            ...
            self.expert_model_parallel_size = max(self.expert_model_parallel_size, 1)
        ...
        if self.hybrid_parallel_expert_grad_scale is None:
            ...
            self.hybrid_parallel_expert_grad_scale = (
                tensor_model_parallel_size * expert_model_parallel_size / expert_model_parallel_size
            )
    ...
    def _post_init_parallel_degree(self):
        ...
        if world_size > 1:
            ...
            assert not (
                sep_parallel_degree > 1 and expert_model_parallel_size > 1
            ), f"sep parallel and context parallel cannot be used together, sep_parallel_degree:{sep_parallel_degree}, expert_model_parallel_size:{expert_model_parallel_size}."
    ...
    def cp_sharding_degree(self):
        """cp_sharding_degree"""
        assert self.sharding_parallel_degree % self.expert_model_parallel_size == 0, (
            f"sharding parallel degree {self.sharding_parallel_degree} "
            f"is not divisible by context parallel degree {self.expert_model_parallel_size}"
        )
        hcg = None
        ...
        if hasattr(hcg, "get_context_parallel_world_size"):
            ...
            assert self.expert_model_parallel_size == 1, (
                "expert_model_parallel_size > 1 requires 'get_context_parallel_world_size' in hcg. "
    ...
    def cp_sharding_rank(self):
        ...
        if self.use_hybrid_parallel:
            hcg = fleet.get_hybrid_communicate_group()
            if hasattr(hcg, "get_context_parallel_world_size") and self.expert_model_parallel_size > 1:
    ...
    def dataset_rank(self):
        if self.use_hybrid_parallel:
            sharding_parallel_degree = (
                self.cp_sharding_degree if self.expert_model_parallel_size > 1 else self.sharding_parallel_degree
            )
            sharding_parallel_rank = (
                self.cp_sharding_rank if self.expert_model_parallel_size > 1 else self.sharding_parallel_rank
            )
