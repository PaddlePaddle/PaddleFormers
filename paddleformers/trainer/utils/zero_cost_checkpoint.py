...
def sharded_state_dict_compatibility(func, *, return_sharded_state_dict=False):
    ...
    def wrapper(*args, **kwargs):
        def should_convert(maybe_sharded_state_dict):
            all_shared_weights = all(
                isinstance(value, ShardedWeight)
                for value in maybe_sharded_state_dict.values()
            )
            any_shared_weights = any(
                isinstance(value, ShardedWeight)
                for value in maybe_sharded_state_dict.values()
            )
            logger.debug(
                f"all sharded weight {all_shared_weights}, any shared weight {any_shared_weights}"
            )
            if not any_shared_weights:
                logger.debug(
                    "this is not a sharded state dict, no need to convert."
                )
                return False

            if any_shared_weights and (not all_shared_weights):
                logger.debug(
                    "this is a mixed state dict(normal and sharded), not support to convert."
                )
                return False
            logger.debug(
                "this is a sharded state dict, will convert it to local tensor dict."
            )
            return True
        ...
        if return_sharded_state_dict:
            assert isinstance(result, dict), (
                f"expected dict, but got {type(result)}"
            )
...
def get_fused_param_mappings(optimizer, manipulated_state_dict):
    ...
    for buffer in sharding_comm_buffers:
        ...
        for k, v in manipulated_state_dict.items():
            if v.name in buffer._sharding_param_grad_view:
                assert k not in param_mappings, (
                    f"{k} has already been mapped, which is unexpected."
                )
                param_meta = {}
                param_meta["buffer_index"] = str(index)
                param_meta["shape"] = v.shape
                param_meta["name"] = v.name
                param_meta["start"] = buffer._sharding_param_grad_view[
                    v.name
                ]._index
    ...
    for k, v in manipulated_state_dict.items():
        if k not in param_mappings:
            ...
            param_meta["name"] = v.name
            param_mappings[k] = param_meta
            ipc_meta_mappings[unshard_buffer_index] = (
                v.get_tensor()._share_cuda()
            )
    ...
    assert len(manipulated_state_dict) == len(param_mappings), (
...
class ZeroCostCheckpointEMAProcessor:
    ...
    def build_ema_buffer(self):
        ...
        master_max_offset = max(
            self.optimizer_fusion_storage_helper.master_weights_meta.values(),
            key=lambda i: i["end"],
        )["end"]
        ...
        with device_guard("cpu"):
            ...
            ema_buffer_model_params = {
                ...
                for k, (
                    cuda_buf,
                    cpu_buf,
                ) in self.param_fusion_storage_helper.inited_buffers.items()
                if cuda_buf.dtype == paddle.float32
            }
        logger.info(
            f"[ZCCworker] build buffer done:{ema_buffer.dtype} {ema_buffer.place}"
        )
        return (
            ema_buffer,
            ema_buffer_model_params,
    ...
    def ema_accumulate(self, global_step, loss, zcc_ema_loss_threshold):
        ...
        logger.info(
            f"[ZCC EMA] accumulating, buffer type:{self.ema_buffer.place} {self.ema_buffer.dtype}"
        )
        with device_guard("cpu"):
            cpu_master_weights = (
                self.optimizer_fusion_storage_helper.cpu_buffer._slice(
                    self.master_min_offset, self.master_max_offset
                ).cpu()
            )
            if zcc_ema_loss_threshold is None or loss < zcc_ema_loss_threshold:
                self.ema_buffer = (
                    self.ema_coef * self.ema_buffer
                    + (1 - self.ema_coef) * cpu_master_weights
                )
    ...
    def ema_state_dict(self):
        ...
        with device_guard("cpu"):
            ...
            for (
                k,
                tensor_meta,
            ) in self.param_fusion_storage_helper.model_weights_metas.items():
                ...
                tensor = cpu_buffer._slice(
                    start, end
                ).clone()  # slice 出来的 tensor 在执行`paddle.save`会异常慢，此处必须clone
                tensor.get_tensor()._set_dims(shape)
                tensor.name = name
                ema_state_dict[k] = tensor
            ...
            for (
                ...
                self.optimizer_fusion_storage_helper.master_weights_meta.items()
            ):
    ...
    def load_ema_state_dict(self, state_dict):
        for (
            ...
            if k in state_dict:
                cpu_buffer = self.ema_buffer_model_params[
        ...
        for (
            k,
            meta,
        ) in self.optimizer_fusion_storage_helper.master_weights_meta.items():
...
class ParamFusionStorageHelper:
    ...
    def init_buffer(self, meta):
        if paddle.is_compiled_with_xpu():
            cuda_buffer = paddle.to_tensor(
                paddle.base.core.LoDTensor._new_shared_xpu(meta)
            )
        else:
            cuda_buffer = paddle.to_tensor(
                paddle.base.core.LoDTensor._new_shared_cuda(meta)
            )
    ...
    def sync_unshard_buffers(self):
        ...
        for tensor_meta in self.model_weights_metas.values():
            buffer_index = tensor_meta["buffer_index"]
            if (
                not buffer_index.startswith("unshard_")
                or buffer_index in synced_buffer_indices
            ):
                continue
    ...
    def sync_partial_param(self, numel_to_sync):
        ...
        for i, k in enumerate(meta_keys_in_order):
            if (
                self.current_offloaded_numel
                >= self.model_weights_metas[k]["logical_end"]
            ):
                continue
        ...
        while numel_to_sync > 0:
            ...
            offloading_param_meta = self.model_weights_metas[
                offloading_param_key
            ]
            logical_offload_param_start = self.current_offloaded_numel
            logical_offload_param_end = min(
                offloading_param_meta["logical_end"],
                logical_offload_param_start + numel_to_sync,
            )
            actual_offload_start = (
                logical_offload_param_start
                - offloading_param_meta["logical_start"]
            ...
            current_param_buffer = self.inited_buffers[
                offloading_param_meta["buffer_index"]
            ][0]
            current_param_cpu_buffer = self.inited_buffers[
                offloading_param_meta["buffer_index"]
            ][1]
...
class ZeroCostCheckpointCallback(TrainerCallback):
    ...
    * (when offload done, dump model)
    ...
    def on_optimizer_begin(self, args, state, control, **kwargs):
        if (
            args.enable_zero_cost_checkpoint
            and self.manager.current_worker is not None
        ):
            logger.info("[ZCC manager] Start syncing checkpoints")
            assert self.manager.global_step != 0, (
                "global_step should set, when calling `on_optimizer_begin`"
            )
            self.manager.sync_offload_status()
            logger.info("[ZCC manager] Synced checkpoints.")

    def on_step_end(
        self, args, state, control, model, lr_scheduler, optimizer, **kwargs
    ):
        if not control.should_save:
            if (
                args.zcc_save_ema_coef is not None
                and state.global_step % self.zcc_ema_interval == 0
            ):
                self.maybe_update_zcc_worker(
                    args, model, optimizer, state.global_step
                )
                self.manager.get_idle_worker_for_saving(
            ...
            save_infos = self._get_save_infos_based_on_steps(
                state, args, checkpoint_folder
            )
            non_cached_objects = (
                lr_scheduler.state_dict(),
                state,
                self.get_rng_states(args),
            )
            self.manager.get_idle_worker_for_saving(
    ...
    def _pack_dynamic_objects(self):
        ...
        dynamic_objecs["optimizer_states_name_path"] = (
            self.optimizer_states_name_path
        )
    ...
    def maybe_update_zcc_worker(self, args, model, optimizer, global_step):
        ...
        param_mappings, ipc_meta_mappings = get_fused_param_mappings(
            optimizer, self.manipulated_state_dict
        )
        ...
        self.optimizer_states_name_path = _add_variant(
            PADDLE_OPTIMIZER_NAME, args.optimizer_name_suffix
        )
        self.model_states_name_path = _add_variant(
            PADDLE_WEIGHTS_NAME, self.manipulated_weight_suffix
        )

        dynamic_objects = self._pack_dynamic_objects()
        static_objects = self._pack_static_objects(args)

        self.manager.update_zcc_workers(
            inner_opt.fused_buffer_version,
            dynamic_objects,
            static_objects,
            global_step,
        )
        logger.info(
            f"[ZCC Callback] after first update:{inner_opt.fused_states_buffer_ipc_meta}"
        )
    ...
    def _muon_manipulate_state_dict(self, model, optimizer):
        ...
        all_1d_names = {p.name for p in optimizer._params_1d}
        ...
        if inner_opt._multi_precision:
            ...
            if sharding_group.nranks > 1:
                ...
                paddle.distributed.all_gather_object(
                    tmp, list(master_weight_names), group=sharding_group
                )
                master_weight_names = {name for item in tmp for name in item}
    ...
    def _cache_meta_for_sharded_save(self, model, optimizer):
        ...
        if _is_muon_sharding_optimizer(optimizer):
            self.manipulated_state_dict = self._muon_manipulate_state_dict(
                model, optimizer
            )
            ...
            self.manipulated_config_to_save = copy.deepcopy(
            ...
            self.manipulated_config_to_save = (
                self.manipulated_config_to_save.to_json_string(use_diff=True)
            )
            ...
            (
                ...
                model, merge_tensor_parallel=False
            )
            ...
            if self.manipulated_config_to_save is None:
                ...
                self.manipulated_config_to_save = (
                    self.manipulated_config_to_save.to_json_string(
                        use_diff=True
                    )
                )
...
class ZeroCostCheckpointManager:
    def __init__(
        ...
        assert capacity_usage <= 1.0, (
            "capacity_usage must be less than or equal to 1.0"
        )
        ...
        self.pipeline_hooks_steps = max(
            int(pipeline_hooks_capacity * capacity_usage), 1
        )
        logger.info(
        ...
        assert hasattr(fleet, "_hcg"), (
            "ZeroCostCheckpoint Only support `use_hybrid_parallel`"
        )
    ...
    def set_ema_state_dict(self, path):
        ...
        for worker in self.workers:
            assert worker.status.value == ZCCWorkerStatus.IDLE.value, (
                "[ZCC manager] worker should be idle, when "
            )
    ...
    def update_zcc_workers(
        self, new_version, dynamic_objecs, static_object, global_step
    ):
        ...
        task = (
            ZCCTaskType.UPDATE,
            [self.cache_version, dynamic_objecs, static_object],
        )
        logger.info(
            f"[ZCC manager] updating zcc workers, version: {self.cache_version}"
        )
        for worker in self.workers:
        ...
        if self.ema_shared_metas is not None:
            ...
            for worker in self.workers:
                worker.task_queue.put(
                    (
                        ZCCTaskType.LOAD_EMA_FROM_SHARED_MEM,
                        self.ema_shared_metas,
                    )
                )
            logger.info(
    ...
    def sync_offload_status(self):
        ...
        assert self.current_worker is not None, (
            "[ZCC manager] current_worker must not be None"
        )
        while True:
    ...
    def report_error_worker(self):
        for worker in self.workers:
            if worker.status.value == ZCCWorkerStatus.ERROR.value:
                logger.error(
                    f"[ZCC manager] Worker{worker.worker_id} encountered error."
                )
...
class ZeroCostCheckpointWorker:
    ...
    def process_update_task(self, updates):
        ...
        self.optimizer_states_name_path = dynamic_objecs[
            "optimizer_states_name_path"
        ]
        self.model_states_name_path = dynamic_objecs["model_states_name_path"]
        self.build_fusion_storage_helper(
            optimizer_states_meta, model_states_meta
        )
    ...
    def process_prepare_task(self, prepares):
        ...
        self.lr_scheduler, self.trainer_state, self.rng_state = (
            non_cached_objects
        )
    ...
    def process_offload_task(self, dump, global_step):
        ...
        actual_offload_size = (
            min(
                self.offloaded_numels + self.chunk_size_in_numel, self.all_numel
            )
            - self.offloaded_numels
        )
        # Scene1: offload optimizer only
        if (
            self.offloaded_numels + actual_offload_size
            <= self.optimizer_fusion_storage_helper.buffer_length
        ):
            self.optimizer_fusion_storage_helper.sync_partial_param(
                start=self.offloaded_numels,
                end=self.offloaded_numels + actual_offload_size,
            )
        # Scene2: offload optimizer and param
            ...
            self.optimizer_fusion_storage_helper.sync_partial_param(
                start=self.offloaded_numels,
                end=self.optimizer_fusion_storage_helper.buffer_length,
            )
        ...
        if self.offloaded_numels == self.all_numel:
            ...
            self.offloaded_numels = 0
            self.status.value = (
                ZCCWorkerStatus.ERROR.value
                if need_report_error
                else ZCCWorkerStatus.IDLE.value
            )
            return True
        return False
    ...
    def process_dump_task(self):
        ...
        if self.flash_device_save_dir:
            try:
                self.process_dump_task_impl(
                    self.flash_device_save_dir, saved_signal_type="formal"
                )
                logger.info(
                    f"[ZCC Worker{self.worker_id}] Dumping to flash device done: {self.flash_device_save_dir}"
                )
            except Exception as e:
                logger.error(
                    f"{FC_DUMP_ERROR} [ZCC Worker{self.worker_id}] Failed to dump to flash device: {e}"
                )
        if self.persistent_save_dir:
            try:
                ...
                logger.error(
                    f"[ZCC Worker{self.worker_id}] Failed to dump to persistent device: {e}"
                )
                need_report_error = True
        return need_report_error

    def _filter_moe_no_sync_optimizer_params(
        self, model_meta, optimzier_state_dict
    ):
        """
        ...
        assert "master_weights" in optimzier_state_dict, (
            optimzier_state_dict.keys()
        )
        param_names_in_master_weights = list(
            optimzier_state_dict["master_weights"].keys()
        )
        filter_optimzier_state_dict["master_weights"] = OrderedDict()
        suffix = f"tp{self.mp_rank:0>2d}_pp{self.pp_rank:0>2d}"
        dyname_to_pname = model_meta["sharding_metas"][suffix][
        ...
        for k, pname in dyname_to_pname.items():
            ...
            if is_no_sync:
                if pname in param_names_in_master_weights:
                    filter_optimzier_state_dict["master_weights"][pname] = (
                        optimzier_state_dict["master_weights"][pname]
                    )
                else:
                    pass
    ...
    def _dump_states(self, output_dir):
        ...
        with device_guard("cpu"):
            model_states_name_path = os.path.join(
                output_dir, self.model_states_name_path
            )
            state_dict = self.param_fusion_storage_helper.state_dict()
            # Step2.2: save optimizer states
            optimizer_state_name_path = os.path.join(
                output_dir, self.optimizer_states_name_path
            )
        ...
        if self.ema_coef is not None:
            ema_name_path = os.path.join(
                output_dir, self.optimizer_states_name_path
            ).replace("optimizer", "ema")
            ema_state_dict = self.zcc_ema_processor.ema_state_dict()

        if self.dp_rank <= 0 or self.use_expert_parallel:
            if self.dp_rank > 0:  # ep
                opt_state_dict = self._filter_moe_no_sync_optimizer_params(
                    self.model_meta_content, opt_state_dict
                )
                if self.ema_coef is not None:
                    # non master-weights in `ema-state-dict` when dp >1 will be filtered, which is acceptable
                    ema_state_dict = self._filter_moe_no_sync_optimizer_params(
                        self.model_meta_content, ema_state_dict
                    )
    ...
    def _dump_args_and_state(self, output_dir):
        ...
        if self.rng_state is not None:
            rng_state_name_path = os.path.join(
    ...
    def process_dump_task_impl(self, output_dir, saved_signal_type="tmp"):
        ...
        saved_signal_path = os.path.join(
            output_dir, f"{saved_signal_prefix}_{self.global_rank}"
        )
        potential_signal_path = os.path.join(
            output_dir, f"saved_signal_{self.global_rank}"
        )
        ...
        try:
            while True:
                ...
                if task_type == ZCCTaskType.FINISH:
                    ...
                    dumped = self.process_offload_task(
                        dump=save_info_tuple is not None, global_step=task_body
                    )
                    if dumped:
            ...
            logger.info(
                f"[ZCC Worker{self.worker_id}] failed!!, Exception:{e}\n Traceback:{traceback.format_exc()}\n"
            )
            raise e

    def build_fusion_storage_helper(
        self, optimizer_states_meta, model_states_meta
    ):
        (
            accumulators_meta,
            master_weights_meta,
        ...
        if self.param_fusion_storage_helper is None:
            self.param_fusion_storage_helper = ParamFusionStorageHelper(
                model_param_mappings, model_ipc_meta_mappings
            )
        else:
            self.param_fusion_storage_helper.reset_meta(
                model_param_mappings, model_ipc_meta_mappings
            )

    def manage_offload_chunk(self):
        # TODO(@gexiao): more precise slice for different dtype
        optimizer_offload_numel = (
            self.optimizer_fusion_storage_helper.buffer_length
        )
        ...
        self.chunk_size_in_numel = (
            self.all_numel - 1
        ) // self.offload_chunks + 1
        logger.info(
    ...
    def _load_ema_from_shared_memory(self, ema_shared_metas):
        ...
        logger.warning(
            "[ZCC Worker] _load_ema_from_shared_memory not implemented for this worker type"
        )
    ...
    def _reverse_unified_name_for_ema(self, state_dict):
        ...
        """
        if (
            not hasattr(self, "unified_name_mapping")
            or self.unified_name_mapping is None
        ):
            return state_dict
...
class EMABuffer(ABC):
    ...
    def _load(self, resume_from_checkpoint):
        ...
        success, err_msg = self._check_consistent_dist_strategy(
    ...
    def save(self, global_step):
        base_path = os.path.join(
            self.args.output_dir, f"{PREFIX_CHECKPOINT_DIR}-{global_step}"
        )
    ...
    def _ema_impl(self, state_dict, ema_state_dict):
        ...
        for k, v in state_dict.items():
            if k in ema_state_dict:
                ...
                ema_tensor = (
                    ema_coef * ema_tensor.cuda() + (1 - ema_coef) * v.cuda()
                )
            ...
            if self.offload:
                src = (
                    v.detach()
                    if isinstance(v, paddle.base.framework.EagerParamBase)
                    else v
                )
...
class EMABufferShardingIOBased(EMABuffer):
    def __init__(self, resume_from_checkpoint, args, sharding_io, offload=True):
        assert sharding_io is not None, (
            "EMA should be only enabled when save_sharded_model is True"
        )
        self.sharding_io = sharding_io
...
class EMABufferFcBased(EMABuffer):
    def __init__(
        ...
        self.dist_info_collector_and_validator = DistInfoCollectorValidator(
            args, hcg
        )
        if get_env_device() == "xpu":
            self.device_id = int(os.getenv("FLAGS_selected_xpus"))
    ...
    def _get_model_meta(self):
        return self.dist_info_collector_and_validator.gather_distributed_model_meta(
            self.model, self.optimizer
        )

    def _ema_path(self, base_path):
        return os.path.join(
            base_path, "ema_state", f"{dist.get_rank()}_0.distcp"
        )
    ...
    def _check_consistent_dist_strategy(self, resume_from_checkpoint):
        return self.dist_info_collector_and_validator.check_same_strategy(
            resume_from_checkpoint
        )
    ...
    def save(self, global_step):
        ...
        logger.info(
            f"[NonZCC EMA] Save EMA state (FC format) to {ema_save_dir} done"
        )
    ...
    def _build_ema_save_sharded_state_dict(self):
        ...
        for unified_key, sw in opt_sharded_sd.items():
            ...
            if sw.is_flattened and sw.flattened_range is not None:
                ...
                actual_numel = (
                    sw.flattened_range.stop - sw.flattened_range.start
                )
            ...
            if (
                getattr(sw, "grouped_gemm_param", False)
                and local_data.ndim == 3
            ):
    ...
    def _load_fc_format(self, ema_dir):
        ...
        logger.info(
            f"[NonZCC EMA] Loading {len(ema_target)} EMA tensors via dist.load_state_dict from {ema_dir}"
        )
        dist.load_state_dict(
            ema_target, ema_dir, aoa_config=self.args.aoa_config
        )
            ...
            if unified_key.endswith(".w_0"):
                ...
                if opt_tensor.ndim == 1:
                    ...
                    if flat._numel() < expected_numel:
                        padded = paddle.zeros(
                            [expected_numel], dtype=loaded_tensor.dtype
                        )
...
class NonZCCEMACallback(TrainerCallback):
    ...
    def create_nonzcc_callback(
        ...
        if args.save_checkpoint_format == "flex_checkpoint":
            ...
            ema_buffer = EMABufferShardingIOBased(
                resume_from_checkpoint, args, sharding_io, offload=offload
            )
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step % args.zcc_ema_interval == 0:
            self.buffer.ema_accumulate(
                state.global_step, state.loss, args.zcc_ema_loss_threshold
            )
        if control.should_save:
            self.buffer.save(state.global_step)
        if control.should_save_hf:
            assert self.ema_state_assembler is not None, (
            ...
            ema_sharded_state_dict = (
                self.ema_state_assembler._build_ema_sharded_state_dict(
                    ema_state_dict_cuda
                )
            )
            self.ema_state_assembler._save_full_ema_states(
                state.global_step, ema_sharded_state_dict
            )
            del ema_sharded_state_dict
            del ema_state_dict_cuda
            end_time = time.time()
            logger.info(
                f"Unsplit EMA states saved successfully, time taken: {end_time - start_time} s"
            )
...
class DistInfoCollectorValidator:
    ...
    def _gather_sharding_metas(self, model, optimizer):
        ...
        if is_muon:
            ...
            if sharding_strategy == SHARDING_STRATEGY_V1:
                optimizer = unwrap_optimizer(
                    optimizer, DygraphShardingOptimizer
                )
                param2rank = dict(optimizer._param2rank.items())
            else:
                pp_overlap = unwrap_optimizer(
                    optimizer, DygraphShardingOptimizerV2
                ).pp_overlap
        ...
        sharding_metas_list = self._all_gather_simple_object(
            sharding_metas, self.hcg.get_model_parallel_group()
        )
        sharding_metas = {
            k: v for e in sharding_metas_list for (k, v) in e.items()
        }
        sharding_metas_list = self._all_gather_simple_object(
            sharding_metas, self.hcg.get_pipe_parallel_group()
        )
        sharding_metas = {
            k: v for e in sharding_metas_list for (k, v) in e.items()
        }
        if self.args.expert_model_parallel_size > 1:
            sharding_metas_list = self._all_gather_simple_object(
                sharding_metas, self.hcg.get_expert_parallel_group()
            )
            sharding_metas = {
                k: v for e in sharding_metas_list for (k, v) in e.items()
            }
    ...
    def _check_distributed_strategy(self, parallel_config):
        ...
        if ep_degree > 1:
            ...
            assert (
                tp_degree * sharding_degree == ep_degree * moe_sharding_degree
            ), "mismatch parallel degree settings"
    ...
    def check_same_strategy(self, resume_from_checkpoint=None):
        if resume_from_checkpoint:
            ...
            old_config = self._load_model_meta_impl(resume_from_checkpoint)[
                "parallel_config"
            ]
            ...
            for key in keys:
                if key not in cur_config:
                    ...
                    if old_value != cur_value:
                        return (
                            False,
                            f"{key} not match: {old_value} vs {cur_value}",
                        )
        return True, None
...
def saved_ckptmeta(
    ...
    ckpt_file_name,
    process_group=None,
    replicate_saved_into_local=False,
):
    with paddle.base.dygraph.guard():
        ...
        for key, val in flat_state_dict.items():
            assert isinstance(val, ShardedWeight), (
                f"expected ShardedWeight, but got {type(val)}"
            )
            ...
            if (flattened_range is not None) and (
                flattened_range.stop - flattened_range.start <= 0
            ):
                continue
...
class ZeroCostCheckpointCallbackFcBased(ZeroCostCheckpointCallback):
    ...
    def _manipulate_state_dict_and_config(self, model_to_save, optimizer):
        ...
        if self.args.bf16:
            ...
            optimzier_state_dict = split_opt_state(
                optimzier_state_dict, group_getter
            )
            state_dict = split_model_state(state_dict, group_getter)
            for gid in gids:
                sub_opt_state = optimzier_state_dict.get(gid, {})
                param_names_in_master_weights = list(
                    sub_opt_state.get("master_weights", {}).keys()
                )
            ...
            logger.info(
                f"param_names_in_master_weights len:{len(param_names_in_master_weights)}, bf16 state_dict len:{len(state_dict)}, :{state_dict.keys()}"
            )
    ...
    def _cache_meta_for_sharded_save(self, model, optimizer):
        ...
        if _is_muon_sharding_optimizer(optimizer):
            self.manipulated_state_dict = (
                self._muon_manipulate_sharded_state_dict(model, optimizer)
            )
        else:
            (
                self.manipulated_state_dict
            ) = self._manipulate_state_dict_and_config(model, optimizer)
        ...
        self.model_meta = DistInfoCollectorValidator(
            self.args, self.hcg
        ).gather_distributed_model_meta(model, optimizer)
        ...
        opt_state_dict_tmp = optimizer.sharded_state_dict(
        ...
        self.master_weight_ckpt_meta, self.master_weights_filter = (
            saved_ckptmeta(
                master_weights,
                self.ckpt_data_name,
                replicate_saved_into_local=self.args.replicate_saved_into_local,
        ...
        if ema_master_weights_sharded:
            self.ema_master_weight_ckpt_meta, self.ema_master_weights_filter = (
                saved_ckptmeta(
        ...
        if ema_model_params_sharded:
            self.ema_model_params_ckpt_meta, self.ema_model_state_filter = (
                saved_ckptmeta(
                    ema_model_params_sharded,
                    self.ckpt_data_name,
                    replicate_saved_into_local=True,
        ...
        self.unified_name_mapping, self.param_slice_info = (
            self._gen_unified_name(optimizer, model.sharded_state_dict())
        )
        logger.info("Cache distributed model meta done.")
    ...
    def _pack_dynamic_objects(self):
        ...
        dynamic_objecs["distcp_file_name"] = (
            self.ckpt_data_name,
            self.ckpt_meta_name,
        )
        ...
        dynamic_objecs["ema_master_weight_ckpt_meta"] = (
            self.ema_master_weight_ckpt_meta
        )
        dynamic_objecs["ema_master_weights_filter"] = (
            self.ema_master_weights_filter
        )
        dynamic_objecs["ema_model_params_ckpt_meta"] = (
            self.ema_model_params_ckpt_meta
        )
...
class ZeroCostCheckpointWorkerFcBased(ZeroCostCheckpointWorker):
    def process_update_task(self, updates):
        ...
        self.ema_master_weight_ckpt_meta = dynamic_objecs.get(
            "ema_master_weight_ckpt_meta"
        )
        self.ema_master_weights_filter = dynamic_objecs.get(
            "ema_master_weights_filter", {}
        )
        self.ema_model_params_ckpt_meta = dynamic_objecs.get(
            "ema_model_params_ckpt_meta"
        )
        self.ema_model_state_filter = dynamic_objecs.get(
            "ema_model_state_filter", {}
        )
    ...
    def _replace_pname_with_unified(self, state_dict):
        ...
        for key, value in state_dict.items():
            assert key in self.unified_name_mapping, (
                f"{key} not in {self.unified_name_mapping.keys()}"
            )
    ...
    def _save_model_state(self, output_dir):
        ...
        self.model_states_path = os.path.join(
            output_dir, MODEL_STATE_DIC, data_file_name
        )
        self.model_states_meta_path = os.path.join(
            output_dir, MODEL_STATE_DIC, meta_file_name
        )
        ...
        if self.dp_rank <= 0 or self.use_expert_parallel:
            with device_guard("cpu"):
                ...
                state_dict = self._filter_state_dict(
                    state_dict, self.model_state_filter
                )
                ...
                if self.device_id == 0:
                    paddle.save(
                        self.model_ckpt_meta, self.model_states_meta_path
                    )
        logger.info("[ZCC worker] Finish model states saved.")

    def _save_opt_state(self, output_dir):
        data_file_name, meta_file_name = self.distcp_file_name
        self.opt_state_path = os.path.join(
            output_dir, OPTIMIZER_STATE_DIC, data_file_name
        )
        self.opt_state_meta_path = os.path.join(
            output_dir, OPTIMIZER_STATE_DIC, meta_file_name
        )

        self.master_weight_path = os.path.join(
            output_dir, MASTER_WEIGHT_DIC, data_file_name
        )
        self.master_weight_meta_path = os.path.join(
            output_dir, MASTER_WEIGHT_DIC, meta_file_name
        )

        if self.dp_rank <= 0 or self.use_expert_parallel:
            with device_guard("cpu"):
                opt_state_dict = (
                    self.optimizer_fusion_storage_helper.state_dict()
                )
                master_weights = opt_state_dict.pop("master_weights", {})

                opt_state_dict = self._replace_pname_with_unified(
                    opt_state_dict
                )
                logger.info(
                    "[ZCC worker] opt state dict replace pname using unified name."
                )

                master_weights = self._replace_pname_with_unified(
                    master_weights
                )
                logger.info(
                    "[ZCC worker] master weightsdict replace pname using unified name."
                )

                opt_state_dict = self._slice_padded_tensor(
                    opt_state_dict, self.param_slice_info
                )
                logger.info(
                    "[ZCC worker] opt state dict slice padded tensor complete."
                )
                master_weights = self._slice_padded_tensor(
                    master_weights, self.param_slice_info
                )
                logger.info(
                    "[ZCC worker] master weights slice padded tensor complete."
                )
            ...
            opt_state_dict = self._filter_state_dict(
                opt_state_dict, self.opt_state_filter
            )
            logger.info(
                "[ZCC worker] opt state dict filter by opt_state_filter complete."
            )
            master_weights = self._filter_state_dict(
                master_weights, self.master_weights_filter
            )
            logger.info(
                "[ZCC worker] master weights dict filter by master_weights_filter complete."
            )
            ...
            if self.grouped_gemm_params and len(self.grouped_gemm_params) > 0:
                for k, v in opt_state_dict.items():
                    ...
                    if (
                        ...
                        and struct_name in self.grouped_gemm_params
                    ):
            ...
            if self.device_id == 0:
                ...
                paddle.save(
                    self.master_weight_ckpt_meta, self.master_weight_meta_path
                )
            logger.info(
                "[ZCC worker] Finish opt states and master weights saved."
            )

    def _save_ema_state(self, output_dir):
        data_file_name, meta_file_name = self.distcp_file_name
        if (
            self.dp_rank <= 0 or self.use_expert_parallel
        ) and self.ema_coef is not None:
            self.ema_name_path = os.path.join(
                output_dir, EMA_STATE_DIC, data_file_name
            )
            self.ema_meta_path = os.path.join(
                output_dir, EMA_STATE_DIC, meta_file_name
            )
            ema_state_dict = self.zcc_ema_processor.ema_state_dict()
            ...
            unmapped_model_params = {
                k: v
                for k, v in model_params.items()
                if k not in self.unified_name_mapping
            }
            model_params = {
                k: v
                for k, v in model_params.items()
                if k in self.unified_name_mapping
            }
            unmapped_master_weights = {
                k: v
                for k, v in master_weights.items()
                if k not in self.unified_name_mapping
            }
            ...
            if self.dp_rank > 0:  # ep
                ...
                master_weights = self._filter_state_dict(
                    master_weights, self.ema_master_weights_filter
                )
                model_params = self._filter_state_dict(
                    model_params, self.ema_model_state_filter
                )
    ...
    def _merge_ema_ckpt_meta(self):
        ...
        if (
            self.ema_master_weight_ckpt_meta is None
            and self.ema_model_params_ckpt_meta is None
        ):
            return None
        ...
        for meta in [
            ...
            if meta is None:
                continue
            if (
                hasattr(meta, "state_dict_metadata")
                and meta.state_dict_metadata
            ):
    ...
    def _load_ema_from_shared_memory(self, ema_shared_metas):
        ...
        assert self.zcc_ema_processor is not None, (
            "zcc_ema_processor not initialized."
        )
