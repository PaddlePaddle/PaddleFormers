...
def merge_splited_param(
    ...
    for key in list(state_dict.keys()):
        ...
        static_name = (
            key if is_master_weights else generate_base_static_name(key)[0]
        )
            ...
            for send_rank, begin, end in send_info:
                ...
                if send_rank == recv_rank:
                    tensor = (
                        ...
                        else state_dict[key][: padding_start - begin]
                    )
                    ...
                    length = (
                        ...
                        if padding_start >= padding_end
                        else padding_start - begin
                    )
                    tmp_tensor = paddle.empty(
                        shape=[length], dtype=state_dict[key].dtype
                    )
        ...
        for key in list(state_dict.keys()):
            if int(state_dict[key].numel()) == 1:  # for example: beta1, beta2
                static_name = (
                    ...
                    else generate_base_static_name(key)[0]
                )
...
def gather_splited_param_for_optimizer(optimizer, ckpt_quant_stage="O0"):
    ...
    dist.all_gather_object(
        param_slice_info_list, param_slice_info, group=sharding_group
    )
    ...
    for key in list(optim_state_dict.keys()):
        ...
        if static_name in param_slice_info.keys():
            if (
                int(optim_state_dict[key].numel()) == 1
    ...
    for key in partial_tensor_list:
        ...
        for slice_info in param_slice_info_list:
            ...
            if end > begin:
                sharding_ranklist.append(
                    (slice_info["global_rank"], begin, end)
                )
        recv_table[key] = sharding_ranklist[0][
            0
        ]  # which sharding_rank to recv the splited tensor
        send_table[key] = [
            (rank, begin, end) for rank, begin, end in sharding_ranklist
        ]
...
def reshape_params(
    state_dict, struct2static_name_mappings, param_shape_info, param_slice_info
):
...
def load_unified_optimizer_split_param(
    args, model, optimizer, resume_from_checkpoint, ckpt_quant_stage="O0"
):
    ...
    if (
        hasattr(args, "enable_sharding_comm_overlap")
        and args.enable_sharding_comm_overlap
    ):
        comm_buffer_list = list(chain(*model._chunk_2_comm_buffers.values()))
        model = unwrap_model(model)

    expected_keys, param_slice_info, param_shape_info = get_params_info(
        comm_buffer_list
    )
    expected_keys = {
        static2struct_name_mappings.get(name, None) for name in expected_keys
    }
    expected_keys_optim = []
    ...
    if len(resolved_archive_file) > 1:
        resolved_archive_file = tqdm(
            resolved_archive_file, desc="Loading optimizer shards"
        )
    ...
    if has_master_weights:
        ...
        if len(resolved_archive_file_mw) > 1:
            resolved_archive_file_mw = tqdm(
                resolved_archive_file_mw, desc="Loading master weights shards"
            )
    ...
    def load_resolved_archive_file(
        ...
        if model.config.tensor_model_parallel_size > 1:
            ...
            if not is_master_weights:
                tp_actions = mapping_optimizer_tp_actions(
                    tp_actions, expected_keys
                )

        for shard_file in resolved_archive_file:
            if expected_keys.isdisjoint(
                sharded_metadata["file_map"][os.path.split(shard_file)[-1]]
            ):
                continue
            if model.config.tensor_model_parallel_size > 1:
    ...
    for key in list(state_dict_optim.keys()):
        ...
        state_dict_optim[key] = state_dict_optim[key]._copy_to(
    ...
    if has_master_weights:
        ...
        for key in list(state_dict_master_weight.keys()):
            ...
            state_dict_master_weight[key] = state_dict_master_weight[
                key
            ]._copy_to(paddle.framework._current_expected_place(), False)
            returned_optim_state_dict["master_weights"][static_name] = (
                state_dict_master_weight.pop(key)
            )

            # master weight cast (only in remove_master_weight)
            if (
                returned_optim_state_dict["master_weights"][static_name].dtype
                != paddle.float32
            ):
                returned_optim_state_dict["master_weights"][static_name] = (
                    paddle.cast(
...
def load_non_merge_optimizer_with_split_param(
    args, model, optimizer, resume_from_checkpoint, ckpt_quant_stage="O0"
):
    returned_optim_state_dict = nested_copy(optimizer.state_dict())

    optimizer_name = _add_variant(
        SAFE_OPTIMIZER_NAME, args.optimizer_name_suffix
    )
    master_weights_name = _add_variant(
        SAFE_MASTER_WEIGHTS_NAME, args.optimizer_name_suffix
    )
                ...
                with safe_open(new_path, framework="numpy") as fin:
                    ...
                    for key in unfound_keys:
                        if key in keys:
                            ...
                            state_dict[key] = tensor._copy_to(
