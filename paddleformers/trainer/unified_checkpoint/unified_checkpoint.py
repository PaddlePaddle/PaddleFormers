...
class UnifiedCheckpointHandler:
    ...
    def save_unified_checkpoint(
        ...
        if isinstance(model, PretrainedModel):
            ...
            raise ValueError(
                "Unified checkpoint only supports PretrainedModel, LoRAModel!"
            )
        ...
        if (
            ...
            if is_need_master_weight(
                optimizer, is_fp16_or_bp16=(self.args.fp16 or self.args.bf16)
            ):
                logger.info(
        ...
        if not skip_save_model_weight:
            state_dict, shard_file, sharded_index = (
    ...
    def load_unified_checkpoint(
        self, model, resume_from_checkpoint: str, convert_from_hf=False
    ):
        ...
        if paddle.distributed.get_world_size() <= 1:
            load_single_card_checkpoint(
                model, resume_from_checkpoint, convert_from_hf=convert_from_hf
            )
            return

        local_resume = check_unified_checkpoint(
            self.args, model, resume_from_checkpoint, safe_serialization=True
        )
    ...
    def save_non_merge_optimizer(
        ...
        global_master_weights = reduce_master_weights_status(
        ...
        for key in list(optim_state_dict.keys()):
            ...
            new_name = (
                static2struct_name_mappings[static_name] + "/" + type_name
            )
        ...
        master_weights_name = _add_variant(
            SAFE_MASTER_WEIGHTS_NAME, self.args.optimizer_name_suffix
        )

        sharded_optim_index = {}
        # save opt index json if checkpoint quantization is on.
        if (
            self.args.ckpt_quant_stage != "O0"
            and "quant_reach_limit" not in infohub
        ):
            sharded_optim_index["ckpt_quant_stage"] = self.args.ckpt_quant_stage

        sharded_optim_index["quant_ckpt_resume_times"] = (
            infohub["quant_ckpt_resume_times"]
        ...
        if optim_shard_num > 1:
            ...
            split_key_list = [
                key_list[i::optim_shard_num] for i in range(optim_shard_num)
            ]
            for i in range(optim_shard_num):
                ...
                self.async_handler._file_save_async_or_sync(
                    state_dict_shard,
                    path=os.path.join(
                        output_dir, optimizer_name + f"_shard_{i + 1:04d}"
                    ),
                    signal_path=signal_dir,
                    is_sync=is_sync_save,
                    state_dict_type="optimizer_weight",
                    ckpt_quant_stage=self.args.ckpt_quant_stage
                    if "quant_reach_limit" not in infohub
                    else "O0",
                )
            ...
            if master_weights is not None:
                ...
                for i in range(optim_shard_num):
                    ...
                    self.async_handler._file_save_async_or_sync(
                        ...
                        path=os.path.join(
                            output_dir,
                            master_weights_name + f"_shard_{i + 1:04d}",
                        ),
                        signal_path=signal_dir,
                        is_sync=is_sync_save,
    ...
    def load_non_merge_optimizer(
        ...
        if (
            is_sharding_split_param_mode(self.args)
            and "gather_split_param" in self.args.unified_checkpoint_config
        ):
        ...
        master_weights_path = os.path.join(
            resume_from_checkpoint, master_weights_name
        )
        ...
        optim_shard_num = 1
        optimizer_index_file = os.path.join(
        ...
        if optim_shard_num > 1:
            ...
            for i in range(optim_shard_num):
                ...
                # normal AMP O2
                if not is_amp_o1 and os.path.isfile(
                    master_weights_path + f"_shard_{i + 1:04d}"
                ):
            ...
            if not is_amp_o1 and os.path.isfile(master_weights_path):
                master_weights = load_state_dict(
                    master_weights_path, None, None, device=device
                )
    ...
    def save_unified_optimizer(
        self, model, optimizer, output_dir, signal_dir, optim_shard_num=1
    ):
        ...
        if "ignore_merge_optimizer" in self.args.unified_checkpoint_config:
            self.save_non_merge_optimizer(
                ...
                output_dir,
                signal_dir,
                optim_shard_num=optim_shard_num,
            )
            return

        # Split into naive optimizer params and master weights.
        optim_state_dict, shard_optim_file, sharded_optim_index = (
    ...
    def load_unified_optimizer(
        self, model, optimizer, resume_from_checkpoint, offload=False
    ):
        """Load potential model checkpoint
        ...
        if paddle.distributed.get_world_size() <= 1:
            optim_state_dict = load_single_card_optimizer(
                model, optimizer, resume_from_checkpoint
            )
        ...
        if quant_ckpt_resume_times >= MAX_QUANTIZATION_TIMES:
            ...
            logger.info(
                "Checkpoint quantization time reach limit and will be closed."
            )
...
def unified_checkpoint_into_shards(
    ...
    if config_to_save.tensor_model_parallel_size > 1:
        ...
        state_dict = merge_tensor_parallel_with_shard(
    ...
    if isinstance(model_to_save, LoRAModel):
        weights_name = (
            SAFE_PEFT_WEIGHTS_NAME if safe_serialization else LORA_WEIGHTS_NAME
        )
    else:
        weights_name = (
...
def unified_optimizer_into_shards(
    ...
    if not is_master_weights:
        ...
        if state_dict is not None:
            for key in list(state_dict.keys()):
                state_dict[static2struct_name_mappings[key]] = state_dict.pop(
                    key
                )
                if key in extra_save_keys:
                    state_dict[extra_save_keys[key]] = state_dict[
                        static2struct_name_mappings[key]
                    ]
    ...
    if tp_size > 1:
        ...
        if isinstance(model, LoRAModel):
            tp_actions = model._get_tensor_parallel_convert_actions(
                model_keys, is_split=False, ignore_error=True
            )
        else:
    ...
    if not is_master_weights:
        name = (
            SAFE_OPTIMIZER_NAME if safe_serialization else PADDLE_OPTIMIZER_NAME
        )
    else:
        name = (
            SAFE_MASTER_WEIGHTS_NAME
            if safe_serialization
            else PADDLE_MASTER_WEIGHTS_NAME
        )
        if (
            UnifiedCheckpointOption.SKIP_SAVE_MODEL_WEIGHT.value
    ...
    if args.should_save and state_dict is not None:
        if not is_master_weights:
            if (
                args.ckpt_quant_stage in ["O1", "O2"]
                and "quant_reach_limit" not in infohub
            ):
