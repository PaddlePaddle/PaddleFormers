...
class Template:
    ...
    def fix_special_tokens(self, tokenizer: "PreTrainedTokenizer") -> None:
        ...
        if stop_words:
            num_added_tokens = tokenizer.add_special_tokens(
                {"additional_special_tokens": stop_words},
...
class ReasoningTemplate(Template):
    ...
    def encode_oneturn(
        ...
        if self.enable_thinking is False:  # remove all cot
            messages[-1]["content"] = self.remove_thought(
                messages[-1]["content"]
            )
        ...
        if (
            ...
            if not self.enable_thinking:  # do not compute loss
                ...
                response_ids = (
                    self.get_thought_word_ids(tokenizer) + response_ids
                )
    ...
    def encode_multiturn(
        ...
        if self.enable_thinking is False:  # remove all cot
            for i in range(1, len(messages), 2):
                messages[i]["content"] = self.remove_thought(
                    messages[i]["content"]
                )
        ...
        for i in range(0, len(messages), 2):
            if (
                ...
                and self.thought_words[1].strip()
                not in messages[i + 1]["content"]
            ):  # add empty cot
                if not self.enable_thinking:  # do not compute loss
                    encoded_messages[i] += self.get_thought_word_ids(tokenizer)
                else:  # do compute loss
                    encoded_messages[i + 1] = (
                        self.get_thought_word_ids(tokenizer)
                        + encoded_messages[i + 1]
                    )
...
def register_template(
    ...
    default_slots = (
        ["{{content}}"] if efficient_eos else ["{{content}}", {"eos_token"}]
    )
...
def parse_template(tokenizer: "PreTrainedTokenizer") -> "Template":
    ...
    template_class = (
        ReasoningTemplate if "<think>" in assistant_slot else Template
    )
    assistant_slot = (
        assistant_slot.replace("<think>", "")
        .replace("</think>", "")
        .lstrip("\n")
    )  # remove thought tags
    ...
    return template_class(
        ...
        format_prefix=EmptyFormatter(slots=[prefix])
        if prefix
        else EmptyFormatter(),
        default_system=default_system,
        suffix=[],
...
def get_template_and_fix_tokenizer(dataset_config) -> "Template":
    ...
    if dataset_config["template"] is None:
        ...
        if dataset_config["template"] not in TEMPLATES:
            raise ValueError(
                f"Template {dataset_config['template']} does not exist."
            )
    ...
    if dataset_config["tool_format"] is not None:
        ...
        template.format_function = FunctionFormatter(
            slots=default_slots, tool_format=dataset_config["tool_format"]
        )
        template.format_tools = ToolFormatter(
            tool_format=dataset_config["tool_format"]
        )
...
register_template(
    ...
    format_assistant=ThinkingFormatter(
        slots=["<response>\n{{content}}\n</response>\n"]
    ),
    format_system=StringFormatter(
        slots=[
...
register_template(
    ...
    chat_sep="<|end_of_sentence|>",
    mm_plugin=get_mm_plugin(
        name="paddleocr_vl", image_token="<|IMAGE_PLACEHOLDER|>"
    ),
)
...
register_template(
    ...
    format_function=FunctionFormatter(
        slots=["{{content}}"], tool_format="qwen"
    ),
    format_observation=StringFormatter(
        slots=[
            "<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n"
...
register_template(
    ...
    format_system=StringFormatter(slots=["<|system|>\n{{content}}"]),
    format_function=FunctionFormatter(
        slots=["{{content}}"], tool_format="glm4"
    ),
    format_observation=StringFormatter(
        slots=["<|observation|>\n{{content}}<|assistant|>"]
    ),
...
register_template(
    ...
    format_assistant=StringFormatter(
        slots=["<|channel|>final<|message|>{{content}}"]
    ),
    format_system=StringFormatter(
        slots=[
...
register_template(
    ...
    format_function=FunctionFormatter(
        slots=["{{content}}"], tool_format="llama3"
    ),
    format_observation=StringFormatter(
        slots=[
            (
...
register_template(
    name="phi4",
    format_user=StringFormatter(
        slots=["<|user|>{{content}}<|end|><|assistant|>"]
    ),
    format_assistant=StringFormatter(slots=["{{content}}"]),
