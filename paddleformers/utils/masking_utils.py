...
def _make_causal_mask(input_ids_shape, past_key_values_length):
    ...
    if get_env_device() == "npu":
        mask = paddle.tril(paddle.ones((target_length, target_length))).astype(
            "int32"
        )
    else:
        mask = paddle.tril(
...
def _get_interleave(n):
    ...
    if np.log2(n).is_integer():
        ...
        return (
            ...
            + _get_interleave(2 * closest_power_of_2)[0::2][
...
def build_alibi_tensor(
    bool_attention_mask: paddle.Tensor,
    num_heads: int,
    dtype: paddle.dtype,
    tensor_model_parallel_size=1,
) -> paddle.Tensor:
    batch_size, seq_length = (
        bool_attention_mask.shape[0],
        bool_attention_mask.shape[-1],
    )
    slopes = paddle.to_tensor(_get_interleave(num_heads), dtype="float32")
    alibi = slopes.unsqueeze(axis=[1, 2]) * paddle.arange(
        seq_length, dtype="float32"
    ).unsqueeze(axis=[0, 1]).expand([num_heads, -1, -1])
    alibi = alibi.reshape(shape=(1, num_heads, 1, seq_length)).expand(
        [batch_size, -1, -1, -1]
    )
