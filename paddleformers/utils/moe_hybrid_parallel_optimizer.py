...
class MoEHybridParallelClipGrad:
    def __init__(self, clip, hcg, timers=None):
        ...
        self._reproducible_norm = bool(getattr(clip, "reproducible_norm", False))
    ...
    def _dygraph_clip(self, params_grads):
        ...
        if self._reproducible_norm:
            ...
            norm_fn = lambda _p, g: accumulator.accumulate(accumulator.zeros(), g)
        ...
        def add_n_list(tensor_list):
            if self._reproducible_norm:
                return paddle.add_n(tensor_list) if tensor_list else accumulator.zeros()
            if not tensor_list:
    ...
    def _comm_and_clip(
        ...
        global_norm_var_fp32 = ReproducibleL2Norm().finish(total)[0] if self._reproducible_norm else paddle.sqrt(total)
        ...
        if self._reproducible_norm:
            clip_var = paddle.minimum(
                max_global_norm / (global_norm_var_fp32 + 1e-6), paddle.ones_like(global_norm_var_fp32)
            )
