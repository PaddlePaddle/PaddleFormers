...
from ..transformers.video_utils import VideoMetadataType


def positive_any_number(value: float | None = None):
    if value is not None and (
        not isinstance(value, (int, float)) or not value >= 0
    ):
        raise ValueError(
            f"Value must be a positive integer or floating number, got {value}"
        )
...
def padding_validator(value: bool | str | PaddingStrategy | None = None):
    ...
    if value is None:
        ...
        raise ValueError(
            "Value for padding must be either a boolean, a string or a `PaddingStrategy`"
        )
    elif isinstance(value, str) and value not in possible_names:
        raise ValueError(
            f"If padding is a string, the value must be one of {possible_names}"
        )
...
def truncation_validator(value: bool | str | TruncationStrategy | None = None):
    ...
    if value is None:
        ...
        raise ValueError(
            "Value for truncation must be either a boolean, a string or a `TruncationStrategy`"
        )
    elif isinstance(value, str) and value not in possible_names:
        raise ValueError(
            f"If truncation is a string, value must be one of {possible_names}"
        )


def image_size_validator(
    value: int | Sequence[int] | dict[str, int] | None = None,
):
    possible_keys = [
        "height",
        "width",
    ...
    if value is None:
        ...
        k not in possible_keys for k in value.keys()
    ):
        raise ValueError(
            f"Value for size must be a dict with keys {possible_keys} but got size={value}"
        )


def device_validator(value: str | int | None = None):
    possible_names = ["cpu", "gpu"]
    if value is None:
        pass
        ...
        raise ValueError(
            f"If device is an string, the value must be one of {possible_names} but got device={value}"
        )
...
def resampling_validator(value: Union[int, "PILImageResampling"] | None = None):
    ...
    elif not isinstance(value, (PILImageResampling, int)):
        raise ValueError(
            f"The resampling should an integer or `PIL.Image.Resampling`, but got resampling={value}"
        )
...
def tensor_type_validator(value: str | None = None):
    possible_names = ["pd", "np"]
    if value is None:
        pass
    elif not isinstance(value, str) or value not in possible_names:
        raise ValueError(
            f"The tensor type should be one of {possible_names} but got tensor_type={value}"
        )
