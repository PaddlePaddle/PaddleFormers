...
class MergeConfig:
    ...
    tensor_type: str = field(
        default="np",
        metadata={
            "help": "Tensor type to use for the merge. Choose np(CPU Only) or pd (CPU/GPU)"
        },
    )
    n_process: int = field(
        default=1,
        metadata={"help": "Number of processes to use for the merge."},
    )
    merge_prefix: str = field(
        default="model",
        metadata={"help": "Prefix name: model or master_weights"},
    )
    merge_method: str = field(
        default="linear", metadata={"help": "The merge strategy."}
    )
    merge_type: str = field(
        default="linear", metadata={"help": "The type of merge process."}
    )
    sparsify_type: str = field(
        default=None, metadata={"help": "The type of sparsify process."}
    )
    split_pieces: int = field(
        default=8, metadata={"help": "Split large tensor to multi-piece"}
    )
    max_tensor_mem: float = field(
        default=0.5,
        metadata={"help": "Split tensor if exceed setting max_tensor_mem."},
    )
    convert_from_hf: Optional[bool] = field(
        default=True,
        metadata={"help": "Load base model from HuggingFace safetensors."},
    )
    ...
    model_path_str: Optional[str] = field(
        default=None,
        metadata={"help": "Merge model name or path string.(split by ',')"},
    )
    base_model_path: str = field(
        default=None, metadata={"help": "Base model name or path."}
    )
    output_path: str = field(
        default=None, metadata={"help": "Output model name or path."}
    )
    lora_model_path: str = field(
        default=None, metadata={"help": "LoRA model name or path."}
    )
    copy_file_list: Optional[list[str]] = field(
        default=None,
        metadata={
            "help": "Copy file list from base model path or first model path."
        },
    )
    # merge parameters
    weight_list: Optional[list[float]] = field(
        default=None,
        metadata={
            "help": "Relative (or absolute if normalize=False) weighting of a given tensor"
        },
    )
    normalize: bool = field(
        default=True, metadata={"help": "Whether to normalize the weighting."}
    )
    slerp_alpha: float = field(default=0.5, metadata={"help": "Slerp alpha."})
    slerp_normalize_eps: float = field(
        default=1e-8, metadata={"help": "Slerp normalization epsilon value"}
    )
    slerp_dot_threshold: float = field(
        default=0.9995,
        metadata={
            "help": "Slerp dot threshold. If dot value exceeds this threshold, then we consider them as collinear, so use linear instead."
        },
    )
    ties_elect_type: str = field(
        default="sum",
        metadata={"help": "The type of ties mask. 'sum' or 'count'"},
    )
    ...
    rescale: bool = field(
        default=True,
        metadata={"help": "Rescale the weights after sparsifying."},
    )
    reserve_p: float = field(
        default=0.7,
        metadata={"help": "Random reserve probability for the sparsify model."},
    )
    epsilon: float = field(
        default=0.14, metadata={"help": "Epsilon value for magprune."}
    )
    ...
    def config_check(self):
        ...
        if self.tensor_type not in ["np", "pd"]:
            raise ValueError(
                f"Unsupported tensor type: {self.tensor_type}. Support 'np' and 'pd' only."
            )
        if self.lora_model_path is not None:
            if self.base_model_path is None:
                raise ValueError(
                    "Please specify the base_model_path when using LoRA merge."
                )
            self.tensor_type = "pd"
        ...
        if self.lora_model_path is None:
            ...
            if self.model_path_list is not None:
                if (
                    not isinstance(self.model_path_list, list)
                    or len(self.model_path_list) < 2
                ):
                    raise ValueError(
                ...
                if len(self.model_path_list) != len(self.weight_list):
                    raise ValueError(
                        "The length of model_path_list and weight_list must be the same."
                    )
            ...
            if "della" in self.merge_method or self.sparsify_type == "magprune":
                if self.reserve_p <= self.epsilon / 2 or self.reserve_p >= (
                    1 - self.epsilon
                ):
                    raise ValueError(
    ...
    def from_pretrained(cls, pretrained_model_path, **kwargs):
        ...
        """
        if os.path.isfile(
            os.path.join(pretrained_model_path, MERGE_CONFIG_NAME)
        ):
            config_file = os.path.join(pretrained_model_path, MERGE_CONFIG_NAME)
        else:
            raise ValueError(
                f"Can't find merge_config.json at '{pretrained_model_path}'"
            )
