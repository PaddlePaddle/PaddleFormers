...
ColumnSequenceParallelLoRALinear = lora_layers[
    "ColumnSequenceParallelLoRALinear"
]
...
FleetRowSequenceParallelLoRALinear = lora_layers[
    "FleetRowSequenceParallelLoRALinear"
]
FleetColumnSequenceParallelLoRALinear = lora_layers[
    "FleetColumnSequenceParallelLoRALinear"
]
...
class LoRAModel(nn.Layer):
    ...
    def _get_tensor_parallel_mappings(self, config, is_split=True):
        ...
        if issubclass(type(self.model), tuple(pipeline_layer_types)):
            ...
            for k in keys:
                ...
                rename_lora_split_mapping[".".join(single_name)] = (
                    self.lora_split_mapping[k]
                )
        ...
        if os.path.exists(lora_model_index_file):
            ...
            if load_checkpoint_format == "flex_checkpoint":
                ...
                metadata_path = os.path.join(
                    lora_path, FLEX_CKPT_AUTO_GENERATED_METADATA
                )
    ...
    def set_state_dict(self, state_dict):
        ...
        if self.lora_config.loraga:
            ...
            def process_split_and_assign(
                name, concat_tensor, init_dict, state_dict
            ):
                if "lora_A" in name:
                    axis = 1
                else:
    ...
    def _merge_trainable_tensor_parallel(
        self, trainable_state_dict, offload=True
    ):
            ...
            if key in trainable_name_action_mappings:
                if get_env_device() == "xpu":
                    ret = distributed_allgather(
                        tensor, group=mp_group, offload=offload
                    )
                else:
                ...
                if (
                    key in self.lora_split_mapping
                    and not self.lora_split_mapping[key]
                    and "_scale" in key
                    and is_dst
                ):
                ...
                if offload:
                    trainable_state_dict[key] = (
                        tensor.cpu().numpy() if is_dst else None
                    )
                else:
    ...
    def save_pretrained(
        ...
        if (
            ...
            trainable_state_dict = self._merge_trainable_tensor_parallel(
                trainable_state_dict, offload=not safetensors
            )
        ...
        if any(
            ...
            reverse_key_mapping = {
                v: k
                for k, v in self.model._checkpoint_conversion_mapping.items()
            }
        ...
        if save_checkpoint_format == "flex_checkpoint":
            ...
            if hasattr(self.model, "_gen_lora_inv_aoa_config"):
                aoa_config["aoa_statements"] += (
                    self.model._gen_lora_inv_aoa_config(self.model.config)
                )
            ...
            paddle.save(
                trainable_state_dict, weight_filename, safetensors=safetensors
            )
        ...
        if is_main_process:
            ...
            if save_model_config:
                if hasattr(self.model, "config_to_save"):
                    model_config_to_save = copy.deepcopy(
                        self.model.config_to_save
                    )
                else:
    ...
    def _find_and_replace_module(self, model, module_name, lora_config):
        ...
        lora_module = None
        if isinstance(module, (nn.Linear, FusedLinear)):
            ...
            if self.lora_config.do_qat:
                self.add_lora_split_mapping(
                    module_name + ".weight_quanter._scale", is_column=True
                )
                self.add_lora_split_mapping(
                    module_name + ".activation_quanter._scale", is_column=False
                )
                self.add_lora_split_mapping(
                    module_name + ".activation_quanter.quanter._scale",
                    is_column=False,
                )
            ...
            self.add_lora_split_mapping(
                module_name + ".lora_A", is_column=False
            )

            # for lora qat
            if self.lora_config.do_qat:
                self.add_lora_split_mapping(
                    module_name + ".weight_quanter._scale", is_column=False
                )
                self.add_lora_split_mapping(
                    module_name + ".activation_quanter._scale", is_column=False
                )
            ...
            isinstance(
                module, (FleetColumnParallelLinear, FleetRowParallelLinear)
            )
        ):
            if module.world_size == 1:
                lora_module = FleetLoRALinear(
                    ...
                    is_distributed=getattr(
                        module.weight, "is_distributed", False
                    ),
                )
                ...
                # Lora column parallel will spilt lora B matrix
                self.add_lora_split_mapping(
                    module_name + ".lora_B", is_column=True
                )
        ...
        elif is_paddleformers_available() and isinstance(
            module, FleetQuantizationLinear
        ):
            lora_module = FleetQuantizationLoRALinear(
                module, module.skip_bias_add, lora_config
            )
        elif is_paddleformers_available() and isinstance(
            module, FleetColumnParallelQuantizationLinear
        ):
            lora_module = FleetColumnParallelQuantizationLoRALinear(
                module, module.skip_bias_add, lora_config
            )
            # Lora column parallel will spilt lora B matrix
            self.add_lora_split_mapping(module_name + ".lora_B", is_column=True)
        elif is_paddleformers_available() and isinstance(
            module, FleetRowParallelQuantizationLinear
        ):
            lora_module = FleetRowParallelQuantizationLoRALinear(
                module, module.skip_bias_add, lora_config
            )
        ...
        elif isinstance(module, (MoeExpertsBase, GroupedMLPExpert)):
            cls = MOE_EXPERTS_LORA_MAPPING.get(
                module.__class__.__name__, LoRAMoeExperts
            )
            lora_module = cls(
                module,
                r=lora_config.r,
    ...
    def mark_only_lora_as_trainable(self) -> None:
        for _, layer in self.model.named_sublayers():
            if (
                ...
                or QuantizationLoRALinear is not None
                and isinstance(layer, QuantizationLoRALinear)
                or ColumnParallelQuantizationLoRALinear is not None
                and isinstance(layer, ColumnParallelQuantizationLoRALinear)
                or RowParallelQuantizationLoRALinear is not None
                and isinstance(layer, RowParallelQuantizationLoRALinear)
                or FleetQuantizationLoRALinear is not None
                and isinstance(layer, FleetQuantizationLoRALinear)
                or FleetColumnParallelQuantizationLoRALinear is not None
                and isinstance(layer, FleetColumnParallelQuantizationLoRALinear)
                or FleetRowParallelQuantizationLoRALinear is not None
    ...
    def get_lora_model(
        self, model: PretrainedModel | nn.Layer, lora_config: LoRAConfig
    ):
    ...
    def restore_original_model(self):
        ...
        for layer_name, layer in self.model.named_sublayers():
            if isinstance(layer, LoRALinear):
                ...
                raise NotImplementedError(
                    f"{layer} restoration is not supported yet."
                )
        return self.model
        ...
        for k in model_key_list:
            ...
            if k in weight_to_lora:
                ...
                lora_A_tensor, lora_B_tensor = (
                    base_state_dict.pop(lora_A_key),
                    base_state_dict.pop(lora_B_key),
                )
