...
def decoder_pipeline_forward(self, args):
    ...
    if self.config.recompute and self.config.recompute_granularity == "full" and has_gradient:
        ...
        hidden_states = self.decoder_cls.forward(
...
class CriterionLayerPipe(CriterionLayer):
    ...
    def forward(self, logits, labels):
        if isinstance(labels, tuple) and self.loss_type == "sft":
...
class GeneralModelForCausalLMPipe(PipelinePretrainedModel, PipelineLayer):
    ...
    def __init__(self, config: PretrainedConfig, base_model_prefix: str, decoder_layer, **kwargs):
        ...
        if config.tie_word_embeddings:
            ...
            self.add_sequential_layer(LayerDesc(EmbeddingPipe, config=config), base_model_prefix)
