...
def dpo_logps(
    ...
    **kwargs,
    ...
    rejected_response_lengths = response_indexs[:, 3] - response_indexs[:, 2] - offset
    sft_loss = -chosen_logps.sum() / chosen_response_lengths.sum()

    # 5.average & normalize
    if average_log_prob:
        chosen_logps /= chosen_response_lengths.astype("float32")
        rejected_logps /= rejected_response_lengths.astype("float32")
    elif self.dpo_config.normalize_logps:
        avg_response_length = (chosen_response_lengths + rejected_response_lengths) / 2
        chosen_logps *= avg_response_length / chosen_response_lengths.astype("float32")
        rejected_logps *= avg_response_length / rejected_response_lengths.astype("float32")
