...
class PretrainedMoEGate(nn.Layer, MoEGateMixin):
    ...
    def _topk_noaux_tc(
        ...
        reshape_tmp_rst = scores_for_choice.reshape(
            [bsz_seq_len, self.n_group, -1]
        )
        top_k = min(reshape_tmp_rst.shape[2], 2)
        group_scores = reshape_tmp_rst.topk(top_k, axis=-1)[0].sum(
            axis=-1
        )  # fmt:skip [n, n_group]
        group_idx = paddle.topk(
            group_scores, k=topk_group, axis=-1, sorted=True
    ...
    def top1gating(
        ...
        new_mask1 = mask1 * paddle.zeros_like(mask1).put_along_axis(
            top_idx, paddle.ones([], dtype="float32"), axis=0
        )
        mask1 = new_mask1
    ...
    def _cal_seq_aux_loss(self, gates, top_k, topk_idx) -> paddle.Tensor:
        ...
        if self.config.sequence_parallel:
            ...
            ce.put_along_axis_(
                indices=topk_idx,
                values=paddle.ones_like(topk_idx, dtype=ce.dtype),
                axis=1,
                reduce="add",
            )
            ce = ce / (top_k / local_num_experts)
            gates_mean = paddle.mean(
                gates, axis=tuple(range(len(gates.shape) - 1))
            ).unsqueeze(0)
    ...
    def topkgating(
        ...
        mask = paddle.zeros_like(gates).put_along_axis(
            top_idx, paddle.ones([], dtype="float32"), axis=1
        )
