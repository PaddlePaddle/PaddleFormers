...
class ExpertsGroupGemmNode:
    ...
    def fwd_gate_up(self, x_bf16, expert_w1, expert_w_count, tokens_per_expert):
        ...
        w1_t_quant = w1_t_quant.reshape(
            [expert_w_count, -1, w1_t_quant.shape[-1]]
        )
        w1_t_scale = w1_t_scale.reshape(
            [expert_w_count, -1, w1_t_scale.shape[-1]]
        )
    ...
    def fwd_down(self, o1, unzipped_probs, expert_w_count, tokens_per_expert):
        ...
        if has_config(self.fp8_fused_ops_configs, "stack_quant"):
            w2_quant, w2_scale = fused_stack_transpose_quant(
                expert_w2, transpose=True
            )
        else:
        ...
        unzipped_probs = unzipped_probs.unsqueeze(-1).reshape(
            [expert_w_count, -1, 1]
        )
        ...
        o3 = paddle.zeros(
            [expert_w_count, o2_quant.shape[1], w2_quant.shape[1]],
            dtype=o1.dtype,
        )
    ...
    def fwd_down_no_probs(
        self, o1, expert_w2, expert_w_count, tokens_per_expert
    ):
        """
    ...
    def bwd_down_input(
        ...
        unzipped_grad_scale = paddle.transpose(
            paddle.transpose(unzipped_grad_scale, [0, 2, 1]).contiguous(),
            [0, 2, 1],
        )
        do2_s = paddle.zeros(
    ...
    def bwd_down_weight(self, out_grad, o2, expert_w2):
        ...
        o2_t_scale = paddle.split(
            o2_t_scale, num_or_sections=group_num, axis=-1
        )
    ...
    def bwd_gate_up_weight(self, do1, input_x, expert_w1):
        ...
        input_x_scale = paddle.split(
            input_x_scale, num_or_sections=group_num, axis=-1
        )
    ...
    def forward(self, hs_out, unzipped_probs, tokens_per_expert):
        expert_w1 = [
            x.up_gate_proj.weight
            for x in self.custom_map.experts
            if x is not None
        ]
        expert_w_count = len(expert_w1)

        o1 = self.fwd_gate_up(
            hs_out, expert_w1, expert_w_count, tokens_per_expert
        )
        self.o1 = o1
    ...
    @paddle.no_grad()
    def backward(
        self, out_grad, tokens_per_expert, dispatched_indices, expected_m
    ):
        expert_w2 = [
            x.down_proj.weight for x in self.custom_map.experts if x is not None
        ]
        ...
        dx = self.bwd_gate_up_input(
            do1, expert_w1, tokens_per_expert, expected_m
        )
    ...
    def forward_no_prob(self, hs_out, tokens_per_expert):
        ...
        self.o1 = o1
        o3 = self.fwd_down_no_probs(
            o1, expert_w2, expert_w_count, tokens_per_expert
        )
        self.unzipped_tokens = hs_out
        return o3
...
class ExpertsGroupGemmContiguousNode:
    ...
    def bwd_gate_up_input(self, do1, expert_w1):
        ...
        dx = paddle.empty(
            shape=[do1_fp8.shape[0], bw_w1_quant.shape[1]],
            dtype=paddle.bfloat16,
        )
        if numpy.prod(do1_fp8.shape) != 0:
            if self.is_split_group_gemm:
    ...
    def fused_transpose_split_quant(self, x, tokens_per_expert, pow_2_scales):
        ...
        with paddle.amp.auto_cast(False):
            out, scale = (
                paddle.incubate.nn.functional.fused_transpose_split_quant(
    ...
    def forward(
        ...
        if not self.recompute_fwd_gate_up:
            self.o1 = o1
        o3, unzipped_probs = self.fwd_down(
            o1, unzipped_probs, expert_w2, num_expert
        )
        self.unzipped_probs = unzipped_probs
        return o3
...
class ExpertsGroupGemmWLCHNode(ExpertsGroupGemmContiguousNode):
    ...
    def fused_transpose_split_quant(self, x, tokens_per_expert, pow_2_scales):
        ...
        out, scale = (
            paddle.incubate.nn.functional.fused_transpose_wlch_split_quant(
