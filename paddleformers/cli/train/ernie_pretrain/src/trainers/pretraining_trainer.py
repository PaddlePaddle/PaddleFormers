...
from paddleformers.cli.train.ernie_pretrain.src.clip import (
...
class PreTrainingArguments(TrainingArguments):
    ...
    split: str = field(
        default="949,50,1",
        metadata={"help": "Train/valid/test data split ratio"},
    )
    ...
    global_batch_size: int = field(
        default=-1,
        metadata={
            "help": "if `global_batch_size` and `per_device_train_batch_size` is provided, "
    ...
    moe_group: Optional[str] = field(
        default="dp", metadata={"help": "moe comm group"}
    )
    ...
    def __post_init__(self):
        ...
        if self.pipeline_model_parallel_size > 1:
            ...
            logger.warn(
                f"eval_batch_size set to {self.per_device_eval_batch_size}"
            )
...
class WeightedDistributedSampler(PaddleNLPDistributedBatchSampler):
    ...
    def gen_data_seq(self):
        ...
        for ex in self.inner_dataset.exs:
            total.extend(
                [(ex.part, 0, i) for i in range(ex.data_status, len(ex))]
            )
        assert len(total) > self.num_consecutive, (
            f"total={total} < num_consecutive={self.num_consecutive}"
        )
        indices = np.array_split(
            np.array(total), len(total) // self.num_consecutive
        )
        if self.shuffle:
    ...
    def gen_data_seq_weighted(self, num_examples, data_type=None):
        assert self.load_data_seq is False, (
        ...
        if data_type is not None:
            weights = [
                ex.weights
                for ex in self.inner_dataset.exs
                if ex.data_type == data_type
            ]
            exs = [
                ex for ex in self.inner_dataset.exs if ex.data_type == data_type
            ]
        else:
        ...
        logger.info(
            ...
            + "\n".join(
                ["%-100s...%.3e" % (e.path, w) for w, e in zip(weights, exs)]
            )
        )
        ...
        for part_id in indices:
            ...
            if len(_index) % self.combine_batch != 0:
                _index += [-1] * (
                    self.combine_batch - len(_index) % self.combine_batch
                )
        ...
        if self.shuffle_consecutive and self.combine_batch >= 1:
            ...
            logger.info("consecutive placeholder 2 shuffle................")
            indices_ret = [
                next(part_data_gen_iter[i]) for i in placeholder_indices
            ]
    ...
    def __iter__(self):
        self.rng = random.Random(
            self.seed + self.epoch + self.global_shuffle_seed
        )
        ...
        if any(w is None for w in weights) or sum(weights) == 0.0:
            logger.info(
                f"using normal sampler, num_consecutive={self.num_consecutive}"
            )
            ...
            num_examples = sum(
                [ex.num_examples for ex in self.inner_dataset.exs]
            )

            if self.global_shuffle_num_examples > 0:
                num_examples = min(
                    [self.global_shuffle_num_examples, num_examples]
                )
                logger.info(
        ...
        def ret():
            ...
            buf = []
            logger.info(
                f"start training sequence, data-sequence: {indices[:10]}"
            )
            while 1:
...
class PretrainingTrainer(Trainer):
    def __init__(self, args=None, model=None, callbacks=[], **kwargs):
        ...
        args.use_async_save = (
            args.use_async_save
            and args.save_sharded_model
            and args.load_sharded_model
        )
    ...
    def _load_optimizer_state(self, checkpoint):
        def _broadcast_moe_optimizer_state(state_dict):
            ...
            for k, s in buf[0].items():
                v = state_dict.get(k, paddle.zeros(s, "float32")).to(
                    get_env_device()
                )
                v.name = k
    ...
    def _save_moe_weights(self, output_dir):
        ...
        for k, v in state_dict.items():
            if getattr(v, "no_sync", False):
                ...
                if not (
                    getattr(
                        self.args, "should_save_sharding_stage1_model", False
                    )
    ...
    def evaluate(
        ...
        self.model_wrapped.accumulate_steps = (
    ...
    def restore_dataloader_status(self):
        if self.args.same_data is None or self.args.same_data == "":
            if self.args.resume_from_checkpoint is not None:
                ...
                assert os.path.exists(train_bin_file), (
                    f"{train_bin_file} not found."
                )
                ...
                self.args.same_data = (
                    ...
                    and (
                        ...
                        or old_reeao_data_world_size
                        == self.args.reeao_data_world_size
                    )
                )
                logger.info(
            ...
            logger.info(
                f"User has defined same_data value: {self.args.same_data}"
            )
    ...
    def _maybe_log_save_evaluate(
        ...
        if self.control.should_log:
            ...
            if not hasattr(self, "model_numel"):
                ...
                self.model_numel = (
                    numel_tensor.item() // self.args.dataset_world_size
                )
                ...
                logs.update(
                    {
                        ...
                        for k in {
                            key for item in info_list for key in item.keys()
                        }
                    }
                )
            ...
            if flag_log:
                ...
                if (
                    ...
                    and (
                        ...
                        + self.args.flash_device_save_steps
                    )
                ):
                    actual_tbk = (
                        self._start_save_time - self._first_end_save_time
                    )
                ...
                if is_persistent_ckpt:
                    tokens_per_steps = (
                        self.args.max_seq_len * total_train_batch_size
                    )
                    avg_speed_step = (
                        self.args.save_steps
                        * tokens_per_steps
                    ...
                    logs["average_tokens_per_sec_per_card_without_save"] = (
                        round(avg_speed_step, 1)
                    )
                    ...
                    if (
                        ...
                        logs[
                            "actual_average_tokens_per_sec_per_card_without_save"
                        ] = round(actual_avg_speed_step, 1)
                        logs[
                            "actual_average_tokens_per_sec_per_card_with_save"
                        ] = round(
                            self.args.save_steps
                            * tokens_per_steps
                            / (actual_tbk + ts)
    ...
    def create_scheduler(self, num_training_steps):
        ...
        logger.info(
            f"using wsd lr scheduler, num_steady_steps={num_steady_steps}"
        )
    ...
    def create_optimizer(self, lr_scheduler=None):
        ...
        if self.args.train_moe_only:
            ...
            logger.info(
                f"using `train_moe-only`, #moe params={len(optimizer_params)}"
            )
        ...
        if self.optimizer is None:
            ...
            self.optimizer = optimizer_cls(
                ...
                lr_ratio=(
                    lr_ratio_fn
                    if (self.args.moe_gate_lr_ratio is not None)
                    else None
                ),
                **optimizer_kwargs,
            )
