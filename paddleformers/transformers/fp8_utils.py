...
try:
    ...
    try:
        ...
        deep_gemm.set_num_sms = lambda num: setattr(
            deep_gemm.jit_kernels.utils, "_num_sms", num
        )
...
def set_parameter_color(
    parameters, color, group=None, offline_quant_expert_weight=True
):
    if offline_quant_expert_weight:
        if group is None:
            ...
            for p in parameters:
                if hasattr(p, "color") and p.color is not None:
                    continue
                p.color = {"color": color, "group": group}
...
def weight_quant(weight, transpose=False):
    if transpose:
        if hasattr(weight, "fp8_weight_transpose"):
            ...
            return (
                weight.fp8_weight.T.contiguous(),
                weight.fp8_scale.T.contiguous(),
            )
        else:
        ...
        if hasattr(weight, "fp8_weight"):
            ...
            return (
                weight.fp8_weight_transpose.T.contiguous(),
                weight.fp8_scale_transpose.T.contiguous(),
            )
        else:
            return paddle.incubate.nn.functional.fp8_quant_blockwise(
                weight,
...
class FP8LinearFunctionBase:
    ...
    def dequantize_fp8_to_fp32(fp8_tensor, scale):
        res = fp8_tensor.reshape([-1, 128]).astype("bfloat16") * (
            scale.reshape([-1, 1])
        )
    ...
    def padding_and_quant_input(tensor):
        ...
        if tensor.shape[0] % 512 != 0:
            ...
            tensor_fp8, tensor_scale, tensor_t_fp8, tensor_t_scale = (
                paddle.incubate.nn.functional.fp8_quant_blockwise(
                    tensor,
                    output_scale_transpose=True,
                    quant_method="1x128",
    ...
    def kitchen_gemm(
        ...
        if (
            ...
            if numpy.prod(x_fp8.shape) != 0 and numpy.prod(w_fp8.shape) != 0:
                recipe = (
                    (1, 1, 128) if (is_a_1d_scaled and is_b_1d_scaled) else None
                )
    ...
    def compute_fp8_linear(
        ...
        *,
        ...
        # check input
        is_input_quantized = (
            isinstance(input, (tuple, list)) and len(input) == 2
        )
        ...
        if is_input_quantized:
            ...
            if return_mode == "with_input_transpose_quant":
                raise RuntimeError(
                    "Cannot return transposed quant if input is already quantized. "
                    "Use raw input instead."
                )
        else:
            # quant input (with optional transposed output)
            if return_mode == "with_input_transpose_quant":
                input_fp8, input_scale, input_t_fp8, input_t_scale = (
                    FP8LinearFunctionBase.padding_and_quant_input(input)
                )
            else:
        ...
        if out is None:
            out = paddle.empty(
                [input_fp8.shape[0], weight_fp8.shape[0]], dtype=weight.dtype
            )

        deep_gemm.set_num_sms(get_sm_num())
        deep_gemm.fp8_gemm_nt(
            (input_fp8, input_scale.T), (weight_fp8, weight_scale), out
        )
    ...
    def common_fp8_mlp_bwd(
        ...
        if o1 is not None and (x_fp8 is not None or x_scale is not None):
            raise ValueError(
                "When o1 is provided, both x_fp8 and x_scale must be None."
            )

        if o1 is None:
            if x_fp8 is None or x_scale is None:
                raise ValueError(
                    "When o1 is None, both x_fp8 and x_scale must be provided."
                )
            ...
            o1 = paddle.empty(
                [x_fp8.shape[0], w1_fp8.shape[0]], dtype=do3.dtype
            )
    ...
    def fp8_mlp_fwd(x, w1, w2):
        ...
        o3 = FP8LinearFunctionBase.compute_fp8_linear(
            o2, w2, weight_transpose=True, return_transpose_only=True
        )
    ...
    def fp8_mlp_fwd_norm_rc(x, norm_w, norm_eps, w1, w2):
        # compute norm_output
        norm_output, _ = paddle.incubate.nn.functional.fused_rms_norm_ext(
            x, norm_w, norm_eps
        )
        # compute fp8_mlp_fwd
    ...
    def fp8_mlp_bwd(do3, x, w1, w2, apply_backward_hook=False):
        ...
        x_fp8, x_scale, x_t_fp8, x_t_scale = (
            FP8LinearFunctionBase.padding_and_quant_input(x)
        )
    ...
    def fp8_mlp_bwd_norm_rc(do3, x, norm_w, norm_eps, w1, w2):
        ...
        # compute fp8_mlp_fwd
        d_norm_output = FP8LinearFunctionBase.fp8_mlp_bwd(
            do3, norm_output, w1, w2, True
        )

        # ===== compute norm grad =====
        dx, d_rms_norm_weight = paddle._C_ops.fused_rms_norm_ext_grad(
            x, norm_w, invar, d_norm_output, norm_eps
        )
        if hasattr(norm_w, "main_grad"):
            if norm_w.main_grad is None:
                norm_w.main_grad = paddle.zeros(
                    shape=norm_w.shape, dtype=paddle.float32
                )
            norm_w.main_grad += d_rms_norm_weight
        else:
            if norm_w.grad is None:
                norm_w.grad = paddle.zeros(
                    shape=norm_w.shape, dtype=paddle.float32
                )
...
class FP8Linear(paddle.nn.Layer):
    def __init__(
        self, in_features: int, out_features: int, bias_attr: bool = False
    ) -> None:
        super().__init__()
...
def cache_fp8_weight(weight, quant_transpose=None):
    ...
    if quant_transpose is None:
        w_fp8, w_scale, w_t_fp8, w_t_scale = (
            paddle.incubate.nn.functional.fp8_quant_blockwise(
                weight,
                output_scale_transpose=False,
                quant_method="128x128",
        ...
        weight.fp8_weight_transpose = w_t_fp8
        weight.fp8_scale_transpose = w_t_scale
        weight.fp8_weight = w_fp8
        weight.fp8_scale = w_scale
...
class FusedNormFP8MLPFunction(paddle.autograd.PyLayer):
    ...
    def backward(ctx, do3):
        ...
        norm_output, invar, x, norm_w, w1, w2, norm_eps, x_orig_shape = (
            ctx.saved_tensor()
        )
        ...
        # reshape to origin shape
        if len(x_orig_shape) > 2:
            d_norm_output = d_norm_output.reshape(
                [x_orig_shape[0], -1, d_norm_output.shape[-1]]
            )
...
class FP8Mlp(paddle.nn.Layer):
    ...
    def forward(self, x):
        if self.using_post_norm_recompute:
            return FusedNormFP8MLPFunction.apply(
                x, self.norm_weight, self.w1, self.w2, self.norm_eps
            )
        else:
            return FP8MlpFunction.apply(
                x, self.w1, self.w2, self.recompute_fwd_gate_up
            )
...
class FP8GroupGemmMlpFunctionNode:
    ...
    def fwd_gate_up(
        self, x, expert_w1, num_expert, tokens_per_expert, m_indices=None
    ):
        """
        o1 = x * w1
    ...
    def fwd_down(
        ...
        if hasattr(expert_w2[0], "fp8_weight_stacked") and not hasattr(
        ...
        if numpy.prod(o2_fp8.shape) != 0:
            if self.is_split_group_gemm:
                split_group_gemm(
                    o2_fp8, o2_scale, w2_quant, w2_scale, tokens_per_expert, o3
                )
            else:
                deep_gemm.m_grouped_fp8_gemm_nt_contiguous(
                    (o2_fp8, o2_scale),
    ...
    def bwd_dowm_input(
        ...
        do2_s = paddle.empty(
            [unzipped_grad_fp8.shape[0], bw_w2_quant.shape[1]], dtype="bfloat16"
        )
    ...
    def bwd_gate_up_input(
        self, do1, expert_w1, tokens_per_expert, m_indices=None, dx=None
    ):
        """
        dx = do1 * w1_t
        [m_sum, k] = [m_sum, n] * [num_groups, n, k]
    ...
    def bwd_gate_up_weight(
        ...
        if input_x is None:
            inp = (
                (input_fp8_slice, input_scale_slice)
                if self.bwd_subbatch
                else (self.input_fp8, self.input_scale)
            )
    ...
    @paddle.no_grad()
    def forward(
        self, hs_out, unzipped_probs, tokens_per_expert, m_indices=None
    ):
        # check subbatch
        if self.fwd_subbatch:
        ...
        # o1
        o1 = self.fwd_gate_up(
            hs_out, expert_w1, num_expert, tokens_per_expert, m_indices
        )
    ...
    def backward(
        ...
        dtype = paddle.bfloat16
        shape = (
            out_grad[0].shape if isinstance(out_grad, tuple) else out_grad.shape
        )
        if shape[0] == 0:
            return paddle.zeros_like(
                extract_first_if_tuple(out_grad), dtype=dtype
            ), paddle.zeros_like(unzipped_probs)
        ...
        if self.recompute_fwd_gate_up:
            inp = (
                None
                if not self.bwd_subbatch
                else (input_fp8_slice, input_scale_slice)
            )
            o1 = self.fwd_gate_up(
                inp,
                expert_w1,
        ...
        if isinstance(out_grad, tuple):
            ...
            self.bwd_down_weight(
                (do3_fp8, do3_scale), o2_s, expert_w2, tokens_per_expert
            )
        else:
