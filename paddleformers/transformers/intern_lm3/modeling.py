...
_triton_apply_rotary_pos_emb = None
try:
    from ...triton_kernels.rope_triton import (
        apply_rotary_pos_emb_vision as _triton_apply_rotary_pos_emb,
    )
except Exception:
    pass

from paddle.incubate.nn.functional import (
    fused_rotary_position_embedding as _fused_rotary_position_embedding,
)
...
class InternLM3Attention(nn.Layer):
    ...
    def forward(
        ...
        shape = (batch_size, seq_len, -1, self.head_dim)

        query_states = self.q_proj(hidden_states).reshape(shape)
...
class InternLM3DecoderLayer(nn.Layer):
    def __init__(self, config: InternLM3Config, layer_idx: int):
        super().__init__()
        self.config = config
...
class InternLM3ForCausalLM(InternLM3PretrainedModel):
    ...
    @property
    def chat(self):
        """Check if chat method is available (for compatibility)"""
        return True
...
class InternLM3ForCausalLMPipe(GeneralModelForCausalLMPipe):
    config_class = InternLM3Config
    _decoder_layer_cls = InternLM3DecoderLayer
    _get_tensor_parallel_mappings = InternLM3Model._get_tensor_parallel_mappings
    _init_weights = InternLM3Model._init_weights
    _keep_in_fp32_modules = InternLM3Model._keep_in_fp32_modules
    _tied_weights_keys = ["lm_head.weight"]
    transpose_weight_keys = InternLM3Model.transpose_weight_keys
    _gen_aoa_config = InternLM3ForCausalLM._gen_aoa_config
    _gen_inv_aoa_config = InternLM3ForCausalLM._gen_inv_aoa_config
