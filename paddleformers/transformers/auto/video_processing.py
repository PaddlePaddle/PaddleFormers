...
VIDEO_PROCESSOR_MAPPING = _LazyAutoMapping(
    CONFIG_MAPPING_NAMES, VIDEO_PROCESSOR_MAPPING_NAMES
)
...
class AutoVideoProcessor:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if "AutoVideoProcessor" in config_dict.get("auto_map", {}):
            video_processor_auto_map = config_dict["auto_map"][
                "AutoVideoProcessor"
            ]

        if video_processor_class is None and video_processor_auto_map is None:
            image_processor_class = config_dict.pop(
                "image_processor_type", None
            )
            if image_processor_class is not None:
                video_processor_class_inferred = image_processor_class.replace(
                ...
                if (
                    video_processor_class_inferred
                    in VIDEO_PROCESSOR_MAPPING_NAMES.values()
                ):
            ...
            if "AutoImageProcessor" in config_dict.get("auto_map", {}):
                ...
                video_processor_auto_map = image_processor_auto_map.replace(
                    "ImageProcessor", "VideoProcessor"
                )
        ...
        if video_processor_class is None and video_processor_auto_map is None:
            ...
            video_processor_class = getattr(
                config, "video_processor_type", None
            )
            if (
                hasattr(config, "auto_map")
                and "AutoVideoProcessor" in config.auto_map
            ):
                video_processor_auto_map = config.auto_map["AutoVideoProcessor"]

        if video_processor_class is not None:
            video_processor_class = video_processor_class_from_name(
                video_processor_class
            )

        has_remote_code = video_processor_auto_map is not None
        has_local_code = (
            video_processor_class is not None
            or type(config) in VIDEO_PROCESSOR_MAPPING
        )
        ...
        if has_remote_code and trust_remote_code:
            ...
            video_processor_class = get_class_from_dynamic_module(
