...
class KTOCriterion(nn.Layer):
    ...
    def __init__(
        self, config, kto_config=None, ignore_label=0, use_infohub=False
    ):
        ...
        if kto_config is None:
            if getattr(self.config, "kto_config", None) is None:
                raise ValueError(
                    "KTO Criterion requires model_config.kto_config."
                )
    ...
    def kto_logps(
        self, logits, response_labels, response_kl_labels, response_indexs
    ):
        """KTO logprobs"""
    ...
    def kto_loss(
        ...
        if dist.get_world_size() > 1:
            kl = (
                self._nested_gather(paddle.tile(kl, repeat_times=[1, 1]))
                .mean()
                .clip(min=0)
            )
        if (
            ...
            chosen_losses = 1 - F.sigmoid(
                self.kto_config.beta * (chosen_logratios - kl)
            )
        if (
            ...
            rejected_losses = 1 - F.sigmoid(
                self.kto_config.beta * (kl - rejected_logratios)
            )
        losses = paddle.cat(
            (
