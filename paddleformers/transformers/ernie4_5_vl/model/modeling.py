    ...
    def __init__(self, config, layer_idx=0):
        ...
        if config.tensor_parallel_degree > 1:
            ...
            self.down_proj = LinearFN(
                self.intermediate_size, self.hidden_size, bias_attr=config.text_config.use_bias
            )
...
class Ernie4_5_LMHead(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.weight.is_distributed = (
            True if (vocab_size != config.text_config.vocab_size) else False
        )
        if config.text_config.weight_share_add_bias and config.text_config.use_bias:
        ...
        if (
            config.text_config.weight_share_add_bias
            and config.text_config.use_bias
