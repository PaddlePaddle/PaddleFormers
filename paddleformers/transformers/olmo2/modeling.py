...
from ...nn.mlp import MLP as GeneralMLP
...
class Olmo2Attention(nn.Layer):
    def __init__(self, config: Olmo2Config, layer_idx: int = 0):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = self.q_norm(self.q_proj(hidden_states)).reshape(q_shape).transpose(1, 2)
        key_states = self.k_norm(self.k_proj(hidden_states)).reshape(kv_shape).transpose(1, 2)
...
class Olmo2PretrainedModel(PretrainedModel):
    ...
    def _gen_aoa_config(cls, config: Olmo2Config):
        ...
        if config.attention_bias:
            aoa_statements.extend(
                [
                    f"model.layers.$LAYER_ID.self_attn.{name}.bias -> {model_prefix}layers.$LAYER_ID.self_attn.{name}.bias"
                    for name in ["q_proj", "k_proj", "v_proj", "o_proj"]
                ]
            )
    ...
    def _gen_inv_aoa_config(cls, config: Olmo2Config):
        ...
        if config.attention_bias:
            aoa_statements.extend(
                [
                    f"{model_prefix}layers.$LAYER_ID.self_attn.{name}.bias -> model.layers.$LAYER_ID.self_attn.{name}.bias"
                    for name in ["q_proj", "k_proj", "v_proj", "o_proj"]
                ]
            )
...
class Olmo2Model(Olmo2PretrainedModel):
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = paddle.arange(cache_length, seq_length + cache_length, dtype=paddle.int64).unsqueeze(0)
...
__all__ = ["Olmo2ForCausalLM", "Olmo2ForCausalLMPipe", "Olmo2Model", "Olmo2PretrainedModel"]
