...
from ..qwen2 import fusion_ops
from ..qwen2.modeling import get_use_casual_mask
...
class Qwen23RotaryEmbedding(Qwen2RotaryEmbedding):
    def __init__(
        self,
        dim,
...
class Qwen2NTKScalingRotaryEmbedding(Qwen2RotaryEmbedding):
...
class ConcatMaskedLoss(PyLayer):
    ...
    def backward(ctx, grad):
        ...
        return grad
class Qwen2PretrainingCriterion(paddle.nn.Layer):
    """
    Criterion for Llama.
...
class Qwen2Attention(nn.Layer):
    ...
    def _init_rope(self):
        if (
            ...
            self.rotary_emb = Qwen23RotaryEmbedding(
                self.head_dim,
            ...
            self.rotary_emb = Qwen2NTKScalingRotaryEmbedding(
                self.head_dim,
                max_position_embeddings=self.max_position_embeddings,
                scaling_factor=self.config.rope_scaling_factor,
                base=self.config.rope_theta,
...
class Qwen2PretrainedModel(PretrainedModel):
    ...
    pretrained_init_configuration = QWEN2_PRETRAINED_INIT_CONFIGURATION
    pretrained_resource_files_map = QWEN2_PRETRAINED_RESOURCE_FILES_MAP
