    ...
    def _cal_seq_aux_loss(self, probs, top_k, routing_map):
        ...
        if self.config.tensor_model_parallel_size > 1:
            assert (
                ...
                and max_seq_len % self.config.tensor_model_parallel_size == 0
            )
            local_seq_len = (
                sub_max_seq_len // self.config.tensor_model_parallel_size
            )
            # [B*S, E]
...
class PretrainedMoEGate(nn.Layer, MoEGateMixin):
    def __init__(self, config, num_experts, expert_hidden_size, **kwargs):
        ...
        self.config = config
        self.scoring_func = (
            config.scoring_func if hasattr(config, "scoring_func") else None
        )
