...
from .configuration import InternLMConfig
...
class InternLMAttention(nn.Layer):
    def __init__(self, config: InternLMConfig, layer_idx: int):
        super().__init__()
        self.config = config
...
class InternLMDecoderLayer(nn.Layer):
    def __init__(self, config: InternLMConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.self_attn = InternLMAttention(config=config, layer_idx=layer_idx)
        self.mlp = MLP(config)
...
class InternLMRotaryEmbedding(nn.Layer):
    ...
    def compute_default_rope_parameters(
        config: Optional[InternLMConfig] = None,
...
class InternLMPretrainedModel(PretrainedModel):
    ...
    # The HF and Paddle parameter names are identical, but Linear weights still
    # need `^T` conversion. Prevent the generic flex-checkpoint identity dtype
    # rules from shadowing the transpose rules in `_gen_aoa_config`.
    is_fleet = True
    ...
    @classmethod
    def _gen_aoa_config(cls, config: InternLMConfig):
    ...
    @classmethod
    def _gen_inv_aoa_config(cls, config: InternLMConfig):
        model_prefix = cls.base_model_prefix + "." if cls != cls.base_model_class else ""
...
class InternLMModel(InternLMPretrainedModel):
    def __init__(self, config: InternLMConfig):
        ...
        self.layers = nn.LayerList(
            [InternLMDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        ...
        self.rotary_emb = InternLMRotaryEmbedding(config=config)
...
class InternLMForCausalLM(InternLMPretrainedModel):
    ...
    def forward(
        ...
        output_attentions: bool | None = None,
        output_hidden_states: bool | None = None,
        return_dict: bool | None = None,  # true when decode, false when pretrain & eval
        **kwargs,
    ):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            if cache_length == 0 and isinstance(past_key_values, tuple) and len(past_key_values) > 0:
...
class InternLMForCausalLMPipe(GeneralModelForCausalLMPipe):
    config_class = InternLMConfig
    _decoder_layer_cls = InternLMDecoderLayer
    _get_tensor_parallel_mappings = InternLMModel._get_tensor_parallel_mappings
    _init_weights = InternLMModel._init_weights
    _keep_in_fp32_modules = InternLMModel._keep_in_fp32_modules
