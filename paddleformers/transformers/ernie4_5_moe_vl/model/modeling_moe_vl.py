...
class ErniePretrainingCriterion(ErniePretrainingCriterionBase):
    ...
    def forward(
        ...
        Args:
            ...
            scores_image: only contains image special-token logits。
...
class Ernie4_5_VLMoeForConditionalGeneration(Ernie4_5_MoeForCausalLM):
    ...
    def _gen_inv_aoa_config(cls, config):
        ...
        aoa_config["aoa_statements"] += [
            f"{'lm_head.weight' if config.tie_word_embeddings else 'lm_head.weight^T'} -> lm_head.weight",
        ]
        ...
        for i, input_ids in enumerate(total_input_ids):
            ...
            for _ in range(image_nums + video_nums):
                ...
                if ed_image < ed_video:
                    ...
                    pixel_length = ed_grid_thw.prod().item()
                    images.append(
                        pixel_values[
                    ...
                    remain_images -= 1
                    ed = ed_image
                    last_image_pixel_index += pixel_length
                    vision_type = "image"
                else:
                    t, h, w = (
                        video_grid_thw[video_index][0],
                    ...
                    remain_videos -= 1
                    ed = ed_video
                    last_video_pixel_index += pixel_length
                    vision_type = "video"
                llm_grid_t, llm_grid_h, llm_grid_w = (
                    t.item()
                    if t.item() == 1
        ...
        if self.training and self.modality_detach:
            ...
            def fwdfn(
                ...
                nonlocal \
                    ...
                    mm_input_ids, \
                    image_type_ids
                """During the backward of this function, the stop_gradient attribute of param is reset"""
