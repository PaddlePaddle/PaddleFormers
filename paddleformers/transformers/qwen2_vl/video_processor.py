...
class Qwen2VLVideoProcessor(BaseVideoProcessor):
    ...
    max_frames = 768
    do_sample_frames = (
        False  # Set to False for BC, recommended to set `True` in new models
    )
    ...
    def sample_frames(
        ...
        **kwargs,
        ...
        if num_frames is not None:
            ...
            max_frames = (
                math.floor(
                    min(max_frames, total_num_frames) / temporal_patch_size
                )
                * temporal_patch_size
            )
            num_frames = total_num_frames / metadata.fps * fps
            num_frames = min(
                max(num_frames, min_frames), max_frames, total_num_frames
            )
            num_frames = (
                math.floor(num_frames / temporal_patch_size)
                * temporal_patch_size
            )
        ...
        for shape, stacked_videos in grouped_videos.items():
            ...
            if patches.shape[1] % temporal_patch_size != 0:
                repeats = patches[:, -1:].repeat(
                    1, self.temporal_patch_size - 1, 1, 1, 1
                )
    ...
    def get_num_of_video_patches(
        ...
        """
        min_pixels = (
            videos_kwargs.get("min_pixels", None) or self.size["shortest_edge"]
        )
        max_pixels = (
            videos_kwargs.get("max_pixels", None) or self.size["longest_edge"]
        )
        ...
        temporal_patch_size = (
            videos_kwargs.get("temporal_patch_size", None)
            or self.temporal_patch_size
        )
