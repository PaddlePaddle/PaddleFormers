...
class BlendableDataset(paddle.io.Dataset):
    def __init__(
        self, datasets, weights, size, share_folder, *, data_cache_path=None
    ):
        ...
        def _build_indices():
            ...
            fast_dataindex_version = importlib.metadata.version(
                "fast_dataindex"
            )
        ...
        if data_cache_path:
            ...
            if share_folder:
                check_rank_flag = (
                    not cache_hit and paddle.distributed.get_rank() == 0
                )
            ...
            #     exit()
                ...
                while True:
                    if (not os.path.isfile(index_path)) or (
                        not os.path.isfile(sample_index_path)
                    ):
        ...
        try:
            ...
            pass
        print_rank_0(f"> size of blendable dataset: {self.size} samples")
