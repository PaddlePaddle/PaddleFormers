...
class LLaVAModel(FleetLayer):
    """LLaVA multi-modal model.

    Args:
        language_transformer_config (TransformerConfig): Transformer config for the language model.
        language_transformer_layer_spec (LayerSpec): Language model spec.
        language_vocab_size (int): Language model vocabulary size.
        language_max_sequence_length (int): Language model maximum sequence length.
        vision_transformer_config (TransformerConfig): Transformer config for the vision model.
        vision_transformer_layer_spec (LayerSpec): Vision model spec.
        ...
        vision_projection_layer_spec (LayerSpec): Vision projection spec.
    ...
    def __init__(
        ...
        language_transformer_layer_spec: LayerSpec,
        language_vocab_size: int,
        language_max_sequence_length: int,
        vision_transformer_config: TransformerConfig,
        vision_transformer_layer_spec: LayerSpec,
        drop_vision_class_token: bool,
        vision_projection_config: TransformerConfig,
        vision_projection_layer_spec: LayerSpec,
    ...
    def _preprocess_data(
        ...
        *,
        ...
        Returns:
            final_embedding (paddle.Tensor): image and text embeddings [combined_seq_len, b, h].
            final_labels (paddle.Tensor): labels for image and text positions [b, combined_seq_len].
        ...
        if self.post_process and has_labels:
            ...
            batch_image_indices, image_indices = paddle.where(image_token_mask)
    ...
    def _process_embedding_token_parallel(
        ...
        Returns:
            combined_embeddings (paddle.Tensor): image and text embeddings combined and distributed.
            new_labels (paddle.Tensor): Distributed labels for image and text positions.
            new_loss_mask (paddle.Tensor): Distributed loss mask.
    ...
    def _apply_tile_tagging(self, image_embeddings, num_image_tiles):
        ...
        Args:
            image_embeddings (paddle.Tensor): [img_seq_len, num_tiles, h_language].
            num_image_tiles (paddle.Tensor): Number of tiles for each input image [num_images].
        ...
        tile_tag_input_ids = paddle.tensor(
            tile_tags,
            dtype=paddle.int64,
        )
        ...
        image_embeddings = paddle.cat([tile_tag_embeds, image_embeddings])
    ...
    def forward(
        ...
        num_image_tiles: list[int] | None = None,
        ...
        *,
        ...
        Args:
            images (paddle.Tensor): input images of shape [num_tiles, img_h, img_w].
                num_tiles means the number of image tiles in this batch.
                num_tiles = 0 if the batch doesn't contain images.
            input_ids (paddle.Tensor): input text ids [batch, text_seq_len].
            position_ids (paddle.Tensor): input text position ids [batch, text_seq_len].
            attention_mask (paddle.Tensor): Language model attention mask
            ...
            loss_mask (paddle.Tensor): Text loss mask [batch, text_seq_len].
...
def _load_state_dict_hook_ignore_param_names(
    ...
    Args:
        ...
        module (paddle.nn.Layer): The paddle module this hook applies to. Required by the paddle API.
...
def pixel_shuffle(x, scale_factor=0.5, version=2):
    ...
    Args:
        x (paddle.Tensor): Vision model outputs [num_tiles, img_seq_len, h_vision]
