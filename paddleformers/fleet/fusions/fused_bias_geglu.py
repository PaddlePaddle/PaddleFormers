...
def bias_geglu(bias, y):
    ...
    Args:
        bias (paddle.Tensor): Bias tensor to be added to the input.
        y (paddle.Tensor): Input tensor to be split and gated.

    Returns:
        paddle.Tensor: Result of bias addition followed by GEGLU activation.
    """
    y = y + bias
    return geglu(y)
...
def bias_geglu_back(g, y, bias):
    ...
    Returns:
        paddle.Tensor: Gradient with respect to the input tensor after bias addition.
    """
    y = y + bias
    return geglu_back(g, y)


class BiasGeGLUFunction(paddle.autograd.PyLayer):
    ...
    def forward(ctx, input, bias):
        ...
        Args:
            ...
            input (paddle.Tensor): Input tensor to apply GEGLU to.
            bias (paddle.Tensor): Bias tensor to be added to input before GEGLU.

        Returns:
            paddle.Tensor: Result of applying bias addition followed by GEGLU activation.
        """
...
class GeGLUFunction(paddle.autograd.PyLayer):
    ...
    def forward(ctx, input):
        ...
        Returns:
            paddle.Tensor: Result of applying GEGLU activation.
        """
...
def bias_geglu_impl(input, bias):
    ...
    Args:
        input (paddle.Tensor): Input tensor to apply GEGLU activation.
        bias (paddle.Tensor, optional): Bias tensor to be added to input. If None,
            uses the bias-free GEGLU variant.

    Returns:
        paddle.Tensor: Result of biased GEGLU activation.
...
@jit_fuser
def quick_gelu(y: paddle.Tensor) -> paddle.Tensor:
    """Sigmoid approximation of gelu"""
    return y * paddle.sigmoid(1.702 * y)


@jit_fuser
def quick_geglu(y: paddle.Tensor, linear_offset: float = 0.0) -> paddle.Tensor:
...
@jit_fuser
def quick_geglu_back(g, y, linear_offset: float = 0.0) -> paddle.Tensor:
    """Backward helper for Quick-GEGLU.

    Args:
        g (paddle.Tensor): Upstream gradient tensor.
        y (paddle.Tensor): Input tensor used in the forward pass.
...
def weighted_bias_quick_geglu_back(
    ...
    return (
        input_grad.to(input_dtype),
        bias_grad.to(input_dtype),
        weights_grad.to(w_dtype),
    )


class WeightedQuickGeGLUFunction(paddle.autograd.PyLayer):
    ...
    def forward(
        ...
        Args:
            ctx: Autograd context object for saving tensors for backward pass.
            input (paddle.Tensor): Input tensor of shape [N, 2H].
            weights (paddle.Tensor): Per-token weights of shape [N, 1].
            fp8_input_store (bool): If True, stores input for backward in FP8.
            linear_offset (paddle.Tensor): Scalar tensor offset added to the linear half.
    ...
    def backward(ctx, grad_output):
        ...
        Args:
            ctx: Autograd context object containing saved tensors from forward pass.
            grad_output (paddle.Tensor): Upstream gradient w.r.t. the output.
...
class WeightedBiasQuickGeGLUFunction(paddle.autograd.PyLayer):
    ...
    def forward(
        ...
        Args:
            ctx: Autograd context object for saving tensors for backward pass.
            input (paddle.Tensor): Input tensor of shape [N, 2H].
            bias (paddle.Tensor): Bias tensor of shape [N, 1].
            weights (paddle.Tensor): Per-token weights of shape [N, 1].
            fp8_input_store (bool): If True, stores input for backward in FP8.
            linear_offset (paddle.Tensor): Scalar tensor offset added to the linear half.
