...
if _CUTILE_AVAILABLE:
    ...
    def _cutile_sinkhorn_fwd(input_logits: Tensor, num_iterations: int, eps: float = 1e-8) -> tuple[Tensor, Tensor]:
    ...
    def _cutile_sinkhorn_bwd(
        ...
        TILE_SIZE = math.gcd(N_batch, 128)
        ws_M = paddle.empty(shape=[N_batch * 2 * num_iterations, hc, hc], dtype="float32")
        ws_rs = paddle.empty(shape=[N_batch * num_iterations, hc, 1], dtype="float32")
        ws_cs = paddle.empty(shape=[N_batch * num_iterations, 1, hc], dtype="float32")
        grad_input = paddle.empty(shape=[N_batch, hc, hc], dtype=grad_output.dtype)
        ct.launch(
            _get_cuda_stream(),
            (math.ceil(N_batch / TILE_SIZE), 1, 1),
    ...
    def _cutile_h_aggregate_bwd(grad_output: Tensor, x: Tensor, h_pre: Tensor) -> tuple[Tensor, Tensor]:
        s, b, n, C = x.shape
        sb = s * b
        TILE_C = math.gcd(C, 1024)
    ...
    @ct.kernel
    def _ct_hpb_fwd_kernel(hr, orig, hp, x, out, N: ConstInt, TILE_C: ConstInt, TILE_SIZE: ConstInt):
        pid = ct.bid(0)
    ...
    def _ct_hpb_bwd_kernel(
        ...
        for ct_idx in range(num_c_tiles):
            ...
            for j in range(N):
                g_x_2d += ct.extract(hp_2d, (0, j), shape=(1, 1)).item() * ct.extract(go_2d, (j, 0), shape=(1, TILE_C))
                g_orig_2d += ct.extract(hr_2d, (0, j), shape=(N, 1)) * ct.extract(go_2d, (j, 0), shape=(1, TILE_C))
    ...
    def _ct_proj_rms_bwd_small_k_kernel(
        ...
        if ct.bid(1) == 1:
            for tile_id in range(ct.bid(0), NUM_DA_TILES, ct.num_blocks(0)):
                ...
                accumulator_da = accumulator_da + _ct_rms_dnorm(a_tile.astype(ct.float32), norm_tile, dr_tile, K, eps)
                b_tile = ct.load(
                    B,
                    index=(0, b_tile_idx),
    ...
    def _cutile_proj_rms_fwd(x: Tensor, weight: Tensor, eps: float = 1e-8) -> tuple[Tensor, Tensor, Tensor]:
        M, K = x.shape
        N = weight.shape[0]
        TILE_M = 128
...
if not _CUTILE_AVAILABLE:
    ...
    class FusedHPostBDA(paddle.autograd.PyLayer):
        ...
        def backward(ctx, grad_output):
            ...
            if ctx.has_bias:
                ...
                g_hr, g_res, g_hp, g_x, g_bias = _cutile_h_post_bda_bwd(grad_output, h_res, orig_res, h_post, x, bias)
                ...
                g_hr, g_res, g_hp, g_x, _ = _cutile_h_post_bda_bwd(grad_output, h_res, orig_res, h_post, x, None)
                if ctx.x_stop_gradient:
                    g_x = None
    ...
    class FusedProjRms(paddle.autograd.PyLayer):
        ...
        def backward(ctx, grad_proj, grad_r):
            ...
            grad_x, grad_weight = _cutile_proj_rms_bwd(grad_proj_2d, grad_r_2d, x_2d, weight, norm, ctx.eps)
    ...
    def fused_sinkhorn(input_logits: Tensor, num_iterations: int, eps: float = 1e-6) -> Tensor:
        ...
        assert input_logits.shape[-1] == input_logits.shape[-2], (
            f"fused_sinkhorn: last two dims must be equal (square matrix), " f"got shape {list(input_logits.shape)}"
        )
        hc = input_logits.shape[-1]
        N_batch = input_logits.size // (hc * hc)
        assert N_batch <= _INT32_MAX, f"fused_sinkhorn: N_batch={N_batch} exceeds int32 max ({_INT32_MAX})"
    ...
    def fused_h_aggregate(x: Tensor, h_pre: Tensor) -> Tensor:
        ...
        """
        assert x.ndim == 4, f"fused_h_aggregate: x must be 4D [s,b,n,C], got shape {list(x.shape)}"
        assert h_pre.ndim == 3, f"fused_h_aggregate: h_pre must be 3D [s,b,n], got shape {list(h_pre.shape)}"
        ...
        s, b, n, C = x.shape
        assert s * b <= _INT32_MAX, f"fused_h_aggregate: s*b={s * b} exceeds int32 max ({_INT32_MAX})"
        assert C <= _INT32_MAX, f"fused_h_aggregate: C={C} exceeds int32 max ({_INT32_MAX})"
    ...
    def fused_h_post_bda(
        ...
        assert s * b <= _INT32_MAX, f"fused_h_post_bda: s*b={s * b} exceeds int32 max ({_INT32_MAX})"
        assert C <= _INT32_MAX, f"fused_h_post_bda: C={C} exceeds int32 max ({_INT32_MAX})"
    ...
    def fused_proj_rms(x: Tensor, weight: Tensor, eps: float = 1e-6) -> tuple[Tensor, Tensor]:
        ...
        # [K, N] --> [N, K]
        weight = weight.t()
        assert weight.ndim == 2, f"fused_proj_rms: weight must be 2D [N, K], got shape {list(weight.shape)}"
        K = x.shape[-1]
        N, K_w = weight.shape
        assert K == K_w, (
        ...
        M = x.size // K
        assert M <= _INT32_MAX, f"fused_proj_rms: M={M} (x reshaped to [M, K]) exceeds int32 max ({_INT32_MAX})"
        assert K <= _INT32_MAX, f"fused_proj_rms: K={K} exceeds int32 max ({_INT32_MAX})"
