...
def is_pp_last_stage(pp_group: paddle.distributed.communication.group.Group):
...
def get_pp_first_rank(pp_group: paddle.distributed.communication.group.Group):
    """Return the global rank of the first rank in the pipeline parallel group."""
    pp_ranks = pp_group.ranks()
    return pp_ranks[0]


def get_pp_last_rank(pp_group: paddle.distributed.communication.group.Group):
    """Return the global rank of the last rank in the pipeline parallel group."""
    pp_ranks = pp_group.ranks()
    return pp_ranks[-1]


def get_pp_next_rank(pp_group: paddle.distributed.communication.group.Group):
...
def get_pp_prev_rank(pp_group: paddle.distributed.communication.group.Group):
...
# NOTE(zhangweilong): ScheduleNode is a subgraph of the pipeline, capable of independently calling
...
class ScheduleNode:
    ...
    def __init__(
        ...
        backward_func: Callable | None = None,
        ...
        Args:
            ...
            stream (paddle.cuda.Stream): The CUDA stream for this node's computation.
            ...
            event (paddle.cuda.Event): The CUDA event used for synchronization. Each
    ...
    def _forward(self, *inputs):
        ...
        # NOTE(zhangweilong): Paddle does not have record_stream *untyped_storage APIs so currently does not support them
    ...
    def _backward(self, *output_grad):
        with stream_acquire_context(self.stream, self.event):
            paddle.cuda.nvtx.range_push(f"{self.name} backward")
            with paddle.cuda.stream(self.stream):
                outputs = self.output
        ...
        # NOTE(zhangweilong): Paddle does not have record_stream APIs so temp skip ,may occur resource leakage.
...
def set_streams(comp_stream=None, comm_stream=None):
    ...
    if comp_stream is None:
        comp_stream = paddle.cuda.current_stream()
    if comm_stream is None:
        comm_stream = paddle.cuda.Stream(device="cuda")
