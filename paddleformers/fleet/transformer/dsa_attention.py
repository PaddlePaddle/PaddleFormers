...
def hadamard_transform(x: Tensor, scale: float = 1.0) -> Tensor:
    ...
    assert dim > 0 and (dim & (dim - 1)) == 0, f"hadamard_transform requires dim to be a power of 2, got {dim}"
...
def rotate_activation(x: Tensor) -> Tensor:
    ...
    Returns:
        Rotated tensor.
    """
    assert x.dtype == paddle.bfloat16, f"rotate_activation only support bf16 input, but got {x.dtype}"
...
def _unfused_dsa_attention(
    ...
    attn_scores = paddle.bmm(q.cast("float32"), k.cast("float32").transpose([0, 2, 1])) * softmax_scale

    # Apply combined mask (causal + sparse index mask)
    if combined_mask is not None:
        mask = combined_mask.expand([b, nhpp, s, s]).contiguous().reshape([b * nhpp, s, s])
    ...
    output = output.reshape([b, nhpp, s, v_hd]).transpose([0, 2, 1, 3]).reshape([b, s, nhpp * v_hd])
...
class DSAIndexer(paddle.nn.Layer):
    ...
    def __init__(
        ...
        if config.rope_type == "rope":
            self.rotary_pos_emb = RotaryEmbedding(
                self.rope_head_dim,
                rotary_percent=1.0,
                rotary_interleaved=getattr(config, "dsa_indexer_rotary_interleaved", False),
            ...
            raise ValueError(f"Unsupported RoPE type: {config.rope_type}, " "supported types are 'rope' and 'yarn'")

    def _apply_rope(self, x: Tensor, freqs: Tensor, mscale: float = 1.0) -> Tensor:
    ...
    def forward_before_topk(
        ...
        if self.config.sequence_parallel and self.pg_collection.tp.nranks > 1:
            ...
            q_latent = gather_from_sequence_parallel_region(q_latent, group=self.pg_collection.tp)
        ...
        if self.config.rope_type == "rope":
            ...
            freqs, mscale = self.rotary_pos_emb(rotary_seq_len, packed_seq=False)
    ...
    def compute_index_scores(
        ...
        topk_indices = paddle.clip(topk_indices, min=0, max=index_scores.shape[-1] - 1)
    ...
    def forward(
        ...
        index_scores, topk_indices = self.compute_index_scores(q, k, weights, attention_mask)
        return index_scores, topk_indices


def _compute_index_scores_fused(q: Tensor, weights: Tensor, k: Tensor) -> Tensor:
    ...
    """
    # q @ k^T -> [sq, b, h, sk]
    index_scores = paddle.einsum("sbhd,tbd->sbht", q.cast("float32"), k.cast("float32"))
    # ReLU activation
...
def _compute_dsa_indexer_loss(
    ...
    attention_scores = paddle.bmm(query_reshaped.cast("float32"), key_reshaped.cast("float32")) * softmax_scale
    ...
    if causal_mask_override is not None:
        ...
        attention_scores = paddle.where(attn_row_mask, attention_scores, paddle.zeros_like(attention_scores))
        index_scores = paddle.where(idx_row_mask, index_scores, paddle.zeros_like(index_scores))
    ...
    if tp_group is not None and tp_group.nranks > 1:
        paddle.distributed.all_reduce(attention_scores.contiguous(), group=tp_group)
    # L1 normalize target on the last dimension
    attention_scores = attention_scores / attention_scores.sum(axis=-1, keepdim=True).clip(min=1e-10)

    # KL divergence: KL(target || index) = target * log(target / index)
    kl_per_element = attention_scores * (paddle.log(attention_scores + 1e-10) - paddle.log(index_scores + 1e-10))
...
def _bwd_fused_indexer_loss(
    ...
    attention_scores_softmax = F.softmax(attention_scores, axis=-1, dtype="float32")
    ...
    if causal_mask_override is not None:
        attention_scores_softmax = attention_scores_softmax * attn_row_mask.cast("float32")
        index_scores_softmax = index_scores_softmax * idx_row_mask.cast("float32")
    ...
    if tp_group is not None and tp_group.nranks > 1:
        paddle.distributed.all_reduce(attention_scores_sum.contiguous(), group=tp_group)

    # L1 normalize
    attention_scores_normalized = attention_scores_sum / attention_scores_sum.sum(axis=-1, keepdim=True).clip(
        min=1e-10
    )
    ...
    sum_grad = (grad_index_scores_softmax * index_scores_softmax).sum(axis=-1, keepdim=True)
    ...
    if causal_mask_override is not None:
        ...
        causal_valid_mask = paddle.tril(paddle.ones([sq, sk], dtype="bool")).unsqueeze(0).expand([b, sq, sk])
    del causal_mask
    ...
    grad_index_scores_logits = grad_index_scores_logits * valid_mask.cast("float32")
    del valid_mask

    # Transpose from [b, sq, sk] to [sq, b, sk]
    grad_index_scores = grad_index_scores_logits.transpose([1, 0, 2])  # [sq, b, sk]
    ...
    scores = paddle.einsum("sbhd,tbd->sbht", q.cast("float32"), k.cast("float32"))  # [sq, b, h, sk]
    relu_mask = scores > 0
    ...
    grad_weights = (grad_weighted_scores * scores_after_relu).sum(axis=-1)  # [sq, b, h]
    ...
    grad_scores = grad_scores_after_relu * relu_mask.cast("float32")  # [sq, b, h, sk]
    del grad_scores_after_relu, relu_mask

    # Backward through einsum 'sbhd,tbd->sbht'
    # ∂L/∂q = einsum('sbht,tbd->sbhd', grad_scores, k)
    grad_q = paddle.einsum("sbht,tbd->sbhd", grad_scores, k.cast("float32"))  # [sq, b, h, d]
    # ∂L/∂k = einsum('sbht,sbhd->tbd', grad_scores, q)
    grad_k = paddle.einsum("sbht,sbhd->tbd", grad_scores, q.cast("float32"))  # [sk, b, d]
    del grad_scores
...
class FusedDSAIndexerLoss(paddle.autograd.PyLayer):
    ...
    def forward(
        ...
        with paddle.amp.auto_cast(False):
            ...
            topk_values, topk_indices = paddle.topk(masked_scores, k=topk_k, axis=-1)
            # Clamp indices to valid range: paddle.topk may return garbage indices
            # for -inf input values
            topk_indices = paddle.clip(topk_indices, min=0, max=masked_scores.shape[-1] - 1)
...
class DSAIndexerLossLoggingHelper:
    ...
    def register_total_num_layers(config):
        DSAIndexerLossLoggingHelper.num_layers = DSAIndexerLossLoggingHelper.get_total_num_layers(config)
    ...
    def reduce_loss_in_tracker(num_layers: int | None = None):
        ...
        # DP avg
        dp_group = parallel_state.get_data_parallel_group(check_initialized=False)
    ...
    def track_indexer_metrics(
        ...
        if csa_compress_ratios is not None:
            num_indexer_layers = sum(1 for ratio in csa_compress_ratios if ratio == 4)
        else:
        ...
        if writer is not None:
            writer.add_scalar("indexer loss", avg_indexer_loss.item(), iteration)
...
class DSAttention(FleetLayer):
    ...
    def __init__(
        ...
        # DSA loss config
        self.dsa_indexer_loss_coeff = getattr(config, "dsa_indexer_loss_coeff", None)
        self.dsa_indexer_use_sparse_loss = getattr(config, "dsa_indexer_use_sparse_loss", False)
    ...
    def forward(
        ...
        if x is None or qr is None:
            raise ValueError(
                "DSAttention requires x and qr parameters. " "These are passed by MultiLatentAttention when using DSA."
            )
        ...
        assert x.dtype == paddle.bfloat16, f"DSAttention: x must be bfloat16, got {x.dtype}"
        assert qr.dtype == paddle.bfloat16, f"DSAttention: qr must be bfloat16, got {qr.dtype}"
        ...
        if attn_mask_type is not None and attn_mask_type == AttnMaskType.causal:
            ...
            indexer_float_mask = causal_mask.unsqueeze(0).unsqueeze(0)  # [1, 1, sq, sk]
        elif attention_mask is not None:
            mask = attention_mask.squeeze(1)
            indexer_float_mask = paddle.zeros_like(mask, dtype="float32").masked_fill(mask.cast("bool"), float("-inf"))
        ...
        if self.training and self.dsa_indexer_loss_coeff is not None:
            ...
            weights_idx_sf = weights_idx.transpose([1, 0, 2])  # [b,s,h] -> [s,b,h]
            ...
            indexer_loss = FusedDSAIndexerLoss.apply(
                ...
                if self.pg_collection.tp is not None and self.pg_collection.tp.nranks > 1
                else None,
            )
        ...
        index_mask = paddle.put_along_axis(index_mask, topk_indices, zeros, axis=-1)
        # Merge causal + index
        ...
        core_attn_out = _unfused_dsa_attention(query, key, value, combined_mask, self.softmax_scale)

        # Attach indexer loss if training
        if self.training and indexer_loss is not None:
            if self.dsa_indexer_loss_coeff is not None and self.dsa_indexer_loss_coeff > 0:
                DSAIndexerLossLoggingHelper.save_loss_to_tracker(
                    loss=indexer_loss,
                    layer_number=self.layer_number,
                    num_layers=DSAIndexerLossLoggingHelper.get_total_num_layers(self.config),
                )
            core_attn_out = DSAIndexerLossAutoScaler.apply(core_attn_out, indexer_loss)
