...
def _validate_indexer_inputs(index_q, index_k_comp, weights):
    ...
    if len(index_q.shape) != 4:
        raise ValueError(f"index_q must have shape [B, S, H_i, D_i], got {index_q.shape}")
    if len(index_k_comp.shape) != 3:
        raise ValueError(f"index_k_comp must have shape [B, S_comp, D_i], got {index_k_comp.shape}")
    if len(weights.shape) != 3:
        raise ValueError(f"weights must have shape [B, S, H_i], got {weights.shape}")
...
def _validate_topk_and_grad(index_q, topk_indices, grad_scores):
    ...
    if len(topk_indices.shape) != 3:
        raise ValueError(f"topk_indices must have shape [B, S, topk], got {topk_indices.shape}")
    if len(grad_scores.shape) != 3:
        raise ValueError(f"grad_scores must have shape [B, S, topk], got {grad_scores.shape}")
    batch, seq_len, _, _ = index_q.shape
    if topk_indices.shape != grad_scores.shape:
        raise ValueError(f"topk_indices shape {topk_indices.shape} must match grad_scores shape {grad_scores.shape}")
    topk_batch, topk_seq_len, topk_last_dim = topk_indices.shape
    if topk_batch != batch or topk_seq_len != seq_len:
        raise ValueError(f"topk_indices shape {topk_indices.shape} is incompatible with index_q shape {index_q.shape}")
    if topk_last_dim <= 0:
...
def csa_indexer_topk_fwd(
    ...
    if topk_indices.shape != expected_shape or topk_scores.shape != expected_shape:
        raise RuntimeError(
...
def csa_attn_target_reducesum(
    ...
    if len(query_mla.shape) != 4:
        raise ValueError(f"query_mla must have shape [B, S, H, D], got {query_mla.shape}")
    if len(key_comp_mla.shape) != 3:
        raise ValueError(f"key_comp_mla must have shape [B, S_comp, D], got {key_comp_mla.shape}")
    if len(topk_indices.shape) != 3:
        raise ValueError(f"topk_indices must have shape [B, S, topk], got {topk_indices.shape}")
    if query_mla.shape[0] != key_comp_mla.shape[0] or query_mla.shape[0] != topk_indices.shape[0]:
        raise ValueError(
    ...
    if query_mla.shape[1] != topk_indices.shape[1]:
        raise ValueError(f"sequence mismatch: query_mla={query_mla.shape}, topk_indices={topk_indices.shape}")
    if query_mla.shape[3] != key_comp_mla.shape[2]:
        raise ValueError(f"dim mismatch: query_mla={query_mla.shape}, key_comp_mla={key_comp_mla.shape}")
    if topk_indices.dtype != paddle.int32:
        topk_indices = topk_indices.cast("int32")
    topk_indices = topk_indices.contiguous()
    ...
    if target.shape != expected_shape:
        raise RuntimeError(f"unexpected CSA attention target shape: target={target.shape}, expected={expected_shape}")
...
def csa_indexer_bwd(
    ...
    if grad_q.shape != index_q.shape or grad_weights.shape != weights.shape or grad_k_comp.shape != index_k_comp.shape:
        raise RuntimeError(
